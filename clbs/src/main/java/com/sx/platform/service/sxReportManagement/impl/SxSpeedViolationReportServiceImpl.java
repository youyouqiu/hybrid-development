...
import com.sx.platform.domain.sxReport.SxSpeedViolationReport;
import com.sx.platform.service.sxReportManagement.SxSpeedViolationReportService;
...
@Service
public class SxSpeedViolationReportServiceImpl implements SxSpeedViolationReportService {
    ...
    public JsonResultBean getListFromPaas(String vehicleList, String startTime, String endTime) throws Exception {
        List<SxSpeedViolationReport> result = new ArrayList<>();
        ...
        String sendResult = HttpClientUtil.send(PaasCloudUrlEnum.SX_SPEED_ALARM_STATISTICS_URL, param);
        ...
        if (CollectionUtils.isNotEmpty(jsonArray)) {
            ...
            for (int i = 0; i < jsonArray.size(); i++) {
                ...
                SxSpeedViolationReport report = new SxSpeedViolationReport();
                String monitorId = object.getString("monitorId");
                report.setBrnad(object.getString("monitorName"));
                // 经纬度.
                report.setStartLocation(object.getString("overSpeedStartAddress"));
                report.setProfessionalName(object.getString("professionalsName"));
                report.setAssignmentName(object.getString("assignmentName"));
                Long overSpeedTime = object.getLong("overSpeedTime");
                report.setDurationTime(DateUtil.formatTime(overSpeedTime * 1000));
                report.setAlarmCount(object.getString("overSpeedNumber"));
                Long time = object.getLong("overSpeedStartTime");
        ...
        RedisKey redisKey = HistoryRedisKeyEnum.SX_SPEED_VIOLATION_REPORT_INFORMATION.of(userId);
        // 再次查询前删除 key
    ...
    public boolean export(String title, int type, HttpServletResponse res) throws Exception {
        String userId = userService.getCurrentUserInfo().getId().toString();
        List<SxSpeedViolationReport> sxSpeedViolationReport =
                RedisHelper.getList(HistoryRedisKeyEnum.SX_SPEED_VIOLATION_REPORT_INFORMATION.of(userId),
                        SxSpeedViolationReport.class);
        // 逆地址编码
        final Set<String> lngLats = new HashSet<>(CommonUtil.ofMapCapacity(sxSpeedViolationReport.size() * 2));
        for (SxSpeedViolationReport violationReport : sxSpeedViolationReport) {
            lngLats.add(violationReport.getStartLocation());
        }
        final Map<String, String> addressMap = AddressUtil.batchInverseAddress(lngLats);
        for (SxSpeedViolationReport violationReport : sxSpeedViolationReport) {
            violationReport.setStartLocation(addressMap.get(violationReport.getStartLocation()));
        }
        return ExportExcelUtil.export(
            new ExportExcelParam(title, type, sxSpeedViolationReport, SxSpeedViolationReport.class, null,
                res.getOutputStream()));
    }
}
