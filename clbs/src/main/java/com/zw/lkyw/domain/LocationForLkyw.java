 ...
 */
@Data
public class LocationForLkyw implements T808MsgBody {
     ...
     */
    private JSONArray alarmStartTimeList = new JSONArray();
    ...
    /**
     * 限速值(两客一危实时监控-全部车辆，新增字段，取自驾驶员引起报警-超速报警值)
     */
    private Integer driverSpeedLimit;

    /**
     * 终端当日里程（不再通过计算得到，直接从最后一条位置数据获取。）
     */
    private String deviceDayMile = "0.0";

    /**
     * 传感器当日里程（不再通过计算得到，直接从最后一条位置数据获取。）
     */
    private String sensorDayMile = "0.0";

    /**
     * 当日油耗（不再通过计算得到，直接从最后一条位置数据获取。）
     */
    private String dayOilWear = "0.0";
