...
/**
 * 修复InMemoryTokenStore的removeAccessToken方法的bug
 * @author Chen Feng
...
public class CustomTokenStore implements TokenStore {
    ...
    private final ConcurrentHashMap<String, Collection<OAuth2AccessToken>> clientIdToAccessTokenStore =
        new ConcurrentHashMap<>();
    ...
    private final ConcurrentHashMap<String, OAuth2Authentication> refreshTokenAuthenticationStore =
        new ConcurrentHashMap<>();
    ...
    private void addToCollection(ConcurrentHashMap<String, Collection<OAuth2AccessToken>> store, String key,
        ...
        store.computeIfAbsent(key, k -> new ConcurrentHashSet<>()).add(token);
    }
    ...
    public void removeAccessToken(String tokenValue) {
        ...
        if (authentication != null) {
            ...
            // 此处原为tokens = this.userNameToAccessTokenStore.get(authentication.getName())
            tokens = this.userNameToAccessTokenStore.get(getApprovalKey(authentication));
            if (tokens != null) {
