...
@Data
@NoArgsConstructor
public class VehicleInfo implements Serializable, ConverterDateUtil {
    ...
    public VehicleInfo(VehicleDTO vehicle) {
        BeanUtils.copyProperties(vehicle, this);
        this.aliases = vehicle.getAlias();
        this.vehiclet = vehicle.getVehicleTypeName();
        this.brand = vehicle.getName();
        this.fuelType = vehicle.getFuelTypeName();
        this.groupId = vehicle.getOrgId();
        this.groupName = vehicle.getOrgName();
        this.assignmentId = vehicle.getGroupId();
        this.simcardNumber = vehicle.getSimCardNumber();
        this.simcardId = vehicle.getSimCardId();
        this.bindId = vehicle.getConfigId();
        this.assignmentName = vehicle.getGroupName();
        this.assign = vehicle.getGroupName();
        this.vehicleSubtypes = vehicle.getVehicleSubType();
        this.deviceNumber = vehicle.getDeviceNumber();
        if (Objects.nonNull(vehicle.getPhoneCheck())) {
            this.phoneCheck = Short.parseShort(String.valueOf(vehicle.getPhoneCheck()));
        }
        if (Objects.nonNull(vehicle.getNumberLoad())) {
            this.numberLoad = Short.parseShort(String.valueOf(vehicle.getNumberLoad()));
        }

        if (Objects.nonNull(vehicle.getManagementType())) {
            this.managementType = Short.parseShort(String.valueOf(vehicle.getManagementType()));
        }
        if (Objects.nonNull(vehicle.getStateRepair())) {
            this.stateRepair = Short.parseShort(String.valueOf(vehicle.getStateRepair()));
        }

        if (Objects.nonNull(vehicle.getPurchaseWay())) {
            this.purchaseWay = Short.parseShort(String.valueOf(vehicle.getPurchaseWay()));
        }
        if (Objects.nonNull(vehicle.getLicenseNumbers())) {
            this.licenseNumbers = Short.parseShort(String.valueOf(vehicle.getLicenseNumbers()));
        }
        if (Objects.nonNull(vehicle.getInternalSizeHigh())) {
            this.internalSizeHigh = Long.parseLong(String.valueOf(vehicle.getInternalSizeHigh()));
        }
    }
}
