...
public class RodSensorForm extends BaseFormBean implements Serializable {
     ...
     * 传感器长度
     */
    @Pattern(message = "【传感器长度】输入错误，请输入合法的数字！",regexp = "^(?:[1-9][0-9]*(?:\\.[0-9]+)?|0\\.(?!0+$)[0-9]+)$", groups = { ValidGroupAdd.class, ValidGroupUpdate.class})
    @ExcelField(title = "传感器长度")
    ...
    @Pattern(message = "【滤波系数】输入错误，只能输入1,2,3,其中1:实时,2:平滑,3:平稳！",regexp = "^[1-3]{1}$", groups = { ValidGroupAdd.class, ValidGroupUpdate.class})
    ...
    @Min(value=1,message = "【奇偶校验】输入错误，只能输入1,2,3,其中1:奇校验,2:偶校验,3:无校验！",groups = { ValidGroupAdd.class, ValidGroupUpdate.class})
    @Max(value=3,message = "【奇偶校验】输入错误，只能输入1,2,3,其中1:奇校验,2:偶校验,3:无校验！",groups = { ValidGroupAdd.class, ValidGroupUpdate.class})
