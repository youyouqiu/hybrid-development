...
public class Reflections {
        ...
        for (Field field : fields) {
            ...
            try {
                if (!"serialVersionUID".equals(field.getName())) {
                    ...
                    result.append(field.get(o) == null ? "" : field.get(o)).append(";");
                }
            } catch (Exception e) {
                logger.error("获取类中所有属性值异常", e);
            }
        }
        if (result.indexOf(",") >= 0) {
    ...
    private static <T> void setVal(String fieldName, Integer value, T entity) {
        Class cls = entity.getClass();
        cls = getFieldCls(cls, fieldName);
        try {
    ...
    private static Class getFieldCls(Class cls, String fieldName) {
        try {
    ...
    public static <T> void setValIfPresent(String fieldName, Integer value, T entity) {
        Optional.ofNullable(value).ifPresent(val -> setVal(fieldName, val, entity));
