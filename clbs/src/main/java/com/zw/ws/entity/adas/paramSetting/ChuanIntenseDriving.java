...
/**
 * @Description:主动安全参数(川标) (激烈驾驶参数设置)
 */
@Data
public class ChuanIntenseDriving implements T808MsgBody, AdasParamCommonMethod {
    /**
     * 激烈驾驶报警使能
     */
    ...
    public ChuanIntenseDriving(AdasParamSettingForm paramSettingForm) {
    ...
    private void handelAlarmParam(AdasAlarmParamSetting paramSetting, String key) {
        ...
        if (params.length == 3) {
            //车速阈值
            setValIfPresent(params[1], IntegerUtil.getOrDefault(paramSetting.getSpeedThreshold(), 0xFFFF));
            //发动机转速阈值
            setValIfPresent(params[2], IntegerUtil.getOrDefault(paramSetting.getEngineThreshold(), 0xFFFF));
        }
    }
