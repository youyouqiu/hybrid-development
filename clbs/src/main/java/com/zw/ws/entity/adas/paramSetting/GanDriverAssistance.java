...
/**
 * @Description:主动安全参数(赣标) (前向参数设置)
 */
@Data
@EqualsAndHashCode(callSuper = false)
public class GanDriverAssistance extends PublicParameters implements T808MsgBody {
    /**
     * 报警使能
     */
    ...
    static {
        ...
        String[][] alarmEnableOrder =
            {{"196407", "0,1"}, {"196405", "2,3"}, {"196402", "4,5"}, {"196401", "6,7"}, {"196404", "8,9"},
                {"196403", "10,11"}, {"196409", "16"}};
        for (String[] ints : alarmEnableOrder) {
            assistAlarmEnableMap.put(ints[0], ints[1]);
        }
        //报警事件参数设置参数交互字段维护
        String[][] assistAlarmParamOrder =
            {{"196407", "obstacleDistance,obstacleSpeed,obstacleVideoTime,obstacleCameraNum,obstacleCameraTime"},
                {"196405", "laneChangeTime,laneChangeSpeed,laneChangeVideoTime,laneChangeCameraNum,"
                    + "laneChangeCameraTime,laneChangeNum"},
                {"196402", "null,deviateSpeed,deviateVideoTime,deviateCameraNum,deviateCameraTime"},
                {"196401", "vehicleCollisionTime,vehicleCollisionSpeed,vehicleCollisionVideoTime,"
                    + "vehicleCollisionCameraNum,vehicleCollisionCameraTime"},
                {"196404", "pedestrianCollisionTime,pedestrianCollisionSpeed,pedestrianCollisionVideoTime,"
                ...
                {"196409", "speedLimitCameraNum,speedLimitCameraTime"}
            };
    ...
    public GanDriverAssistance(AdasParamSettingForm paramSettingForm) {
    ...
    private void handelAlarmParam(AdasAlarmParamSetting paramSetting, String key) {
        ...
        if (!"196402".equals(key) || !"196405".equals(key)) {
            //时距阈值
