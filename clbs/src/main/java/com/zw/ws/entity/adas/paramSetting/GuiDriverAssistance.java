...
/**
 * @Description:主动安全参数(桂标) (前向参数设置)
 */
@Data
@EqualsAndHashCode(callSuper = false)
public class GuiDriverAssistance extends PublicParameters implements T808MsgBody {
    /**
     * 报警使能
     */
    ...
    static {
        ...
        String[][] alarmEnableOrder =
            {{"146407", "0,1"}, {"146405", "2,3"}, {"146402", "4,5"}, {"146401", "6,7"}, {"146404", "8,9"},
                {"146403", "10,11"}, {"146409", "16"}};
        for (String[] ints : alarmEnableOrder) {
            assistAlarmEnableMap.put(ints[0], ints[1]);
        }
        //报警事件参数设置参数交互字段维护
        String[][] assistAlarmParamOrder =
            {{"146407", "obstacleDistance,obstacleSpeed,obstacleVideoTime,obstacleCameraNum,obstacleCameraTime"},
                {"146405", "laneChangeTime,laneChangeSpeed,laneChangeVideoTime,laneChangeCameraNum,"
                    + "laneChangeCameraTime,laneChangeNum"},
                {"146402", "null,deviateSpeed,deviateVideoTime,deviateCameraNum,deviateCameraTime"},
                {"146401", "vehicleCollisionTime,vehicleCollisionSpeed,vehicleCollisionVideoTime,"
                    + "vehicleCollisionCameraNum,vehicleCollisionCameraTime"},
                {"146404", "pedestrianCollisionTime,pedestrianCollisionSpeed,pedestrianCollisionVideoTime,"
                ...
                {"146409", "speedLimitCameraNum,speedLimitCameraTime"}
            };
    ...
    public GuiDriverAssistance(AdasParamSettingForm paramSettingForm) {
    ...
    private void handelAlarmParam(AdasAlarmParamSetting paramSetting, String key) {
        ...
        //146402 车道偏离,146405 频繁变道
        if (!"146402".equals(key) || !"146405".equals(key)) {
            //时距阈值
