...
/**
 * @Description:主动安全参数(沪标) (前向参数设置)
 */
@Data
@EqualsAndHashCode(callSuper = false)
public class HuDriverAssistance extends PublicParameters implements T808MsgBody {
    /**
     * 报警使能
     */
    ...
    static {
        ...
        String[][] alarmEnableOrder =
            {{"206407", "0,1"}, {"206405", "2,3"}, {"206402", "4,5"}, {"206401", "6,7"}, {"206404", "8,9"},
                {"206403", "10,11"}, {"206409", "16"}};
        for (String[] ints : alarmEnableOrder) {
            assistAlarmEnableMap.put(ints[0], ints[1]);
        }
        //报警事件参数设置参数交互字段维护
        String[][] assistAlarmParamOrder =
            {{"206407", "obstacleDistance,obstacleSpeed,obstacleVideoTime,obstacleCameraNum,obstacleCameraTime"},
                {"206405", "laneChangeTime,laneChangeSpeed,laneChangeVideoTime,laneChangeCameraNum,"
                    + "laneChangeCameraTime,laneChangeNum"},
                {"206402", "null,deviateSpeed,deviateVideoTime,deviateCameraNum,deviateCameraTime"},
                {"206401", "vehicleCollisionTime,vehicleCollisionSpeed,vehicleCollisionVideoTime,"
                    + "vehicleCollisionCameraNum,vehicleCollisionCameraTime"},
                {"206404", "pedestrianCollisionTime,pedestrianCollisionSpeed,pedestrianCollisionVideoTime,"
                ...
                {"206409", "speedLimitCameraNum,speedLimitCameraTime"}
            };
    ...
    public HuDriverAssistance(AdasParamSettingForm paramSettingForm) {
    ...
    private void handelAlarmParam(AdasAlarmParamSetting paramSetting, String key) {
        ...
        //206402 车道偏离,206405 频繁变道
        if (!"206402".equals(key) || !"206405".equals(key)) {
            //时距阈值
