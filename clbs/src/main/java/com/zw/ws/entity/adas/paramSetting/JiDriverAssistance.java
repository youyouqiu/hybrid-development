...
/**
 * @Description:主动安全参数(冀标) (前向参数设置)
 */
@Data
@EqualsAndHashCode(callSuper = false)
public class JiDriverAssistance extends PublicParameters implements T808MsgBody {
    /**
     * 报警使能
     */
    ...
    static {
        ...
        String[][] alarmEnableOrder =
            {{"136407", "0,1"}, {"136405", "2,3"}, {"136402", "4,5"}, {"136401", "6,7"}, {"136404", "8,9"},
                {"136403", "10,11"}, {"136409", "16"}};
        for (String[] ints : alarmEnableOrder) {
            assistAlarmEnableMap.put(ints[0], ints[1]);
        }
        //报警事件参数设置参数交互字段维护
        String[][] assistAlarmParamOrder =
            {{"136407", "obstacleDistance,obstacleSpeed,obstacleVideoTime,obstacleCameraNum,obstacleCameraTime"},
                {"136405", "laneChangeTime,laneChangeSpeed,laneChangeVideoTime,laneChangeCameraNum,"
                    + "laneChangeCameraTime,laneChangeNum"},
                {"136402", "null,deviateSpeed,deviateVideoTime,deviateCameraNum,deviateCameraTime"},
                {"136401", "vehicleCollisionTime,vehicleCollisionSpeed,vehicleCollisionVideoTime,"
                    + "vehicleCollisionCameraNum,vehicleCollisionCameraTime"},
                {"136404", "pedestrianCollisionTime,pedestrianCollisionSpeed,pedestrianCollisionVideoTime,"
                ...
                {"136409", "speedLimitCameraNum,speedLimitCameraTime"}
            };
    ...
    public JiDriverAssistance(AdasParamSettingForm paramSettingForm) {
    ...
    private void handelAlarmParam(AdasAlarmParamSetting paramSetting, String key) {
        ...
        if (!"136402".equals(key) || !"136405".equals(key)) {
            //时距阈值
