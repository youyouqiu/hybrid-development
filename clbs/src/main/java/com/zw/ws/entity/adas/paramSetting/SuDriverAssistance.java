...
/**
 * @Description:主动安全参数(苏标) (前向参数设置)
 */
@Data
@EqualsAndHashCode(callSuper = false)
public class SuDriverAssistance extends PublicParameters implements T808MsgBody {
    /**
     * 报警使能
     */
    ...
    static {
        ...
        String[][] alarmEnableOrder =
            {{"156407", "0,1"}, {"156405", "2,3"}, {"156402", "4,5"}, {"156401", "6,7"}, {"156404", "8,9"},
                {"156403", "10,11"}, {"156409", "16"}};
        for (String[] ints : alarmEnableOrder) {
            assistAlarmEnableMap.put(ints[0], ints[1]);
        }
        //报警事件参数设置参数交互字段维护
        String[][] assistAlarmParamOrder =
            {{"156407", "obstacleDistance,obstacleSpeed,obstacleVideoTime,obstacleCameraNum,obstacleCameraTime"},
                {"156405", "laneChangeTime,laneChangeSpeed,laneChangeVideoTime,laneChangeCameraNum,"
                    + "laneChangeCameraTime,laneChangeNum"},
                {"156402", "null,deviateSpeed,deviateVideoTime,deviateCameraNum,deviateCameraTime"},
                {"156401", "vehicleCollisionTime,vehicleCollisionSpeed,vehicleCollisionVideoTime,"
                    + "vehicleCollisionCameraNum,vehicleCollisionCameraTime"},
                {"156404", "pedestrianCollisionTime,pedestrianCollisionSpeed,pedestrianCollisionVideoTime,"
                ...
                {"156409", "speedLimitCameraNum,speedLimitCameraTime"}
            };
    ...
    public SuDriverAssistance(AdasParamSettingForm paramSettingForm) {
    ...
    private void handelAlarmParam(AdasAlarmParamSetting paramSetting, String key) {
        ...
        //156402 车道偏离,156405 频繁变道
        if (!"156402".equals(key) || !"156405".equals(key)) {
            //时距阈值
