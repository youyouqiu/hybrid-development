...
(function ($, window) {
    ...
    var socketTypeArr = ['F864', 'F865', 'F866', 'F867'];//[前向监测, 驾驶员行为监测, 胎压监测, 盲区监测]
    var sensorIDArr = [100, 101, 102, 103];//[前向监测, 驾驶员行为监测, 胎压监测, 盲区监测]
    ...
    chuanPerInfo = {
        init: function () {
            ...
            for(var i=0;i<socketTypeArr.length;i++){
                ...
                chuanPerInfo.subscribeParam(type);//socket订阅
                chuanPerInfo.getDatas(sensorID);//下发指令
            }
        },
        //websocket订阅
        subscribeParam: function (type) {
            webSocket.subscribe(headers, "/topic/per" + type + '_' + vid + "_Info", function (data) {
                chuanPerInfo.getSensor0104Param(data, type);
            }, null, null);
        },
        getSensor0104Param: function (data, type) {
            ...
            chuanPerInfo.queryF3BaseParamCall(result, type);
        },
        //input赋值
        ...
        getDatas: function (sensorID) {
            json_ajax("POST", '/clbs/adas/standard/param/getJiPeripheralInfo', "json", false, {
                ...
                "sensorID": sensorID,
                "commandType": 248,
            }, chuanPerInfo.getF3BaseParamCall);
        },
        getF3BaseParamCall: function (data) {
            if (!data.success) {
                ...
                chuanPerInfo.createSocket0104InfoMonitor();
            }
        },
        ...
        readInformationRefreshClick: function () {
            ...
            for(var i=0;i<sensorIDArr.length;i++){
                var sensorid = sensorIDArr[i];
                chuanPerInfo.getDatas(sensorid);
            }
        },
    }
    ...
    $(function () {
        chuanPerInfo.init();
        //刷新
        $("#readPerStateRefresh").on("click", chuanPerInfo.readInformationRefreshClick);
    })
})($, window)