(function (window, $) {
    ...
    fatigueDrivingMonitor = {
        init: function () {
            // 显示隐藏列
            fatigueDrivingMonitor.columnRender();
            // 月份日期控件初始化
            fatigueDrivingMonitor.monthDataInit();
            // 初始化监控对象树
            fatigueDrivingMonitor.initTree();
        },
        inquireClick: function (number) {
            $(".ToolPanel").css("display", "block");

            fatigueDrivingMonitor.getCheckedNodes();
            if (!fatigueDrivingMonitor.validates()) {
                return;
            }
            // fatigueDrivingMonitor.columnRender();
            fatigueDrivingMonitor.initTable();
        },
        /**
         * 列表相关方法
         * */
        ...
        columnRender: function () {
            ...
            for (var i = 0; i < table.length; i++) {
                menu_text += "<li><label><input type=\"checkbox\" checked=\"checked\" class=\"toggle-vis1\" data-column=\"" + parseInt(i + 1) + "\" />" + table[i].innerHTML + "</label></li>"
            }
        ...
        initTable: function () {
            ...
            var ajaxDataParamFun = function (d) {
                d.fuzzyQueryParam = $('#monitorQueryParam').val();
            ...
            //表格setting
            var setting = {
                listUrl: "/clbs/cb/cbReportManagement/fatigueDriving/listVeh",
                ...
                ajaxCallBack: fatigueDrivingMonitor.drawTableCallbackFun,
                pageable: true, //是否分页
            ...
            $('#monitorDataTable tbody').bind('click', 'tr', function (e) {
                if ($(this).find('.dataTables_empty').length === 0) {
                    fatigueDrivingMonitor.trClickFun(e);
                }
            });
        },
        ...
        exportMonitor: function () {
            ...
            var url = "/clbs/cb/cbReportManagement/fatigueDriving/exportVehListData";
        ...
        monitorDetailExport: function () {
            ...
            var paramer = {
                month: curMonthTime,
                monitorIds: curSelectmonitorId,
                module: '车辆疲劳驾驶报表'
            };
            var url = "/clbs/cb/cbReportManagement/fatigueDriving/exportVehDetailData";
            json_ajax("post", url, "json", true, paramer, function (result) {
                if (result.success) {
                    layer.confirm(exportTitle, {
                        title: '操作确认',
        ...
        trClickFun: function (e) {
            ...
            if (_this.hasClass('active')) {
                ...
                var time = $('#timeInterval').val().replace('-', '');
                curMonthTime = time;
                fatigueDrivingMonitor.getVehicleDetailInfo();
                fatigueDrivingMonitor.initDetailTable();
            }
        ...
        getVehicleDetailInfo: function () {
            ...
            var infoUrl = '/clbs/cb/cbReportManagement/fatigueDriving/getVehRankData';
            json_ajax("post", infoUrl, "json", true, ajaxDataParam, function (result) {
                if (result.success) {
                    ...
                    if (data.monitorName) {
                        ...
                        $('#monitorOrg').html(data.organizationName);
                        $('#plateColor').html(getPlateColor(data.plateColor));
                        $('#objectType').html(data.objectType);
                    }
                    var noStr = '-';
                    $('#monitorAccumulatedNum').html(data.accumulatedNum === undefined ? noStr : data.accumulatedNum);
                    $('#monitorDayNum').html(data.dayNum === undefined ? noStr : data.dayNum);
                    $('#monitorNightNum').html(data.nightNum === undefined ? noStr : data.nightNum);
                    $('#monitorRanking').html(data.rank === undefined ? noStr : data.rank);
                } else if (result.msg) {
                    layer.msg(result.msg);
                }
            });
            // 获取详情图表数据
            var url = '/clbs/cb/cbReportManagement/fatigueDriving/getVehGraphicsData';
            json_ajax("post", url, "json", true, ajaxDataParam, function (result) {
                if (result.success) {
                    fatigueDrivingMonitor.chartRender(result, 'monitorAccountedEchart', 'monitorTrendEchart');
                } else {
                    var result = {
                        obj: {
                            statisticInfo: []
        ...
        initDetailTable: function () {
            ...
            //ajax参数
            var ajaxDataParamFun = function (d) {
                d.monitorIds = curSelectmonitorId;
                d.month = curMonthTime;
            };
            //表格setting
            var setting = {
                listUrl: "/clbs/cb/cbReportManagement/fatigueDriving/getVehDetailData",
                columnDefs: columnDefs, //表格列定义
                columns: columns, //表格列
        ...
        chartRender: function (result, accountedElement, trendElement) {
            ...
            fatigueDrivingMonitor.accountedEchart(accountedData, accountedElement);
            ...
            fatigueDrivingMonitor.trendEchart(echartData, trendElement);
        },
        /**
         * 超速车辆占比情况饼图
         * @param data 饼图数据
        ...
        trendEchart: function (echartData, element) {
            ...
            var option = {
                ...
                color: ['#c74540', '#ff9f7f', '#ffdb5c', '#37a2da', '#5b9fb0', '#fe18fe'],
                grid: {
                    top: 60,
                    right: 50,
                },
        ...
        left_arrow: function () {
            ...
            var newDate = dateYear + '-' + fatigueDrivingMonitor.appendZero(dateMonth);
        ...
        initTree: function () {
            ...
            var setting = {
                async: {
                    url: fatigueDrivingMonitor.getTerrUrl,
                    type: "post",
                    enable: true,
                    autoParam: ["id"],
                    dataType: "json",
                    otherParam: {"type": "multiple", "icoType": "0"},
                    dataFilter: fatigueDrivingMonitor.ajaxDataFilter
                },
                check: {
                    enable: true,
                    chkStyle: "checkbox",
                ...
                callback: {
                    beforeClick: fatigueDrivingMonitor.beforeClickVehicle,
                    onAsyncSuccess: fatigueDrivingMonitor.zTreeOnAsyncSuccess,
                    beforeCheck: fatigueDrivingMonitor.zTreeBeforeCheck,
                    onCheck: fatigueDrivingMonitor.onCheckVehicle,
                    onNodeCreated: fatigueDrivingMonitor.zTreeOnNodeCreated,
                    onExpand: fatigueDrivingMonitor.zTreeOnExpand
                }
            };
            ...
            if (param == null || param == undefined || param == '') {
                ...
                var querySetting = {
                    async: {
                        ...
                        dataFilter: fatigueDrivingMonitor.ajaxQueryDataFilter
                    },
                    check: {
                        enable: true,
                        chkStyle: "checkbox",
        ...
        zTreeOnAsyncSuccess: function (event, treeId, treeNode, msg) {
            ...
            fatigueDrivingMonitor.getCharSelect(treeObj);
        },
        ...
        onCheckVehicle: function (e, treeId, treeNode) {
            ...
            if (treeNode.checked) {
                ...
                setTimeout(() => {
                    fatigueDrivingMonitor.getCheckedNodes();
                    fatigueDrivingMonitor.validates();
                }, 600);
            }
            crrentSubV = [];
            crrentSubV.push(treeNode.id);
            fatigueDrivingMonitor.getCheckedNodes();
            fatigueDrivingMonitor.getCharSelect(zTree);
        },
            ...
            if (nodes != null && nodes != undefined && nodes.length > 0) {
                for (var i = 0; i < nodes.length; i++) {
                    ...
                    if (node.type == "assignment") {
                        ...
                        fatigueDrivingMonitor.getGroupChild(node.children, assign);
                    }
                }
            }
        },
        ...
        monthDataInit: function () {
            ...
            laydate.render({
                ...
                , change: function (value, dates, edate) {
                    ...
                    // 获取当前实际的年月
                    var getNow = fatigueDrivingMonitor.getNow;
                    var myDate = new Date();
                    var nowYear = myDate.getFullYear();
                    var nowMonth = myDate.getMonth() + 1;

                    if (year > nowYear) {
                        final = fatigueDrivingMonitor.getYesterMonth(1);
    ...
    $(function () {
        fatigueDrivingMonitor.init();
        fatigueDrivingMonitor.getTable('#monitorDataTable');

        $("#monitorGroupSelect").bind("click", showMenuContent);
        $("#exportMonitor").bind("click", fatigueDrivingMonitor.exportMonitor);//导出
        $("#monitorDetailExport").bind("click", fatigueDrivingMonitor.monitorDetailExport);//导出
        $("#refreshMonitorTable").bind("click", fatigueDrivingMonitor.refreshTable);

        $('input').inputClear().on('onClearEvent', function (e, data) {
            var id = data.id;
            if (id == 'monitorGroupSelect') {
                var param = $("#monitorGroupSelect").val();
                fatigueDrivingMonitor.searchVehicleTree(param);
            }
        });
        // 监控对象树模糊查询
        var inputChange;
