(function (window, $) {
    ...
    var fluxSensor = $("#fluxSensor");
    var bindFuelConsumptionVehicle = {
        init: function () {
            ...
            $("#outputCorrectionK").blur(function () {
                bindFuelConsumptionVehicle.checkOutputCorrectionCoefficientKModify(this); // 判断当前修正系数是否被修改
            });
            $("#outputCorrectionB").blur(function () {
                bindFuelConsumptionVehicle.checkOutputCorrectionCoefficientBModify(this); // 判断当前修正系数是否被修改
            });
        ...
        // 提交
        doSubmit: function () {
            bindFuelConsumptionVehicle.hideErrorMsg();
            var sensorType = $("#fluxSensor").attr("data-id");
            if (sensorType === undefined || sensorType === "") {
                //tg_confirmDialog("系统提示", "传感器型号不能为空！", null, null);
                bindFuelConsumptionVehicle.showErrorMsg(sensorNull, "fluxSensor");
            } else {
                if (bindFuelConsumptionVehicle.validates()) {
    ...
    $(function () {
        $('input').inputClear();
        bindFuelConsumptionVehicle.init();
        $("#doSubmit").bind("click", bindFuelConsumptionVehicle.doSubmit);
    })
})(window, $);