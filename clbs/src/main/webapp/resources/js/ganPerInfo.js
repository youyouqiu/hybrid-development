...
(function ($, window) {
    ...
    ganPerInfo = {
        init: function () {
            ...
            for(var i=0;i<socketTypeArr.length;i++){
                ...
                ganPerInfo.subscribeParam(type);//socket订阅
                ganPerInfo.getDatas(sensorID);//下发指令
            }
        },
        //websocket订阅
        subscribeParam: function (type) {
            webSocket.subscribe(headers, "/topic/per" + type + '_' + vid + "_Info", function (data) {
                ganPerInfo.getSensor0104Param(data, type);
            }, null, null);
        },
        getSensor0104Param: function (data, type) {
            ...
            ganPerInfo.queryF3BaseParamCall(result, type);
        },
        //input赋值
        ...
        getF3BaseParamCall: function (data) {
            if (!data.success) {
                ...
                ganPerInfo.createSocket0104InfoMonitor();
            }
        },
    ...
    $(function () {
        ganPerInfo.init();
        //刷新
        $("#readPerStateRefresh").on("click", ganPerInfo.readInformationRefreshClick);
    })
})($, window);