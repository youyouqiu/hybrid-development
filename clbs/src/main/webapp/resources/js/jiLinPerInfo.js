...
(function ($, window) {
    ...
    jiLinPerInfo = {
        init: function () {
            ...
            for(var i=0;i<socketTypeArr.length;i++){
                ...
                jiLinPerInfo.subscribeParam(type);//socket订阅
                jiLinPerInfo.getDatas(sensorID);//下发指令
            }
        },
        //websocket订阅
        subscribeParam: function (type) {
            webSocket.subscribe(headers, "/topic/per" + type + '_' + vid + "_Info", function (data) {
                jiLinPerInfo.getSensor0104Param(data, type);
            }, null, null);
        },
        getSensor0104Param: function (data, type) {
            ...
            jiLinPerInfo.queryF3BaseParamCall(result, type);
        },
        //input赋值
        ...
        getDatas: function (sensorID) {
            json_ajax("POST", '/clbs/adas/standard/param/getJiPeripheralInfo', "json", false, {
                ...
                "sensorID": sensorID,
                "commandType": 248,
            }, jiLinPerInfo.getF3BaseParamCall);
        },
        getF3BaseParamCall: function (data) {
            if (!data.success) {
                ...
                jiLinPerInfo.createSocket0104InfoMonitor();
            }
        },
        ...
        readInformationRefreshClick: function () {
            ...
            for(var i=0;i<sensorIDArr.length;i++){
                var sensorid = sensorIDArr[i];
                jiLinPerInfo.getDatas(sensorid);
            }
        },
    }
    ...
    $(function () {
        jiLinPerInfo.init();
        //刷新
        $("#readPerStateRefresh").on("click", jiLinPerInfo.readInformationRefreshClick);
    })
})($, window);