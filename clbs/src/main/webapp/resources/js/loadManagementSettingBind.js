...
(function (window, $) {
            ...
            if (vehicleList !== null && vehicleList.length > 0) {
                ...
                for (i = 0; i < vehicleList.length; i++) {
                    ...
                    if (vehicleList[i] === undefined) {
                        ...
                        obj.name = vehicleList[i].plateNumber;tankBasisInfo
                        dataList.value.push(obj);
                    }
                }
            }
        ...
        renderInput: function(sensor, tabIndexParam){
            ...
            if (tabIndex == 1){
                ...
                if(sensor.personLoadParam != null){
                    container.find('[name="loadMeterWay"]').val(sensor.personLoadParam.loadMeterWay);
                    container.find('[name="loadMeterUnit"]').val(sensor.personLoadParam.loadMeterUnit);
                    container.find('[name="noLoadValue"]').val(sensor.personLoadParam.noLoadValue);
                    container.find('[name="noLoadThreshold"]').val(sensor.personLoadParam.noLoadThreshold);
                    container.find('[name="lightLoadValue"]').val(sensor.personLoadParam.lightLoadValue);
                    container.find('[name="lightLoadThreshold"]').val(sensor.personLoadParam.lightLoadThreshold);
                    container.find('[name="fullLoadValue"]').val(sensor.personLoadParam.fullLoadValue);
                    container.find('[name="fullLoadThreshold"]').val(sensor.personLoadParam.fullLoadThreshold);
                    container.find('[name="overLoadValue"]').val(sensor.personLoadParam.overLoadValue);
                    container.find('[name="overLoadThreshold"]').val(sensor.personLoadParam.overLoadThreshold);
                    container.find('[name="listValue"]').val(sensor.adParamJson);
                }

            } else{
                container.find('[name="compensate2"]').val(compensate);
                container.find('[name="oddEvenCheck2"]').val(oddEvenCheck);
                container.find('[name="baudRate2"]').val(baudRate);
                container.find('[name="filterFactor2"]').val(filterFactor);

                // 个性参数
                if(sensor.twoPersonLoadParam != null){
                    container.find('[name="loadMeterWay2"]').val(sensor.twoPersonLoadParam.loadMeterWay);
                    container.find('[name="loadMeterUnit2"]').val(sensor.twoPersonLoadParam.loadMeterUnit);
                    container.find('[name="noLoadValue2"]').val(sensor.twoPersonLoadParam.noLoadValue);
                    container.find('[name="noLoadThreshold2"]').val(sensor.twoPersonLoadParam.noLoadThreshold);
                    container.find('[name="lightLoadValue2"]').val(sensor.twoPersonLoadParam.lightLoadValue);
                    container.find('[name="lightLoadThreshold2"]').val(sensor.twoPersonLoadParam.lightLoadThreshold);
                    container.find('[name="fullLoadValue2"]').val(sensor.twoPersonLoadParam.fullLoadValue);
                    container.find('[name="fullLoadThreshold2"]').val(sensor.twoPersonLoadParam.fullLoadThreshold);
                    container.find('[name="overLoadValue2"]').val(sensor.twoPersonLoadParam.overLoadValue);
                    container.find('[name="overLoadThreshold2"]').val(sensor.twoPersonLoadParam.overLoadThreshold);
                    container.find('[name="listValue2"]').val(sensor.twoAdParamJson);
                }
            }
        ...
        doSubmit: function () {
            ...
            if (sensorId.length>0){
                if ($('#sensorId1').val().length==0){
                    ...
                    if(loadMeterWay != "0" && loadMeterWay != "1" && loadMeterWay != "2"){
                        bindObj.showErrorMsg("请选择载重测量方法！", '#loadMeterWay');
                        return;
                    }
                    ...
                    switch (loadMeterUnit){
                        ...
                        case 3:
                            ...
                            break;
                        default:
                            bindObj.showErrorMsg("请选择传感器重量单位！", '#loadMeterUnit');
                            return;
                    ...
                    if(bindObj.validates()){
                        ...
                        if(loadMeter == 0.1){
                            if(noLoadValue >= 1000){
                                bindObj.showErrorMsg(rangERROStr, '#noLoadValue');
                                return;
                            }
                            if(lightLoadValue >= 1000){
                                bindObj.showErrorMsg(rangERROStr, '#lightLoadValue');
                                return;
                            }
                            if(fullLoadValue >= 1000){
                                bindObj.showErrorMsg(rangERROStr, '#fullLoadValue');
                                return;
                            }
                            if(overLoadValue >= 1000){
                                bindObj.showErrorMsg(rangERROStr, '#overLoadValue');
                                return;
                            }
                        }else if(loadMeter == 1){
                        ...
                        if(noLoadValue < loadMeter){
                            bindObj.showErrorMsg("空载阈值不能小于传感器重量单位，请确认！", '#noLoadValue');
                            return;
                        }
                        if(lightLoadValue - noLoadValue < loadMeter){
                            bindObj.showErrorMsg("轻载阈值必须比空载阈值大1个传感器重量单位，请确认！", '#lightLoadValue');
                            return;
                        }
                        if(fullLoadValue - lightLoadValue < loadMeter){
                            bindObj.showErrorMsg("满载阈值必须比轻载阈值大1个传感器重量单位，请确认", '#fullLoadValue');
                            return;
                        }
                        if(overLoadValue - fullLoadValue < loadMeter){
                            bindObj.showErrorMsg("超载阈值必须比满载阈值大1个传感器重量单位，请确认", '#overLoadValue');
                            return;
                        }
            ...
            if (twoSensorId.length>0){
                ...
                if ($('#sensorId2').val().length==0){
                    ...
                    if(loadMeterWay != "0" && loadMeterWay != "1" && loadMeterWay != "2"){
                        bindObj.showErrorMsg("请选择载重测量方法！", '#loadMeterWay2');
                        return;
                    }
                    ...
                    switch (loadMeterUnit){
                        ...
                        case 3:
                            ...
                            break;
                        default:
                            bindObj.showErrorMsg("请选择传感器重量单位！", '#loadMeterUnit2');
                            return;
                    ...
                    if(bindObj.validates2()){
                        ...
                        if(loadMeter == 0.1){
                            if(noLoadValue >= 1000){
                                bindObj.showErrorMsg(rangERROStr, '#noLoadValue2');
                                return;
                            }
                            if(lightLoadValue >= 1000){
                                bindObj.showErrorMsg(rangERROStr, '#lightLoadValue2');
                                return;
                            }
                            if(fullLoadValue >= 1000){
                                bindObj.showErrorMsg(rangERROStr, '#fullLoadValue2');
                                return;
                            }
                            if(overLoadValue >= 1000){
                                bindObj.showErrorMsg(rangERROStr, '#overLoadValue2');
                                return;
                            }
                        }else if(loadMeter == 1){
                            if(noLoadValue >= 10000){
                        ...
                        if(noLoadValue < loadMeter){
                            bindObj.showErrorMsg("空载阈值不能小于传感器重量单位，请确认！", '#noLoadValue2');
                            return;
                        }
                        if(lightLoadValue - noLoadValue < loadMeter){
                            bindObj.showErrorMsg("轻载阈值必须比空载阈值大1个传感器重量单位，请确认！", '#lightLoadValue2');
                            return;
                        }
                        if(fullLoadValue - lightLoadValue < loadMeter){
                            bindObj.showErrorMsg("满载阈值必须比轻载阈值大1个传感器重量单位，请确认", '#fullLoadValue2');
                            return;
                        }
                        if(overLoadValue - fullLoadValue < loadMeter){
                            bindObj.showErrorMsg("超载阈值必须比满载阈值大1个传感器重量单位，请确认", '#overLoadValue2');
                            return;
                        }
            ...
            $.ajax({
                type: 'POST',
                url: '/clbs/v/loadmgt/loadvehiclesetting/addWorkHourSetting',
                async: false,
                dataType: 'json',
                data: toPost,
                success: function (data) {
    ...
    $(function () {
        ...
        $("#tankBasisInfo2,#tankBasisInfo").bind("click", bindObj.showTankOrSensorInfoFn);
    })
})(window, $);