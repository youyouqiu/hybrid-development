...
class PCMPlayer
{
  constructor(channels, sampleRate)
  {
    ...
    this._frameSize = this._sampleRate/10;
  }
  ...
  _flush()
  {
    if(!this._channels || !this._sampleRate || this._samples.length<this._frameSize)
    {
      return;
    }

    let bufferLength = this._samples.length;
    let length = bufferLength / this._channels;
    ...
    if (bufferLength<this._samples.length) {
      this._samples =  this._samples.slice(bufferLength);
    } else {
      this._samples =  new Float32Array();
    }
  }
