(function($,window){
	...
	parameters = {
        ...
        privateParameterSettingsSend: function () {
            ...
            var url = '/clbs/v/oilmassmgt/oilvehiclesetting/getF3PrivateParam';
        ...
        privateParameterSettings: function (data) {
            if (data != null) {
                if (data.success) {
                    ...
                    setTimeout(function () {
                        // layer.load(2);
                        $("#paramSend").html("<i class='fa fa-spinner loading-state' style='padding: 0 7px'></i>").prop('disabled', true);
                    }, 0);
                } else{
                    layer.msg(data.msg,{move:false});
                }
        ...
        currencyResponse:function (msg) {
            ...
            if(status==1||status==2||status==3){
                // layer.closeAll();
                $("#paramSend").html("参数下发").prop('disabled', false);
