(function (window, $) {
  ...
  overSpeedGroup = {
      ...
      initTable: function () {
          ...
          //表格setting
          var setting = {
              listUrl: "/clbs/cb/cbReportManagement/offRoute/getOrgDataList",
      ...
      inquireClick: function (number) {
          ...
          var time = $('#month').val();
          var ajaxDataParam = {
              "orgId": groupId.replace(/,/g, ''),
              "month": time.replace('-', ''),
              "isSingle": 1,
          };
          var url = '/clbs/cb/cbReportManagement/offRoute/getOrgChartStatisticsData';
       ...
       * */
      chartRender: function (result, accountedElement, trendElement, type) {
          console.log(result, 'result');
          var data = type && type == 'detail' ? result.obj.alarmTrend : result.obj;
          var accountedData = [
              {value: data.totalNotFollowLine, name: '不按规定线路运行'},
              {value: data.totalCourseDeviation, name: '路线偏离'},
          ];
          ...
          for (var i = 0; i < statisticInfo.length; i++) {
              ...
              echartData.monthRatio.push(item.monthRatio ? item.monthRatio : 0);
          }
      ...
      getOrgDetailInfo: function () {
          var ajaxDataParam = {
              "orgId": curSelectOrgId,
              "month": curSelectTime,
          };
          var infoUrl = '/clbs/cb/cbReportManagement/offRoute/getOrgDetailBasicInfoAndTrend';
          json_ajax("post", infoUrl, "json", true, ajaxDataParam, function (result) {
              if (result.success) {
                  var data = result.obj.basicInfo ? result.obj.basicInfo : {};
                  var chartData = result.obj.alarmTrend ? result.obj.alarmTrend : {};
                  // 详情抽屉字段赋值
                  if (data.orgName) {
                      $('#orgName').html(data.orgName);
                  }
                  var noStr = '-';
                  $('#monitorNum').html(data.alarmMonitorNum === undefined ? noStr : data.alarmMonitorNum);
                  $('#totalNum').html(data.courseDeviation === undefined ? noStr : data.courseDeviation);
                  $('#less20PerNum').html(data.notFollowLine === undefined ? noStr : data.notFollowLine);
                  $('#ranking').html(data.rank === undefined ? noStr : data.rank);
                // 详情图表数据赋值
                  overSpeedGroup.chartRender(result, 'accountedDetailEchart', 'trendDetailEchart', 'detail');
              } else if (result.msg) {
                  var result = {
                      obj: {
                          alarmTrend: {
      ...
      initDetailTable: function () {
          ...
          var ajaxDataParamFun = function (d) {
              ...
              d.orgId = curSelectOrgId;
              d.month = curSelectTime;
          };
          //表格setting
          var setting = {
              listUrl: "/clbs/cb/cbReportManagement/offRoute/getOrgDetailMonitorList",
              columnDefs: columnDefs, //表格列定义
              columns: columns, //表格列
      ...
      export: function () {
          ...
          var url = "/clbs/cb/cbReportManagement/offRoute/exportOrgDataList";
      ...
      detailExport: function () {
          ...
          var url = "/clbs/cb/cbReportManagement/offRoute/exportOrgDetailMonitorList";
          json_ajax("post", url, "json", true, paramer, function (result) {
              if (result.success) {
                  layer.confirm(exportTitle, {
                      title: '操作确认',
