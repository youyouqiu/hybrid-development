...
(function ($, window) {
    ...
    var socketTypes = [62308, 62309, 62320, 62311, 62310];//[前向监测, 驾驶员监测, 激烈驾驶监测, 盲区, 胎压]
    //参数下发是的风险事件id
    var arr0 = ['', '', 126401, 126402, 126403, 126404, 126405, 126407, 126409],//前向事件id,''表示公共参数
        arr1 = ['', '', 126513, 126503, 126502, 126508, 126510, 126515],//驾驶员事件id
        arr2 = [1264081, 1264082, 1264083, 127001, 127002, 127003, 127004, ''],//激烈驾驶事件id
        arr3 = [126704, ''],//盲区
        arr4 = [126608, ''];//胎压
    var paramTypes = [64, 65, 70, 67, 66];//[前向监测, 驾驶员监测, 激烈驾驶监测, 盲区, 胎压]
    ...
    paramInfo = {
        init: function () {
            ...
            for (var i = 0; i < socketTypes.length; i++) {
                ...
                paramInfo.subscribeSocket(socketType);//socket订阅
                paramInfo.getPeripheralInfo(sensorID);//传感器参数下发
            }
        },
        /**
         * socket订阅
        ...
        subscribeSocket: function (type) {
            webSocket.subscribe(headers, "/user/topic/per" + type + "Info", function (data) {
                paramInfo.getSensor0104Param(data, type);
            }, null, null);
        },
        /**
         * 传感器参数指令下发
        ...
        getPeripheralInfo: function (sensorID) {
            ...
            json_ajax("POST", url, "json", false, {
                ...
                "sensorID": sensorID,
                "commandType": 243
            }, paramInfo.setreadConventionalCall);
        },
        setreadConventionalCall: function (data) {
            if (data.success) {
                ...
                paramInfo.createSocket0104InfoMonitor(data.msg);
            } else {
                if (data.msg) {
                    layer.msg(data.msg);
                }
        ...
        getSensor0104Param: function (msg, type) {
            ...
            paramInfo.setSenorInputData(param, type);
        ...
        setSenorInputData: function (result, type) {
            var dataTypeMap = {
                ...
                62320: result.intenseDriver,//激烈驾驶
                62311: result.deadZoneDetection,//盲区
                62310: result.tyre1,//胎压
            };
            var tabId = '';
            var data = '';
            ...
            for (var key in data) {
                ...
                switch (key) {
                    case 'driveDeedType':
                    case 'cameraStrategy':
                        value = paramInfo.getCaptureType(value);
                        dom.val(value);
                        break;
                    case 'eventEnable':
                    case 'alarmEnable':
                        paramInfo.getBinaryValue(value, tabId, key);
                        break;
                    case 'cameraResolution':
                        var dataValue = '0x0' + parseInt(value, 10).toString(16);
                        value = paramInfo.changeCameraResolution(value);
                        dom.val(value).attr('data-value', dataValue);
                        break;
                    case 'videoResolution':
                        var dataValue = '0x0' + parseInt(value, 10).toString(16);
                        value = paramInfo.changeVideoResolution(value);
                        dom.val(value).attr('data-value', dataValue);
                        break;
                    case 'fixedCamera':
                        value = value / 1000;
                    ...
                    case 'unit'://胎压单位
                        ...
                        break;
                    case 'tyreNumber':
                        dom.val(value).attr('data-value', paramInfo.tireModel(value));
                        break;
                    default:
                        dom.val(value);
                        break;
        ...
        getPlatformData: function () {
            ...
            for (var i = 0; i < platformData.length; i++) {
                ...
                if (i < 2) {//有公共参数的页签
                    paramInfo.getCommonParamSetting(item.commonParamSetting, tab);
                }
                paramInfo.getAdasAlarmParamSettings(item.adasAlarmParamSettings, tab);
            }
        },
        //全局参数
            ...
            for (var key in data) {
                ...
                switch (key) {
                    ...
                    case 'cameraResolution'://拍照分辨率
                        ...
                        value = paramInfo.changeCameraResolution(num);
                        break;
                    case 'videoResolution'://视频分辨率
                        var num = Number(parseInt(value, 16).toString(10));
                        value = paramInfo.changeVideoResolution(num);
                        break;
                    case 'photographTime':
                        value = value / 10;
                        break;
        ...
        getAdasAlarmParamSettings: function (data, tab) {
            for (var i = 0; i < data.length; i++) {
                ...
                switch (riskId) {
                    ...
                    case 126608://胎压监测
                    case 1264081://急加速
                        paramInfo.setInputValue(item, riskEvent.eq(0));
                        break;
                    case 126401://前向碰撞
                    case 126513://疲劳驾驶
                    case 1264082://急减速
                        paramInfo.setInputValue(item, riskEvent.eq(1));
                        break;
                    case 126402://车道偏离
                    case 126503://抽烟
                    case 1264083://急转弯
                        paramInfo.setInputValue(item, riskEvent.eq(2));
                        break;
                    case 126403://车距过近
                    case 126502://接打手持电话
                    case 127001://怠速
                        paramInfo.setInputValue(item, riskEvent.eq(3));
                        break;
                    case 126404://行人碰撞
                    case 126508://长时间不目视前方
                    case 127002://异常熄火
                        paramInfo.setInputValue(item, riskEvent.eq(4));
                        break;
                    case 126405://频繁变道
                    case 126510://未检测到驾驶员
                    case 127003://空挡滑行
                        paramInfo.setInputValue(item, riskEvent.eq(5));
                        break;
                    case 126407://障碍物
                    case 126515://驾驶员身份识别
                    case 127004://空挡滑行
                        paramInfo.setInputValue(item, riskEvent.eq(6));
                        break;
                    case 126409://道路标识超限
                        paramInfo.setInputValue(item, riskEvent.eq(7));
                        break;
                    default:
                        break;
                }
        ...
        paramSendAllClick: function () {
            var self = $(this);
            paramInfo.preventclick(self);
            isAll = true;
            var jsonDataArr = [];
            var riskIds = [arr0, arr1, arr2, arr3, arr4];//前向,驾驶员.激烈驾驶,盲区,胎压风险事件id

            for (var i = 0; i < riskIds.length; i++) {
                paramType = paramTypes[i];
                var jsonData = paramInfo.setJsonData(riskIds[i], $('.tab-pane').eq(i));
                jsonDataArr.push(jsonData[0]);
            }
            // console.log('全部下发参数', jsonDataArr);
            paramInfo.dsmInfoSend(jsonDataArr);
        },
        paramSendClick: function () {
            ...
            var jsonData = paramInfo.setJsonData(arr, tab);
            // console.log('本页签下发参数', jsonData);
            paramInfo.dsmInfoSend(jsonData);
        },
        //组装下发参数
        setJsonData: function (arr, tab) {
            ...
            for (var i = 0; i < arr.length; i++) {
                ...
                if (item == '') {//全局参数/主动抓拍策略
                    ...
                    commonParamSetting = paramInfo.getCommontData(inputs);
                } else {//风险事件
                    inputs = tab.find('.info-content').eq(i).find('input');
                    adasAlarmParamSettings.push(paramInfo.getInputData(item, inputs));
                }
            }
        ...
        dsmInfoSend: function (jsonData) {
            ...
            json_ajax("POST", "/clbs/adas/standard/param/setting.gsp", "json", false, data, paramInfo.dsmInfoSendCallback);
        },
        dsmInfoSendCallback: function (data) {
            if (data != null) {
                if (data.success) {
                    ...
                    //获取下发状态
                    setTimeout(function () {
                        timerInterval = setInterval(paramInfo.getStatus(), 5000);
                    }, 2000);
                    setTimeout(function () {
            ...
            json_ajax("POST", url, "json", false, {
                ...
                if (data.success) {
                    ...
                    if (obj) {
                        ...
                        for (var key in obj) {
                            var value = obj[key];
                            status.push(paramInfo.statusChange(value));
                        }
                        ...
                        timer = setTimeout(function () {
                            ...
                            paramInfo.getPeripheralInfo(sensorID);
                        }, 1000);
                    }
    ...
    $(function () {
        ...
        //事件
        $(".info-span").on("click", paramInfo.hiddenparameterFn);//点击显示隐藏信息
        $("#dsmInfoSend").on("click", paramInfo.paramSendAllClick);//全部修正下发
        $("#dsmInfoSend2").on("click", paramInfo.paramSendClick);//本页签修正下发
        $("#dsmInfoRefresh").on("click", paramInfo.readConventionalRefreshClick);//刷新
        $(".sensorBtn").on("click", paramInfo.sensorBtnClick);//以传感器为准
        $(".platformBtn").on("click", paramInfo.platformBtnClick);//以平台设置为准
    })
}($, window))