(function (window, $) {
    ...
    parkingSearch = {
        init: function () {
            ...
            var setting = {
                async: {
                    url: parkingSearch.getParkingReportTreeUrl,
                    type: "post",
                    enable: true,
                    autoParam: ["id"],
                    dataType: "json",
                    otherParam: {"type": "multiple", "icoType": "0"},
                    dataFilter: parkingSearch.ajaxDataFilter
                },
                check: {
                    enable: true,
                    chkStyle: "checkbox",
                ...
                callback: {
                    beforeClick: parkingSearch.beforeClickVehicle,
                    onAsyncSuccess: parkingSearch.zTreeOnAsyncSuccess,
                    beforeCheck: parkingSearch.zTreeBeforeCheck,
                    onCheck: parkingSearch.onCheckVehicle,
                    onNodeCreated: parkingSearch.zTreeOnNodeCreated,
                    onExpand: parkingSearch.zTreeOnExpand
                }
            };
            ...
            if (param == null || param == undefined || param == '') {
                ...
                var querySetting = {
                    async: {
                        ...
                        dataFilter: parkingSearch.ajaxQueryDataFilter
                    },
                    check: {
                        enable: true,
                        chkStyle: "checkbox",
        ...
        startDay: function (day) {
            ...
            if (startValue == "" || endValue == "") {
                ...
                tMonth = parkingSearch.doHandleMonth(tMonth + 1);
                tDate = parkingSearch.doHandleMonth(tDate);
                var num = -(day + 1);
                ...
                endMonth = parkingSearch.doHandleMonth(endMonth + 1);
                endDate = parkingSearch.doHandleMonth(endDate);
                ...
                vMonth = parkingSearch.doHandleMonth(vMonth + 1);
                vDate = parkingSearch.doHandleMonth(vDate);
                ...
                if (day == 1) {
                    ...
                    vendMonth = parkingSearch.doHandleMonth(vendMonth + 1);
                    vendDate = parkingSearch.doHandleMonth(vendDate);
        ...
        inquireClick: function (number) {
            ...
            if (number == 0) {
                parkingSearch.getToday(number);
            } else if (number == -1) {
                parkingSearch.startDay(-1)
            } else if (number == -3) {
                parkingSearch.startDay(-3)
            } else if (number == -7) {
                parkingSearch.startDay(-7)
            }
            parkingSearch.getCheckedNodes();
            if (!parkingSearch.validates()) {
                return;
            }
            ;
            if (number != 1) {
            ...
            ;
            var url = "/clbs/m/reportManagement/parkingReport/getStopBigDataNew";
            var parameter = {"vehicleId": vehicleId, "startTime": startTime, "endTime": endTime};
            json_ajax("POST", url, "json", true, parameter, parkingSearch.getCallback);
        },
        exportAlarm: function () {
            ...
            var url = "/clbs/m/reportManagement/parkingReport/export";
            ...
            json_ajax("POST", url, "json", true, parameter, parkingSearch.exportCallback);*/
        },
        validates: function () {
        ...
        zTreeOnAsyncSuccess: function (event, treeId, treeNode, msg) {
            ...
            parkingSearch.getCharSelect(treeObj);
        },
...
//	       	return flag;
        ...
        onCheckVehicle: function (e, treeId, treeNode) {
            ...
            if (treeNode.checked) {
                ...
                setTimeout(() => {
                    parkingSearch.getCheckedNodes();
                    parkingSearch.validates();
                }, 600);
            }
            crrentSubV = [];
            crrentSubV.push(treeNode.id);
            parkingSearch.getCheckedNodes();
            parkingSearch.getCharSelect(zTree);
        },
            ...
            if (nodes != null && nodes != undefined && nodes.length > 0) {
                for (var i = 0; i < nodes.length; i++) {
                    ...
                    if (node.type == "assignment") {
                        ...
                        parkingSearch.getGroupChild(node.children, assign);
                    }
                }
            }
        },
        ...
        getCallback: function (date) {
            ...
            if (date.success == true) {
                ...
                if (date.obj != null && date.obj.length != 0) {
                    ...
                    parkingSearch.reloadData(dataListArray);
            ...
            myTable = $(table).DataTable({
                ...
                "drawCallback": function (settings) {
                    ...
                    if (alarmLocation == "加载中..." && !getAddressStatus) {
                        parkingSearch.getAddress(newStopLoc);
                    }
                },
                "order": [
                    [0, null]
        ...
        refreshTable: function () {
            parkingSearch.inquireClick(1, false);
        },
        getToday: function () {
            var nowDate = new Date();
    ...
    $(function () {
        parkingSearch.init();
        parkingSearch.getTable('#dataTable');
        $('input').inputClear();
        // $('#timeInterval').dateRangePicker({dateLimit:30});
        $('#timeInterval').dateRangePicker({
            dateLimit: 31,
            isOffLineReportFlag: true,
            nowDate: parkingSearch.getToday(),
            isShowHMS: false
        });
        //当前时间
        parkingSearch.getsTheCurrentTime();
        $("#groupSelect").bind("click", showMenuContent);
        //导出
        $("#exportAlarm").bind("click", parkingSearch.exportAlarm);
        $("#refreshTable").bind("click", parkingSearch.refreshTable);

        $('input').inputClear().on('onClearEvent', function (e, data) {
            var id = data.id;
            if (id == 'groupSelect') {
                var param = $("#groupSelect").val();
                parkingSearch.searchVehicleTree(param);
            }
            ;
        });
