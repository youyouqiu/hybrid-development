(function (window, $) {
    ...
    passengerFlowReport = {
        init: function () {
            ...
            var setting = {
                async: {
                    url: passengerFlowReport.getFenceReportTreeUrl,
                    type: "post",
                    enable: true,
                    autoParam: ["id"],
                    dataType: "json",
                    otherParam: {"type": "multiple", "icoType": "0"},
                    dataFilter: passengerFlowReport.ajaxDataFilter
                ...
                callback: {
                    beforeClick: passengerFlowReport.beforeClickVehicle,
                    onAsyncSuccess: passengerFlowReport.zTreeOnAsyncSuccess,
                    beforeCheck: passengerFlowReport.zTreeBeforeCheck,
                    onCheck: passengerFlowReport.onCheckVehicle,
                    onNodeCreated: passengerFlowReport.zTreeOnNodeCreated,
                    onExpand: passengerFlowReport.zTreeOnExpand
                }
            };
            ...
            if (param == null || param == undefined || param == '') {
                ...
                var querySetting = {
                    async: {
                        ...
                        dataFilter: passengerFlowReport.ajaxQueryDataFilter
                    },
                    check: {
                        enable: true,
                        chkStyle: "checkbox",
        ...
        startDay: function (day) {
            ...
            if (startValue == "" || endValue == "") {
                ...
                tMonth = passengerFlowReport.doHandleMonth(tMonth + 1);
                tDate = passengerFlowReport.doHandleMonth(tDate);
                var num = -(day + 1);
                ...
                endMonth = passengerFlowReport.doHandleMonth(endMonth + 1);
                endDate = passengerFlowReport.doHandleMonth(endDate);
                ...
                vMonth = passengerFlowReport.doHandleMonth(vMonth + 1);
                vDate = passengerFlowReport.doHandleMonth(vDate);
                ...
                if (day == 1) {
                    ...
                    vendMonth = passengerFlowReport.doHandleMonth(vendMonth + 1);
                    vendDate = passengerFlowReport.doHandleMonth(vendDate);
        ...
        inquireClick: function (number) {
            if (number == 0) {
                passengerFlowReport.getsTheCurrentTime();
            } else if (number == -1) {
                passengerFlowReport.startDay(-1)
            } else if (number == -3) {
                passengerFlowReport.startDay(-3)
            } else if (number == -7) {
                passengerFlowReport.startDay(-7)
            }
            if (number != 1) {
            ...
            passengerFlowReport.getCheckedNodes();
            if (!passengerFlowReport.validates()) {
                return;
            }
            var url = "/clbs/m/reportManagement/passengerFlowReport/getPassengerFlowReports";
            var parameter = {"vehicleIdStr": vehicleId, "startTime": startTime, "endTime": endTime};
            json_ajax("POST", url, "json", true, parameter, passengerFlowReport.getCallback);
        },
        exportAlarm: function () {
            if(getRecordsNum() > 60000){
                return layer.msg("导出数据量过大，请缩小查询范围再试（单次导出最多支持6万条）！");
            }
            var url = "/clbs/m/reportManagement/passengerFlowReport/export";
        ...
        zTreeOnAsyncSuccess: function (event, treeId, treeNode, msg) {
            ...
            passengerFlowReport.getCharSelect(treeObj);
        },
        ...
        refreshTableClick: function () {
            ...
            //字段搜索重置
            //刷新表格
            passengerFlowReport.inquireClick(1, false);
        },
        onCheckVehicle: function (e, treeId, treeNode) {
            ...
            if (treeNode.checked) {
                ...
                setTimeout(() => {
                    passengerFlowReport.getCheckedNodes();
                    passengerFlowReport.validates();
                }, 600);
            }
            passengerFlowReport.getCharSelect(zTree);
            passengerFlowReport.getCheckedNodes();
            crrentSubV = [];
            ...
            if (nodes != null && nodes != undefined && nodes.length > 0) {
                for (var i = 0; i < nodes.length; i++) {
                    ...
                    if (node.type == "assignment") {
                        ...
                        passengerFlowReport.getGroupChild(node.children, assign);
                    }
                }
            }
        },
                ...
                if (data.obj != null && data.obj.length != 0) {
                    ...
                    for (var i = 0; i < flowReportList.length; i++) {
                        ...
                        var dateList =
                            [
                                s++,
                                flowReportList[i].brand,
                                flowReportList[i].groupName,
                                flowReportList[i].assignmentName,
                                flowReportList[i].plateColorStr,
                                flowReportList[i].vehicleType,
                                flowReportList[i].startTime,
                                flowReportList[i].endTime,
                                flowReportList[i].aboard,
                                flowReportList[i].getOff

                            ];
                        dataListArray.push(dateList);
                    }

                    passengerFlowReport.reloadData(dataListArray);
    ...
    $(function () {
        ...
        //当前时间
        $('input').inputClear();
        $("#refreshTable").bind("click", passengerFlowReport.refreshTableClick);
        passengerFlowReport.getsTheCurrentTime();
        passengerFlowReport.getTable('#dataTable');
        $('#timeInterval').dateRangePicker({
            dateLimit: 31
        });
        $("#groupSelect").bind("click", showMenuContent);
        //导出
        $("#exportAlarm").bind("click", passengerFlowReport.exportAlarm);

        $('input').inputClear().on('onClearEvent', function (e, data) {
            var id = data.id;
            if (id == 'groupSelect') {
                var param = $("#groupSelect").val();
                passengerFlowReport.searchVehicleTree(param);
            }
            ;
        });
