(function (window, $) {
    ...
    personnelInformation = {
        ...
        init: function () {
            ...
            var columns = [{
            ...
            },
                {
                    ...
                    render: function (data, type, row, meta) {
                        if (data == '2') {
                            return '<div class="mySwitch" onclick="personnelInformation.changeState(\'' + row.id + '\')">在职 <span></span></div>';
                        } else if (data == '0') {
                            return '离职';
                        } else {
                            return '';
            ...
            //表格setting
            var setting = {
                listUrl: '/clbs/talkback/basicinfo/monitoring/personnel/list',
                editUrl: '/clbs/talkback/basicinfo/monitoring/personnel/edit_',
                deleteUrl: '/clbs/talkback/basicinfo/monitoring/personnel/delete_',
                deletemoreUrl: '/clbs/talkback/basicinfo/monitoring/personnel/deletemore',
        ...
        changeState: function (id) {
            layer.confirm('状态修改成离职,对应终端将解绑,同时平台不再处理离职人员实时数据', {btn: ['确定', '取消'], icon: 7, title: '操作确认'}, function () {
                var url = '/clbs/talkback/basicinfo/monitoring/personnel/upDateWorkStae';
        ...
        deleteItemAndCheckBond: function (id) {
            var url = '/clbs/talkback/basicinfo/monitoring/personnel/delete_' + id + '.gsp';
            var data = {'id': id};
        ...
        deleteConfigByVidCallBack: function (data) {
            if (data.success) {
                ...
                var url = '/clbs/talkback/basicinfo/monitoring/personnel/delete_' + vid + '.gsp';
                var data = {'id': vid};
                json_ajax('POST', url, 'json', true, data, personnelInformation.deleteCallBack);
            } else {
                layer.msg(publicError);
        ...
        delModelClick: function () {
            ...
            var url = '/clbs/talkback/basicinfo/monitoring/personnel/deletemore';
            var data = {'checkedList': checkedList.toString()};
            layer.confirm(publicDelete, {btn: ['确定', '取消'], icon: 3, title: '操作确认'}, function () {
                json_ajax('POST', url, 'json', true, data, personnelInformation.deleteMuchCallBack);
            });
        },
        // 批量删除回调
        deleteMuchCallBack: function (data) {
            if (data.success) {
                ...
                if (data != null && data.obj != null && data.obj.infoMsg != null && data.obj.infoMsg.length > 0) {
                    var boundBrandIds = data.obj.boundBrandIds.split(',');
                    layer.confirm('已绑定人员：' + data.obj.boundBrands + '</br>' + data.obj.infoMsg, {btn: ['确认', '取消']}, function () {
                        layer.closeAll();
                        if (boundBrandIds != null && boundBrandIds.length > 0) {
                            for (var i = 0; i < boundBrandIds.length; i++) {
                                var data = {'vehicleId': boundBrandIds[i]};
                                json_ajax('POST', url, 'json', true, data, personnelInformation.getConfigIdByPeopleIdCallBack);
                            }
                        }
                    }, function () {
        ...
        addId: function () {
            $('#addId').attr('href', '/clbs/talkback/basicinfo/monitoring/personnel/add?uuid=' + selectTreepId + '');
        }
    };
    $(function () {
