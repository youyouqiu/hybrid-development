...
(function ($, window) {
    ...
    //socket订阅、下发指令
    var socketTypeArr = ['F764', 'F765', 'F767']; //[前向监测, 驾驶员行为监测, 盲区监测]
    ...
    shanPerState = {
        init: function () {
            ...
            for (var i=0; i<socketTypeArr.length; i++){
                ...
                //socket订阅
                shanPerState.subscribeParam(type);
                // //下发指令
                shanPerState.getDatas(sensorId);
            }
        },
        //websocket订阅
        subscribeParam: function (type) {
            webSocket.subscribe(headers, "/topic/per" + type + '_' + vid + "_Info", function (data) {
                console.log(type);
                shanPerState.getSensor0104Param(data, type);
            }, null, null);
        },
        ...
        queryF3BaseParamCall: function (result, type) {
            ...
            for (var i = 0; i < alarmData.length; i++) {
                ...
                var value = shanPerState.getAlarmStatus(item.value);
        ...
        getDatas: function (sensorID) {
            json_ajax("POST", '/clbs/adas/standard/param/getJiPeripheralInfo', "json", false, {
                ...
                "sensorID": sensorID,
                "commandType": 247,
            }, shanPerState.getF3BaseParamCall);
        },
        getF3BaseParamCall: function (data) {
            if (!data.success) {
                ...
                shanPerState.createSocket0104InfoMonitor();
            }
        },
    ...
    $(function () {
        shanPerState.init();
        //刷新
        $("#readPerStateRefresh").on("click", shanPerState.readInformationRefreshClick);
    })
})($, window);