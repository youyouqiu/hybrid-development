(function (window, $) {
    ...
    speedAlarmSearch = {
        init: function () {
            ...
            var setting = {
                async: {
                    url: speedAlarmSearch.getSpeedAlarmReportTreeUrl,//"/clbs/m/functionconfig/fence/bindfence/vehicelTree"
                    type: "post",
                    enable: true,
                    autoParam: ["id"],
                    dataType: "json",
                    otherParam: {"type": "multiple", "icoType": "0"},
                    dataFilter: speedAlarmSearch.ajaxDataFilter
                },
                check: {
                    enable: true,
                    chkStyle: "checkbox",
                ...
                callback: {
                    beforeClick: speedAlarmSearch.beforeClickVehicle,
                    onAsyncSuccess: speedAlarmSearch.zTreeOnAsyncSuccess,
                    beforeCheck: speedAlarmSearch.zTreeBeforeCheck,
                    onCheck: speedAlarmSearch.onCheckVehicle,
                    onNodeCreated: speedAlarmSearch.zTreeOnNodeCreated,
                    onExpand: speedAlarmSearch.zTreeOnExpand
                }
            };
            ...
            if (param == null || param == undefined || param == '') {
                ...
                var querySetting = {
                    async: {
                        ...
                        dataFilter: speedAlarmSearch.ajaxQueryDataFilter
                    },
                    check: {
                        enable: true,
                        chkStyle: "checkbox",
        ...
        getSpeedAlarmReportTreeUrl: function (treeId, treeNode) {
            if (treeNode == null) {
        ...
        startDay: function (day) {
            ...
            if (startValue == "" || endValue == "") {
                ...
                tMonth = speedAlarmSearch.doHandleMonth(tMonth + 1);
                tDate = speedAlarmSearch.doHandleMonth(tDate);
                var num = -(day + 1);
                ...
                endMonth = speedAlarmSearch.doHandleMonth(endMonth + 1);
                endDate = speedAlarmSearch.doHandleMonth(endDate);
                ...
                vMonth = speedAlarmSearch.doHandleMonth(vMonth + 1);
                vDate = speedAlarmSearch.doHandleMonth(vDate);
                ...
                if (day == 1) {
                    ...
                    vendMonth = speedAlarmSearch.doHandleMonth(vendMonth + 1);
                    vendDate = speedAlarmSearch.doHandleMonth(vendDate);
        ...
        inquireClick: function (number) {
            if (number == 0) {
                speedAlarmSearch.getsTheCurrentTime();
            } else if (number == 'yes') {// 昨天
                speedAlarmSearch.getsTheCurrentTime(true);
            } else if (number == -1) {
                speedAlarmSearch.startDay(-1)
            } else if (number == -3) {
                speedAlarmSearch.startDay(-3)
            } else if (number == -7) {
                speedAlarmSearch.startDay(-7)
            }
            if (number != 1) {
            ...
            ;
            speedAlarmSearch.getCheckedNodes();
            if (!speedAlarmSearch.validates()) {
                return;
            }
            ;
            var url = "/clbs/m/reportManagement/speedAlarmReport/getSpeedAlarmListByF3Pass";
            var parameter = {
            ...
            json_ajax("POST", url, "json", true, parameter, speedAlarmSearch.getCallback);
        },
        exportAlarm: function () {
            ...
            startTime = timeInterval[0];
            endTime = timeInterval[1];
            var url = "/clbs/m/reportManagement/speedAlarmReport/export";
            var parameter = {
                "vehicleId": vehicleId,
                "startTime": startTime,
                "endTime": endTime,
            ...
            json_ajax("POST", url, "json", true, parameter, speedAlarmSearch.exportCallback);
        },
        validates: function () {
        ...
        zTreeOnAsyncSuccess: function (event, treeId, treeNode, msg) {
            ...
            speedAlarmSearch.getCharSelect(treeObj);
        },
        ...
        onCheckVehicle: function (e, treeId, treeNode) {
            ...
            if (treeNode.checked) {
                ...
                setTimeout(() => {
                    speedAlarmSearch.getCheckedNodes();
                    speedAlarmSearch.validates();
                }, 600);
            }
            crrentSubV = [];
            crrentSubV.push(treeNode.id);
            speedAlarmSearch.getCharSelect(zTree);
            speedAlarmSearch.getCheckedNodes();
        },
            ...
            if (nodes != null && nodes != undefined && nodes.length > 0) {
                for (var i = 0; i < nodes.length; i++) {
                    ...
                    if (node.type == "assignment") {
                        ...
                        speedAlarmSearch.getGroupChild(node.children, assign);
                    }
                }
            }
        },
                    ...
                    for (var i = 0; i < alarm.length; i++) {
                        ...
                        var dateList =
                            [
                                ...
                                alarm[i].alarmSource,
                                alarm[i].alarmStartTime == null ? '-' : alarm[i].alarmStartTime,
                                alarm[i].alarmStartLocation == '[]' || alarm[i].alarmStartLocation == null ? '未定位' : alarm[i].alarmStartLocation,
                                parseFloat(alarm[i].startSpeed),
                                alarm[i].alarmEndTime == null ? '-' : alarm[i].alarmEndTime,
                                alarm[i].alarmEndLocation == null || alarm[i].alarmEndLocation == '[]' ? '未定位' : alarm[i].alarmEndLocation,
                                parseFloat(alarm[i].endSpeed),
                                alarm[i].duration
                            ];
...
//                      endLoc.push(alarm[i].alarmEndLocation);
                    ...
                    speedAlarmSearch.reloadData(dataListArray);
        ...
        getAddress: function () {
            ...
            for (var i = 0; i < dataLength; i++) {
                ...
                if (num == dataLength) {
                    var addressIndex = 0;
                    var addressArray = [];
                    backAddressMsg(addressIndex, addressMsg, speedAlarmSearch.goBack, addressArray);
                    addressMsg = [];
                }
            }
            ;
            ...
            myTable = $(table).DataTable({
                ...
                /*"drawCallback": function(settings) {
                      var $dataTableTbody = $("#dataTable tbody")
                        ...
                        //报警位置进行逆地址解析
                        if(alarmLocation == "加载中..."){
                            speedAlarmSearch.getAddress();
                        }
                },*/
                "order": [
                    [0, null]
    ...
    $(function () {
        ...
        //当前时间
        speedAlarmSearch.getsTheCurrentTime();
        speedAlarmSearch.getTable('#dataTable');
        $('#timeInterval').dateRangePicker({
            dateLimit: 31,
            isOffLineReportFlag: true,
            nowDate: speedAlarmSearch.getYesterDay(),
            isShowHMS: false
        });
        $("#groupSelect").bind("click", showMenuContent);
        //导出
        $("#exportAlarm").bind("click", speedAlarmSearch.exportAlarm);

        $('input').inputClear().on('onClearEvent', function (e, data) {
            var id = data.id;
            if (id == 'groupSelect') {
                var param = $("#groupSelect").val();
                speedAlarmSearch.searchVehicleTree(param);
            }
            ;
        });
