...
(function ($, window) {
    ...
    suPerInfo = {
        init: function () {
            ...
            for (var i=0; i<socketTypeArr.length; i++) {
                ...
                suPerInfo.subscribeParam(type); //socket订阅
                suPerInfo.getDatas(sensorId);   //下发指令
            }
        },
        //websocket订阅
        subscribeParam: function (type) {
            webSocket.subscribe(headers, "/topic/per" + type + '_' + vid + "_Info", function (data) {
                suPerInfo.getSensor0104Param(data, type);
            }, null, null);
        },
        getSensor0104Param: function (data, type) {
            ...
            suPerInfo.queryF3BaseParamCall(result, type);
        },
        //input赋值
        ...
        getF3BaseParamCall: function (data) {
            if (!data.success) {
                ...
                suPerInfo.createSocket0104InfoMonitor();
            }
        },
    ...
    $(function () {
        suPerInfo.init();
        //刷新
        $("#readPerStateRefresh").on("click", suPerInfo.readInformationRefreshClick);
    })
})($, window);