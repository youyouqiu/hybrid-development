(function(window,$){
	...
	temperatureSensorSet = {
		...
		init: function(){
            ...
            // 接收到消息
            // setTimeout(function () {
            //     webSocket.subscribe(headers,'/topic/fencestatus', temperatureSensorSet.updataFenceData,"", null);
            // },500);
		    ...
		    },
	        {
	            ...
	            render: function (data, type, row, meta) {
	            	var editUrlPath = myTable.editUrl + row.vehicleId+"?sensorOutId="+data.sensorOutId; //修改地址
					var bindUrlPre = '/clbs/v/temperatureDetection/temperatureDetectionSetting/bind_{id}';
					var detailUrlPre = '/clbs/v/temperatureDetection/temperatureDetectionSetting/detail_{id}';
					var result = '';
					if (row.sensorNumber == null || row.sensorNumber == "") {
						// 设置
                        ...
                        // 下发参数
                        result += '<button onclick="temperatureSensorSet.sendfuelOneVechice(\''+row.id+'\')" type="button" class="editBtn editBtn-info"><i class="fa fa-issue"></i>下发参数</button>&ensp;';
                        // 详细信息
                    ...
                    "class" : "text-center",
                    render : function(data, type, row, meta) {
                        return temperatureSensorSet.renderStatus(row, false);
                    }
            }, {
	            "data": "brand",
	            "class": "text-center"
		    ...
		    var setting = {
		        ...
		        editUrl: '/clbs/v/temperatureDetection/temperatureDetectionSetting/edit_',
	        ...
	        var treeSetting = {
        		async : {
        			...
        			dataFilter: temperatureSensorSet.ajaxDataFilter
        		},
                view : {
        			selectedMulti : false,
        			nameIsHTML: true,
        		...
        		callback : {
        			onClick : temperatureSensorSet.zTreeOnClick
        		}
        	};
		...
		updataFenceData: function(msg){
	        if (msg != null) {
	            ...
	            if (result != null) {
					temperatureSensorSet.updateColumn(result.desc.monitorId);
	            }
	        }
	    },
	    //批量删除
        ...
        sendModelClick: function(){
	        ...
	        // 下发方法调用
	       temperatureSensorSet.sendFuel(JSON.stringify(checkedList));
        },
        sendFuel: function(sendParam){//批量下发执行
            var url="/clbs/v/sensorSettings/sendSetDeviceParam";
            var parameter={"sendParam": sendParam,"sensorType":1, "flag":2};
            json_ajax("POST",url,"json",true,parameter,temperatureSensorSet.sendFuelCallback);
	    },
	    // 下发流量回调方法
	    sendFuelCallback: function(data){
        	if(temsendflag){
				webSocket.subscribe(headers, '/user/topic/temperature_monitoring', temperatureSensorSet.updataFenceData, null, null);
                temsendflag=false;
            }
	    ...
	    sendfuelOneVechice: function(id){// 下发参数 （单个）
			...
			temperatureSensorSet.sendFuel(jsonStr);
		},
		//下发状态
		...
		updateColumn: function (id){
			for(var i= 0; i< tableList.length; i++) {
				if(id == tableList[i].vehicleId){
					var url = '/clbs/v/sensorSettings/refreshSendStatus?vehicleId=' + id + '&sensorType=1';
					json_ajax('GET', url, 'json', false, null, function (messages) {
						if(messages.success){
							var data = messages.obj;
							$("#status_" + id).text(temperatureSensorSet.renderStatus(data, true));
						}
					});
				}
			}
	...
	$(function(){
		...
		//全选
	    $("#checkAll").bind("click",temperatureSensorSet.checkAllClick);
	    subChk.bind("click",temperatureSensorSet.subChkClick);
	    //批量删除
	    $("#del_model").bind("click",temperatureSensorSet.delModelClick);
	    //刷新
	    $("#refreshTable").on("click",temperatureSensorSet.refreshTable);
		//批量下发
	    $("#send_model").bind("click",temperatureSensorSet.sendModelClick);
