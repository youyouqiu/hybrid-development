...
(function ($, window) {
    ...
    vehicleAdd = {
        ...
        init: function () {
            ...
            var setpermission = {
                ...
                callback: {
                    beforeClick: vehicleAdd.beforeClickPermission,
                    onCheck: vehicleAdd.onCheckPermission
                }
            }
            var setting = {
                async: {
                    ...
                    dataType: "json",
                    dataFilter: vehicleAdd.ajaxDataFilter
                },
                view: {
                    dblClickExpand: false,
                    nameIsHTML: true,
                ...
                callback: {
                    beforeClick: vehicleAdd.beforeClick,
                    onClick: vehicleAdd.onClick,
                    onAsyncSuccess: vehicleAdd.zTreeOnAsyncSuccess
                }
            };
            vehicleAdd.getOperation();
            json_ajax("POST", '/clbs/m/dictionary/businessScope', "json", false, null, vehicleAdd.renderScope);
            //车辆用途
            var urlPurposeCategory = "/clbs/m/basicinfo/monitoring/vehicle/findAllPurposeCategory";
            json_ajax("POST", urlPurposeCategory, "json", false, null, vehicleAdd.getVehiclePurposeCallback);
            //燃料类型
            var urlFuelType = "/clbs/m/basicinfo/monitoring/vehicle/findAllFuelType";
            json_ajax("POST", urlFuelType, "json", false, null, vehicleAdd.getFuelTypeCallback);
        ...
        doSubmit: function () {
            if (submissionFlag) { // 防止重复提交
                ...
                if (vehicleAdd.validates()) {
                    //判断自重是否必填
                    if (selfRespectFlag) {
                        vehicleAdd.selfRespectShow();
                        if ($("#selfRespect").val() == '') {
                            vehicleAdd.goErrorPos();
                            return;
                        }
                    }
                    //判断工作能力是否必填
                    if (abilityWorkFlag) {
                        vehicleAdd.abilityWorkShow();
                        if ($("#abilityWork").val() == '') {
                            vehicleAdd.goErrorPos();
                            return;
                        }
        ...
        //跳转至报错位置
        goErrorPos: function () {
            var errorArr = $("#addForm label.error");
        ...
        checkRightBrand: function (id) {
            ...
            if (checkBrands(id)) {
                vehicleAdd.hideErrorMsg();
                return true;
            } else {
                vehicleAdd.hideErrorMsg();
                vehicleAdd.showErrorMsg(errorMsg3, id);
                return false;
            }
        },
        //不能全是横杠
        checkISdhg: function (elementId) {
            ...
            if (regIfAllheng.test(value)) {
                vehicleAdd.hideErrorMsg(); //wjk,先隐藏
                vehicleAdd.showErrorMsg('不能全是横杠', elementId);
                return false;
            } else {
                vehicleAdd.hideErrorMsg();
                return true;
        ...
        showMenu: function (e) {
            ...
            $("body").bind("mousedown", vehicleAdd.onBodyDown);
        },
        hideMenu: function () {
            $("#zTreeContent").fadeOut("fast");
            $("body").unbind("mousedown", vehicleAdd.onBodyDown);
        },
        ...
        ajaxDataFilter: function (treeId, parentNode, responseData) {
            vehicleAdd.hideErrorMsg(); //清除错误提示样式
            ...
            if (responseData != null && responseData != "" && responseData != undefined && responseData.length >= 1) {
                ...
                vehicleAdd.showErrorMsg("您需要先新增一个组织", "zTreeCitySel");
                return;
            }
        },
        isRadio: function () {
        ...
        setImagePreview: function (avalue) {
            vehicleAdd.uploadImage(); // 上传图片到服务器
        ...
        getVehicleCategory: function () {
            ...
            json_ajax("GET", url, "json", false, null, vehicleAdd.categoryCallBack);
        },
        ...
        getVehicleType: function (id) {
            ...
            json_ajax("GET", url, "json", false, null, vehicleAdd.getTypeCallback);
        },
        ...
        getBrand: function () {
            ...
            json_ajax("GET", url, "json", false, null, vehicleAdd.getBrandCallback);
        },
        ...
        getModel: function (id) {
            ...
            json_ajax("GET", url, "json", false, null, vehicleAdd.getModelCallback);
        },
    ...
    $(function () {
        ...
        vehicleAdd.getVehicleCategory();
        $('input').inputClear();
        $("#doSubmitAdd").on("click", vehicleAdd.doSubmit);
        $("#zTreeCitySel").on("click", function () {
            vehicleAdd.showMenu(this)
        });
        $("#isRadio").on("click", vehicleAdd.isRadio);
        $("#noRadio").on("click", vehicleAdd.noRadio);
        $("#radionOn").val($("#isRadio").prop("checked"));
        $("#radionOff").val($("#noRadio").prop("checked"));

        $(".info-span").on("click", vehicleAdd.hiddenparameterFn);


        //按车辆类别对应的标准动态显示信息
        var curStandard = $("#vehicleCategory option:selected").attr("standard");
        vehicleAdd.checkCategoryStandard(curStandard);
        $('#vehicleCategory').change(function () {
            var selectStandard = $(this).find("option:selected").attr("standard");
            vehicleAdd.checkCategoryStandard(selectStandard);
        })

        //获取车辆类型
        var curId = $("#vehicleCategory").val();
        vehicleAdd.getVehicleType(curId);
        $("#vehicleCategory").on("change", function () {
            var curId = $(this).val();
            vehicleAdd.getVehicleType(curId);
            vehicleAdd.selfRespectRequired();
            vehicleAdd.abilityWorkRequired();
        ...
        vehicleAdd.getVehicleSubType(curId1);
        ...
        vehicleAdd.getBrand();
        //获取机型
        var brandId = $("#brandModel").val();
        vehicleAdd.getModel(brandId);
