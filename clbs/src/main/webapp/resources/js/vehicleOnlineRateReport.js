(function (window, $) {
    ...
    roadTransport = {
        init: function () {
            ...
            // 显示隐藏列
            roadTransport.showMenuText(nowMonthDays);
        ...
        exportAlarm: function () {
            ...
            var url = '/clbs/cb/cbReportManagement/vehicleOnlineRateReport/export/1';
            var parameter = {
                enterpriseIds: vehicleId,
                month: $('#select2').val(),
            ...
            if (date.success == true) {
                ...
                if (date.records != null && date.records.length != 0) {
                    ...
                    for (var i = 0; i < alarm.length; i++) {
                        ...
                        var dateList = [
                            i + 1,
                            alarm[i].enterpriseName,
                        ]
                        for (var j = 0; j < alarm[i].days.length; j++) {
                            if (alarm[i].days[j] == null) {
                                alarm[i].days[j] = ''
                            }
                            dateList.push(alarm[i].days[j])
                        }
                        dateList.push(alarm[i].monthReport)
    ...
    overspeedVehicle = {
        ...
        inquireClick: function (number) {
            ...
            var url = "/clbs/cb/cbReportManagement/vehicleOnlineRateReport/vehicleMonthList";
            var parameter = {
        ...
        exportAlarm: function () {
            ...
            var exportUrl = "/clbs/cb/cbReportManagement/vehicleOnlineRateReport/export/2";
            var parameter = {
                vehicleIds: vehicleIdTwo,
                month: $('#select3').val(),
                simpleQueryParam: $('#simpleQueryParamTwo').val()
            }
            json_ajax("POST", exportUrl, "json", true, parameter, function (data) {
                if (data.success) {
                    layer.confirm("已加入到导出队列,请注意查看导出管理消息提醒", {
                        icon: 3,
                        title: '操作确认',
            ...
            if (date.success == true) {
                ...
                if (date.records != null && date.records.length != 0) {
                    ...
                    for (var i = 0; i < alarm.length; i++) {
                        ...
                        var dateList = [
                            i + 1,
                            alarm[i].vehicleBrandNumber,
                            alarm[i].vehicleBrandColor,
        ...
        getTable: function (table) {
            ...
            $('#dataTableTwo').on('draw.dt', function () {
                var widthList = []
                $($('#dataTableTwo tr')[0]).find('th').each(function (idx, item) {
                    widthList.push(item.getBoundingClientRect().width + (widthList[idx - 1] ? widthList[idx - 1] : 0))
                })
                $('#dataTableTwo tr').each(function (idx, item) {
                    addClass(item, 5, widthList)
                })
            });

            function addClass(tr, threshold, widthList) {
                $(tr).find('td,th').each(function (idx, item) {
                    if (idx == 0) {
                    ...
                    if (idx < threshold - 1) {
                        ...
                        item.style.zIndex = 2
                        item.style.left = (widthList[idx - 1] ? widthList[idx - 1] : 0) + 'px'
                    }
                    if (idx == threshold - 1) {
                        item.classList.add('fix-left-last')
                        item.style.position = "sticky"
                        item.style.left = (widthList[idx - 1] ? widthList[idx - 1] : 0) + 'px'
                    }
                })
            ...
            $('#dataTableTwo_wrapper').on('scroll', function () {
                if (this.scrollLeft == 0) {
                    this.classList.remove('dataTable_wrapper')
                } else {
                    this.classList.add('dataTable_wrapper')
                }
            })
            //显示隐藏列
    ...
    vehicleDetail = {
        ...
        inquireClick: function (number) {
            ...
            var url = "/clbs/cb/cbReportManagement/vehicleOnlineRateReport/detailList";
            var parameter = {
        ...
        exportAlarm: function () {
            ...
            var exportUrl = "/clbs/cb/cbReportManagement/vehicleOnlineRateReport/export/3";
            var parameter = {
                vehicleIds: vehicleIdThree,
                startTime: startTimeThree,
                endTime: endTimeThree,
        ...
        getTable: function (table) {
            ...
            $("#search_buttonThree").on("click", function () {
                ...
                if ($('#dataTableThree .dataTables_empty').length != 0) {
        ...
        refreshTable: function () {
            ...
            myTableThree.column(1).search(tsval, false, false).draw();
            if ($('#dataTableThree .dataTables_empty').length > 0) {
                $('#exportAlarmThree').prop('disabled', true)
            } else {
                $('#exportAlarmThree').prop('disabled', false)
            }
        },
        //模糊查询树
        ...
        ajaxQueryDataFilter: function (treeId, parentNode, responseData) {
            ...
            for (var i = 0; i < nodesArr.length; i++) {
                ...
                if (treeId === 'treeDemoThree') {
                    if (nodesArr[i].type === 'group' || nodesArr[i].type === 'assignment') {
