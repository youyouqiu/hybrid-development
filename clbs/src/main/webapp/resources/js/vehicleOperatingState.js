(function (window, $) {
    ...
    vehicleOperatingState = {
        init: function () {
            ...
            var setting = {
                async: {
                    url: vehicleOperatingState.getAlarmReportTreeUrl,//"/clbs/m/functionconfig/fence/bindfence/vehicelTree"
                    type: "post",
                    enable: true,
                    autoParam: ["id"],
                    dataType: "json",
                    otherParam: {"type": "multiple", "icoType": "0"},
                    dataFilter: vehicleOperatingState.ajaxDataFilter
                },
                check: {
                    enable: true,
                    chkStyle: "checkbox",
                ...
                callback: {
                    beforeClick: vehicleOperatingState.beforeClickVehicle,
                    onAsyncSuccess: vehicleOperatingState.zTreeOnAsyncSuccess,
                    beforeCheck: vehicleOperatingState.zTreeBeforeCheck,
                    onCheck: vehicleOperatingState.onCheckVehicle,
                    onNodeCreated: vehicleOperatingState.zTreeOnNodeCreated,
                    onExpand: vehicleOperatingState.zTreeOnExpand
                }
            };
            ...
            if (param == null || param == undefined || param == '') {
                ...
                var querySetting = {
                    async: {
                        ...
                        dataFilter: vehicleOperatingState.ajaxQueryDataFilter
                    },
                    check: {
                        enable: true,
                        chkStyle: "checkbox",
        ...
        zTreeOnAsyncSuccess: function (event, treeId, treeNode, msg) {
            ...
            vehicleOperatingState.getCharSelect(treeObj);
        },
        ...
        onCheckVehicle: function (e, treeId, treeNode) {
            ...
            if (treeNode.checked) {
                ...
                setTimeout(() => {
                    vehicleOperatingState.getCheckedNodes();
                    vehicleOperatingState.validates();
                }, 600);
            }
            crrentSubV = [];
            crrentSubV.push(treeNode.id);
            vehicleOperatingState.getCharSelect(zTree);
            vehicleOperatingState.getCheckedNodes();
        },
            ...
            if (nodes != null && nodes != undefined && nodes.length > 0) {
                for (var i = 0; i < nodes.length; i++) {
                    ...
                    if (node.type == "assignment") {
                        ...
                        vehicleOperatingState.getGroupChild(node.children, assign);
                    }
                }
            }
        },
        ...
        inquireClick: function () {
            ...
            vehicleOperatingState.getCheckedNodes();
            if (!vehicleOperatingState.validates()) {
                return;
            }
            var url = "/clbs/m/reportManagement/vehicle/operationStatus/getData";
            var parameter = {
                "vehicleIds": vehicleId,
            };
            json_ajax("POST", url, "json", true, parameter, vehicleOperatingState.getCallback);
        },
        exportAlarm: function () {
            if(getRecordsNum() > 60000){
                return layer.msg("导出数据量过大，请缩小查询范围再试（单次导出最多支持6万条）！");
            }
            var url = "/clbs/m/reportManagement/vehicle/operationStatus/exportVehicleOperationData?param=" + simpleQueryParam;
        ...
        getCallback: function (date) {
            if (date.success == true) {
                //用来储存显示数据
                dataListArray = [];
                if (date.obj.data != null && date.obj.data.length != 0) {
                    var alarm = date.obj.data;
                    for (var i = 0; i < alarm.length; i++) {
                        var dateList =
                            [
                                ...
                                alarm[i].operatingStateStr ? alarm[i].operatingStateStr : '',
                                alarm[i].roadTransportValidityStartStr ? alarm[i].roadTransportValidityStartStr : '',
                                alarm[i].roadTransportValidityStr ? alarm[i].roadTransportValidityStr : '',
                            ];
                        dataListArray.push(dateList);
                    }
                    vehicleOperatingState.reloadData(dataListArray);
        ...
        refreshTable: function () {
            vehicleOperatingState.inquireClick(1, false);
        },
    };
    ...
    $(function () {
        ...
        vehicleOperatingState.tableFilter();
        vehicleOperatingState.getTable('#dataTable');

        //组织下拉显示
        $("#groupSelect,#typeGroupSelect").bind("click", showMenuContent);
        //导出
        $("#exportAlarm").bind("click", vehicleOperatingState.exportAlarm);
        $("#refreshTable").bind("click", vehicleOperatingState.refreshTable);

        $('input').inputClear().on('onClearEvent', function (e, data) {
            var id = data.id;
            if (id == 'groupSelect') {
                var param = $("#groupSelect").val();
                vehicleOperatingState.searchVehicleTree(param);
            }
            ;
        });
