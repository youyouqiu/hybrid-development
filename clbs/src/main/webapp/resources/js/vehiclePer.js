(function ($, window) {
    ...
    var requestRootURL = "/clbs/m/basicinfo/enterprise/assignment/vehiclePer.gsp";
    ...
    var assignmentVehiclePer = {
        ...
        onVehicleAsyncSuccess: function (event, treeId, node) {
            ...
            if (nodeCount > 0 && nodeCount <= 5000) {
                ...
                assignmentVehiclePer.expandGroupNode();
            }
        },
        expandGroupNode: function () {
            ...
            for (var i = 0; i < nodesByParamFuzzy.length; i++) {
                zTree.expandNode(nodesByParamFuzzy[i], true, false, false, false);
            }
        },
        ...
        onBeforeExpand: function (treeId, treeNode) {
            ...
            var zTree = $.fn.zTree.getZTreeObj("vehicleAssignmentDemo");
            if (treeNode != undefined && treeNode != null && treeNode.type == "assignment" && ("assignName" == queryType || "groupName" == queryType)) {
                isHideNode = false;
        ...
        leftMoveRight: function () {
            ...
            var curAssignNode = curNodes[0]; // 当前操作的分组 的节点
            ...
            if (leftMonitorNodes != null && leftMonitorNodes !== undefined && leftMonitorNodes.length > 0) {
            ...
            if (allCheckMonitorNodes.length > TREE_MAX_CHILDREN_LENGTH) {
                layer.msg("分配后的分组中监控对象将超限（" + TREE_MAX_CHILDREN_LENGTH + "个），请重新选择", {move: false});
                return;
            }
                ...
                for (var i = 0; i < vehicleAssignCheckNodes.length; i++) {
                    ...
                    if (node.type === "vehicle" || node.type === "people" || node.type === "thing") {
                        ...
                        if ((!assignmentVehiclePer.ztreeContains(node.id, checkNodes)) && checkNodes !== null && checkNodes.length >= TREE_MAX_CHILDREN_LENGTH) {
                            layer.msg("分组下已存在" + TREE_MAX_CHILDREN_LENGTH + "个监控对象", {move: false});
                            break;
                        }
                        var pNode = node.getParentNode();
                        var parentName = pNode.name;
                        var deleteObj = {}; // 删除的分组与车辆关系
                        ...
                        if (!assignmentVehiclePer.ztreeContains(node.id, checkNodes)) { // 右边树没有的节点，添加
                            var addObj = {}; // 新增的分组与车辆关系
                            addObj.vehicleId = node.id;
                            addObj.assignmentId = assignId; // 当前操作的分组id
        ...
        rightMoveLeft: function () {
            ...
            if (checkGroupList === null || checkGroupList === undefined || checkGroupList.length <= 0) {
                layer.msg("请选择分组！", {move: false});
                return;
            }
            var queryParam = $("#searchAllVehicle").val();
            var queryType = $("#searchType").val();
            if (queryType === 'name' && !(queryParam === '' || queryParam === undefined || queryParam === null)) {
                layer.msg("请先清空监控对象过滤条件，重新选择分组后再操作", {move: false});
                return;
            }
            if (isb) {
                tipsMsg = tipsMsg.substring(0, tipsMsg.length - 1);
                layer.msg("分配后的分组中监控对象将超限（" + TREE_MAX_CHILDREN_LENGTH + "），请重新选择：" + tipsMsg, {move: false});
                return;
            }
            if (vehicleCurCheckNodes !== null && vehicleCurCheckNodes.length > 0) {
                for (var i = 0; i < vehicleCurCheckNodes.length; i++) {
                    ...
                    if (node.type === "vehicle" || node.type === "people" || node.type === "thing") {
                        ...
                        for (var k = 0; k < checkGroupList.length; k++) {
                            ...
                            // 获取左边选中节点的子节点，判断若左边分组已存在的分组车辆关系，则不添加
                            if (!assignmentVehiclePer.ztreeContains(node.id, onClickLeftChildNodes)) {// 左边树没有的节点，添加
                                ...
                                var addObj = {}; // 新增的分组与车辆关系
                                addObj.vehicleId = node.id;
                                addObj.assignmentId = checkGroupList[k].id; // 当前操作的分组id
                ...
                checkGroupList.forEach(function (node) {
                    ...
                    if (node.children != undefined && node.children.length > TREE_MAX_CHILDREN_LENGTH) {
                        invalidNodesCount++;
                    }
                });
        ...
        rightCopyLeft: function () {
            ...
            if (isb) {
                tipsMsg = tipsMsg.substring(0, tipsMsg.length - 1);
                layer.msg("分配后的分组中监控对象将超限（" + TREE_MAX_CHILDREN_LENGTH + "个），请重新选择：" + tipsMsg, {move: false});
                return;
            }
            if (vehicleCurCheckNodes !== null && vehicleCurCheckNodes.length > 0) {
                for (var i = 0; i < vehicleCurCheckNodes.length; i++) {
        ...
        ztreeContains: function (vid, nodes) { // 分组下是否包含车辆
        ...
        searchAllVehicleAsync: function () {
            ...
            if (queryParam !== "") {
                ...
                if (queryType != null && queryType != undefined && ("assignName" == queryType || "groupName" == queryType)) {
