(function (window, $) {
    ...
    mainObj = {
        init: function () {
            ...
            var columns = [
                ...
                },
                {
                    ...
                    render: function (data, type, row, meta) {
                        var editUrlPath = '/clbs/v/workhourmgt/workhoursetting/getWorkHourSettingEditPage_' + row.vehicleId + '_' + row.sensorSequence + '_' + row.monitorType + ".gsp"; //修改地址
                        var bindUrlPre = '/clbs/v/workhourmgt/workhoursetting/getWorkHourSettingBindPage_{id}_' + row.monitorType;
                        var detailUrlPre = '/clbs/v/workhourmgt/workhoursetting/getWorkHourSettingDetailPage_' + row.vehicleId + '_' + row.sensorSequence+ ".gsp";
                        var basicInfoUrlPre = '/clbs/v/workhourmgt/workhoursetting/basicInfo_{id}.gsp';
                        var generalUrlPre = '/clbs/v/workhourmgt/workhoursetting/general_{id}.gsp';
                        var newsletterUrlPre = '/clbs/v/workhourmgt/workhoursetting/newsletter_{id}.gsp';
                        var baseValueUrlPre = '/clbs/v/workhourmgt/workhoursetting/getWorkHourBaseValue_{id}.gsp';
                        var parametersUrlPre = '/clbs/v/workhourmgt/workhoursetting/parameters_{id}.gsp';
                        var upgradeUrlPre = '/clbs/v/workhourmgt/workhoursetting/upgrade_{id}.gsp';
                        var result = '';
                        var paramId = "";
                        ...
                        if (row.id != null && row.id != "") {
                            ...
                            result += '<div class="btn-group"><button class="editBtn editBtn-info" type="button" id="dropdownMenuOther" data-toggle="dropdown" aria-haspopup="true" aria-expanded="true">其他<span class="caret"></span></button>' +
                                '<ul class="dropdown-menu" aria-labelledby="dropdownMenuOther" id="dropdownMenuContent" style="width: 316px">' +
                                ...
                                /*'<li><a href="' + baseValueUrlPre.replace("{id}", row.id) + '" data-toggle="modal" data-target="#commonWin">工时基值修正</a></li>' +*/
                                '</ul></div>';
                        } else {
                            // 设置
                ...
                },
                {
                    ...
                    render: function (data, type, row) {
                        var detectionMode = row.detectionMode;
                        if (detectionMode === 1) {
                            return row.thresholdVoltage;
                ...
                },
                {
                    ...
                    "data": "baudRateCalculateTimeScope",
                    "class": "text-center",
                    render: function (data) {
                        switch (data) {
                            case 1:
                ...
                {
                    "data": "baudRateCalculateNumber",
                    "class": "text-center"
                },{
                    "data": "baudRateThreshold",
                    "class": "text-center"
                },{
                    "data": "speedThreshold",
                    "class": "text-center"
            ...
            //表格setting
            var setting = {
                listUrl: '/clbs/v/workhourmgt/workhoursetting/list',
                editUrl: '/clbs/v/workhourmgt/workhoursetting/edit_',
                deleteUrl: '/clbs/v/workhourmgt/workhoursetting/deleteWorkHourSettingBind_',
                deletemoreUrl: '/clbs/v/workhourmgt/workhoursetting/deleteMoreWorkHourSettingBind',
        ...
        sendFuel: function (sendParam) {
            var url = "/clbs/v/workhourmgt/workhoursetting/sendWorkHourSetting";
        ...
        sendFuelCallback: function (data) {
            if (oilsendflag) {
                webSocket.subscribe(headers, '/user/topic/time', mainObj.updataFenceData, null, null);
                oilsendflag = false;
            }
            if (data.success) {
        ...
        updateColumn: function (id){
            for(var i= 0; i< tableList.length; i++) {
                if(id == tableList[i].vehicleId){
                    var url = '/clbs/v/workhourmgt/workhoursetting/refreshSendStatus?vehicleId=' + id;
