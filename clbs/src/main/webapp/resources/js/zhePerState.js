...
(function ($, window) {
    ...
    //socket订阅、下发指令
    var socketTypeArr = ['F764', 'F765', 'F766']; //[前向监测, 驾驶员行为监测, 盲区监测]
    var sensorIDArr = [100, 101, 102]; //[前向监测, 驾驶员行为监测, 盲区监测]
    ...
    zhePerState = {
        init: function () {
            ...
            for (var i=0; i<socketTypeArr.length; i++){
                ...
                //socket订阅
                zhePerState.subscribeParam(type);
                // //下发指令
                zhePerState.getDatas(sensorId);
            }
        },
        //websocket订阅
        subscribeParam: function (type) {
            webSocket.subscribe(headers, "/topic/per" + type + '_' + vid + "_Info", function (data) {
                zhePerState.getSensor0104Param(data, type);
            }, null, null);
        },
        ...
        queryF3BaseParamCall: function (result, type) {
            ...
            if (type == 'F764') {         //前向监测
                ...
                pre = '#adas-';
            } else if (type == 'F765') {  //驾驶员监测
                $('#dsm-workState').val(workStatus);
                pre = '#dsm-';
            }else if (type == 'F766') {   //盲区监测
            ...
            for (var i = 0; i < alarmData.length; i++) {
                ...
                var value = zhePerState.getAlarmStatus(item.value);
        ...
        getDatas: function (sensorID) {
            json_ajax("POST", '/clbs/adas/standard/param/getJiPeripheralInfo', "json", false, {
                ...
                "sensorID": sensorID,
                "commandType": 247,
            }, zhePerState.getF3BaseParamCall);
        },
        getF3BaseParamCall: function (data) {
            if (!data.success) {
                ...
                zhePerState.createSocket0104InfoMonitor();
            }
        },
    ...
    $(function () {
        zhePerState.init();
        //刷新
        $("#readPerStateRefresh").on("click", zhePerState.readInformationRefreshClick);
    })
})($, window);