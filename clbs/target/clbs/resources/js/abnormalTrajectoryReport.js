(function (window, $) {
    ...
    var bflag = true; //模糊查询
    var isSearch = true;
    var getAddressStatus = true;

    abnormalTrajectoryReport = {
        //开始时间
        startDay: function (day) {
            ...
            if (startValue == "" || endValue == "") {
                ...
                tMonth = abnormalTrajectoryReport.doHandleMonth(tMonth + 1);
                tDate = abnormalTrajectoryReport.doHandleMonth(tDate);
                var num = -(day + 1);
                ...
                endMonth = abnormalTrajectoryReport.doHandleMonth(endMonth + 1);
                endDate = abnormalTrajectoryReport.doHandleMonth(endDate);
                ...
                vMonth = abnormalTrajectoryReport.doHandleMonth(vMonth + 1);
                vDate = abnormalTrajectoryReport.doHandleMonth(vDate);
                ...
                if (day == 1) {
                    ...
                    vendMonth = abnormalTrajectoryReport.doHandleMonth(vendMonth + 1);
                    vendDate = abnormalTrajectoryReport.doHandleMonth(vendDate);
        ...
        inquireClick: function (number) {
            if (number == 0) {
                abnormalTrajectoryReport.getsTheCurrentTime();
            } else if (number == -1) {
                abnormalTrajectoryReport.startDay(-1)
            } else if (number == -3) {
                abnormalTrajectoryReport.startDay(-3)
            } else if (number == -7) {
                abnormalTrajectoryReport.startDay(-7)
            }
            if (number != 1) {
            ...
            abnormalTrajectoryReport.getCheckedNodes();
            if (!abnormalTrajectoryReport.validates()) {
                return;
            }
            var url = "/clbs/sx/sxReportManagement/abnormalTrajectoryReport/list";
            var parameter = {"band": vehicleId, "startTime": startTime, "endTime": endTime};
            json_ajax("POST", url, "json", true, parameter, abnormalTrajectoryReport.getCallback);
        },
        exportAlarm: function () {
            if(getRecordsNum() > 60000){
                return layer.msg("导出数据量过大，请缩小查询范围再试（单次导出最多支持6万条）！");
            }
            var url = "/clbs/sx/sxReportManagement/abnormalTrajectoryReport/export";
        ...
        zTreeOnAsyncSuccess: function (event, treeId, treeNode, msg) {
            ...
            if (size <= 100 && ifAllCheck) {
                ...
                abnormalTrajectoryReport.getCharSelect(treeObj);
            }
            bflag = false;
        },
        ...
        refreshTableClick: function () {
            ...
            //字段搜索重置
            //刷新表格
            abnormalTrajectoryReport.inquireClick(1, false);
        },
        onCheckVehicle: function (e, treeId, treeNode) {
            ...
            if (treeNode.checked) {
                ...
                setTimeout(() => {
                    abnormalTrajectoryReport.getCheckedNodes();
                    abnormalTrajectoryReport.validates();
                }, 600);
            }
            crrentSubV = [];
            crrentSubV.push(treeNode.id);
            abnormalTrajectoryReport.getCharSelect(zTree);
            abnormalTrajectoryReport.getCheckedNodes();
        },
            ...
            if (nodes != null && nodes != undefined && nodes.length > 0) {
                for (var i = 0; i < nodes.length; i++) {
                    ...
                    if (node.type == "assignment") {
                        ...
                        abnormalTrajectoryReport.getGroupChild(node.children, assign);
                    }
                }
            }
        },
                    ...
                    for (var i = 0; i < flowReportList.length; i++) {
                        ...
                        var dateList =
                            [
                                ...
                                flowReportList[i].lostStartTime,
                                flowReportList[i].lostEndTime,
                                flowReportList[i].lostCount,
                                flowReportList[i].completeRate,
                                "加载中...",
                                "加载中..."
                            ];
                        dataListArray.push(dateList);
                        startLoc.push(flowReportList[i].lostStartLocation);
                        endLoc.push(flowReportList[i].lostEndLocation);
                    }

                    abnormalTrajectoryReport.reloadData(dataListArray);
        ...
        goBack: function (GeocoderResult) {
            ...
            for (var i = 0; i < dataLength; i++) {
                if (msgArray[i] != undefined) {
                    $dataTableTbody.children("tr:nth-child(" + (i + 1) + ")").children("td:nth-child(11)").text(msgArray[i][0] === '中华人民共和国' ? '' : msgArray[i][0]);
                    $dataTableTbody.children("tr:nth-child(" + (i + 1) + ")").children("td:nth-child(12)").text(msgArray[i][1] === '中华人民共和国' ? '' : msgArray[i][1]);
                }
            }
        },
        //对显示的数据进行逆地址解析
        getAddress: function () {
            ...
            for (var i = 0; i < dataLength; i++) {
                ...
                if (startLoc[n - 1] != null && (Reg.test(startLoc[n - 1]) || startLoc[n - 1] === '0.0,0.0')) {
                    startMsg = [startLoc[n - 1].split(",")[0], startLoc[n - 1].split(",")[1]];
                } else {
                    startMsg = ["124.411991", "29.043817"];
                }
                if (endLoc[n - 1] != null && (Reg.test(endLoc[n - 1]) || endLoc[n - 1] === '0.0,0.0')) {
                ...
                if (num == dataLength) {
                    var addressIndex = 0;
                    var addressArray = [];
                    backAddressMsg(addressIndex, addressMsg, abnormalTrajectoryReport.goBack, addressArray);
                    addressMsg = [];
                }
            }
            ;
        ...
        getTable: function (table) {
            ...
            myTable = $(table).DataTable({
                ...
                "drawCallback": function (settings) {
                    var $dataTableTbody = $("#dataTable tbody")
                    var startLocation = $dataTableTbody.children("tr:last-child").children("td:nth-child(11)").text();
                    var endLocation = $dataTableTbody.children("tr:last-child").children("td:nth-child(12)").text();
                    //报警位置进行逆地址解析
                    if ((startLocation == "加载中..." || endLocation == "加载中...") && getAddressStatus) {
                        abnormalTrajectoryReport.getAddress();
                    }
                },
                "order": [
                    [0, null]
        ...
        showMenu: function (e) {
            ...
            $("body").bind("mousedown", abnormalTrajectoryReport.onBodyDown);
        },
        hideMenu: function () {
            $("#menuContent").fadeOut("fast");
            $("body").unbind("mousedown", abnormalTrajectoryReport.onBodyDown);
        },
        onBodyDown: function (event) {
            if (!(event.target.id == "menuBtn" || event.target.id == "menuContent" || $(event.target).parents("#menuContent").length > 0)) {
                abnormalTrajectoryReport.hideMenu();
            }
        },
        //模糊查询树
        searchVehicleTree: function (param) {
            ...
            if (param == null || param == undefined || param == '') {
                bflag = true;
                abnormalTrajectoryReport.treeInit()
            } else {
                bflag = true;
                var querySetting = {
                    async: {
                        ...
                        dataFilter: abnormalTrajectoryReport.ajaxQueryDataFilter
                    },
                    check: {
                        enable: true,
                        chkStyle: "checkbox",
                    ...
                    callback: {
                        beforeClick: abnormalTrajectoryReport.beforeClickVehicle,
                        onCheck: abnormalTrajectoryReport.onCheckVehicle,
                        onExpand: abnormalTrajectoryReport.zTreeOnExpand,
                        onNodeCreated: abnormalTrajectoryReport.zTreeOnNodeCreated
                    }
                };
        ...
        treeInit: function () {
            ...
            var setting = {
                async: {
                    url: abnormalTrajectoryReport.getTreeUrl,
                    type: "post",
                    enable: true,
                    autoParam: ["id"],
                    dataType: "json",
                    otherParam: {"type": "multiple", "icoType": "0"},
                    dataFilter: abnormalTrajectoryReport.ajaxDataFilter
                },
                check: {
                    enable: true,
                    chkStyle: "checkbox",
                ...
                callback: {
                    beforeClick: abnormalTrajectoryReport.beforeClickVehicle,
                    onCheck: abnormalTrajectoryReport.onCheckVehicle,
                    beforeCheck: abnormalTrajectoryReport.zTreeBeforeCheck,
                    onExpand: abnormalTrajectoryReport.zTreeOnExpand,
                    onAsyncSuccess: abnormalTrajectoryReport.zTreeOnAsyncSuccess,
                    onNodeCreated: abnormalTrajectoryReport.zTreeOnNodeCreated
                }
            };
    ...
    $(function () {
        ...
        //当前时间
        $('input').inputClear();
        $("#refreshTable").bind("click", abnormalTrajectoryReport.refreshTableClick);
        abnormalTrajectoryReport.getsTheCurrentTime();
        abnormalTrajectoryReport.getTable('#dataTable');
        $('#timeInterval').dateRangePicker({
            dateLimit: 31
        });
        $("#groupSelectSpan,#groupSelect").bind("click", abnormalTrajectoryReport.showMenu); //组织下拉显示
        $('input').inputClear().on('onClearEvent', function (e, data) {
            var id = data.id;
            if (id == 'groupSelect') {
                var param = $("#groupSelect").val();
                abnormalTrajectoryReport.searchVehicleTree(param);
            }
            ;
        });
        /**
        ...
        $("#groupSelect").on('input propertychange', function (value) {
            ...
            inputChange = setTimeout(function () {
                if (isSearch) {
                    ...
                    abnormalTrajectoryReport.searchVehicleTree(param);
                    setTimeout(function () {
                        abnormalTrajectoryReport.showMenu();
                    }, 300);
                }
                isSearch = true;
            }, 500);
        ...
        //导出
        $("#exportAlarm").bind("click", abnormalTrajectoryReport.exportAlarm);
