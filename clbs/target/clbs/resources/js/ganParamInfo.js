...
(function ($, window) {
    ...
    //参数下发是的风险事件id
    var arr0 = ['', '', 196401, 196402, 196403, 196404, 196405, 196407, 196409],//前向事件id,''表示公共参数
        arr1 = ['', '', 196513, 196503, 196502, 196508, 196504, 196514, 196515],//驾驶员事件id
        arr2 = [196704, ''];  //盲区事件id
    ...
    ganParamInfo = {
        init: function () {
            ...
            for(var i=0;i<socketTypes.length;i++){
                ...
                ganParamInfo.subscribeSocket(socketType);//socket订阅
                ganParamInfo.getPeripheralInfo(sensorID);//传感器参数下发
            }
        },
        /**
         * socket订阅
        ...
        subscribeSocket: function (type) {
            webSocket.subscribe(headers, "/user/topic/per" + type + "Info", function (data) {
                ganParamInfo.getSensor0104Param(data, type);
            }, null, null);
        },
        /**
         * 传感器参数指令下发
        ...
        getPeripheralInfo: function (sensorID) {
            ...
            json_ajax("POST", url, "json", false, {
                ...
                "sensorID": sensorID,
                "commandType": 243
            }, ganParamInfo.setreadConventionalCall);
        },
        setreadConventionalCall: function (data) {
            if (data.success) {
                ...
                ganParamInfo.createSocket0104InfoMonitor(data.msg);
            } else {
                if (data.msg) {
                    layer.msg(data.msg);
                }
        ...
        getSensor0104Param: function (msg, type) {
            ...
            ganParamInfo.setSenorInputData(param, type);
        ...
        setSenorInputData: function (result, type) {
            ...
            for (var key in data) {
                ...
                switch (key) {
                    case 'driveDeedType':
                    case 'cameraStrategy':
                        value = ganParamInfo.getCaptureType(value);
                        dom.val(value);
                        break;
                    case 'eventEnable':
                    case 'alarmEnable':
                        ganParamInfo.getBinaryValue(value, tabId, key);
                        break;
                    case 'cameraResolution':
                        var dataValue = '0x0' + parseInt(value, 10).toString(16);
                        value = ganParamInfo.changeCameraResolution(value);
                        dom.val(value).attr('data-value', dataValue);
                        break;
                    case 'videoResolution':
                        var dataValue = '0x0' + parseInt(value, 10).toString(16);
                        value = ganParamInfo.changeVideoResolution(value);
                        dom.val(value).attr('data-value', dataValue);
                        break;
                    case 'fixedCamera':
                        value = value / 1000;
                    ...
                    case 'unit'://胎压单位
                        ...
                        break;
                    case 'tyreNumber':
                        dom.val(value).attr('data-value',ganParamInfo.tireModel(value));
                        break;
                    default:
                        dom.val(value);
                        break;
        ...
        getPlatformData: function () {
            ...
            for (var i = 0; i < platformData.length; i++) {
                ...
                if (i < 2) {//有公共参数的页签
                    ganParamInfo.getCommonParamSetting(item.commonParamSetting, tab);
                }
                ganParamInfo.getAdasAlarmParamSettings(item.adasAlarmParamSettings, tab);
            }
        },
        //全局参数
            ...
            for (var key in data) {
                ...
                switch (key) {
                    ...
                    case 'cameraResolution'://拍照分辨率
                        ...
                        value = ganParamInfo.changeCameraResolution(num);
                        break;
                    case 'videoResolution'://视频分辨率
                        var num = Number(parseInt(value, 16).toString(10));
                        value = ganParamInfo.changeVideoResolution(num);
                        break;
                    case 'photographTime':
                        value = value / 10;
                        break;
        ...
        getAdasAlarmParamSettings: function (data, tab) {
            for (var i = 0; i < data.length; i++) {
                ...
                switch (riskId) {
                    case 196704: //盲区
                        ganParamInfo.setInputValue(item,riskEvent.eq(0));
                        break;
                    case 196401://前向碰撞
                    case 196513://疲劳驾驶
                        ganParamInfo.setInputValue(item, riskEvent.eq(1));
                        break;
                    case 196402://车道偏离
                    case 196503://抽烟
                        ganParamInfo.setInputValue(item, riskEvent.eq(2));
                        break;
                    case 196403://车距过近
                    case 196502://接打手持电话
                        ganParamInfo.setInputValue(item, riskEvent.eq(3));
                        break;
                    case 196404://行人碰撞
                    case 196508://分神驾驶
                        ganParamInfo.setInputValue(item, riskEvent.eq(4));
                        break;
                    case 196405://频繁变道
                    case 196504://驾驶人行为异常
                        ganParamInfo.setInputValue(item, riskEvent.eq(5));
                        break;
                    case 196407://障碍物
                    case 196514://驾驶员身份识别
                        ganParamInfo.setInputValue(item, riskEvent.eq(6));
                        break;
                    case 196409://道路标识超限
                    case 196515://驾驶员未系安全带
                        ganParamInfo.setInputValue(item, riskEvent.eq(7));
                        break;
                    default:
                        break;
                }
        ...
        paramSendAllClick: function () {
            var self = $(this);
            ganParamInfo.preventclick(self);
            isAll = true;
            var jsonDataArr = [];
            ...
            for (var i = 0; i < riskIds.length; i++) {
                ...
                var jsonData = ganParamInfo.setJsonData(riskIds[i], $('.tab-pane').eq(i));
                jsonDataArr.push(jsonData[0]);
            }
            // console.log('全部下发参数', jsonDataArr);
            ganParamInfo.dsmInfoSend(jsonDataArr);
        },
        paramSendClick: function () {
            ...
            var jsonData = ganParamInfo.setJsonData(arr, tab);
            // console.log('本页签下发参数', jsonData);
            ganParamInfo.dsmInfoSend(jsonData);
        },
        //组装下发参数
        setJsonData: function (arr, tab) {
            ...
            for (var i = 0; i < arr.length; i++) {
                ...
                if (item == '') {//全局参数/主动抓拍策略
                    ...
                    commonParamSetting = ganParamInfo.getCommontData(inputs);
                } else {//风险事件
                    inputs = tab.find('.info-content').eq(i).find('input');
                    adasAlarmParamSettings.push(ganParamInfo.getInputData(item, inputs));
                }
            }
        ...
        dsmInfoSend: function (jsonData) {
            ...
            json_ajax("POST", "/clbs/adas/standard/param/setting.gsp", "json", false, data, ganParamInfo.dsmInfoSendCallback);
        },
        dsmInfoSendCallback: function (data) {
            if (data != null) {
                if (data.success) {
                    ...
                    //获取下发状态
                    setTimeout(function () {
                        timerInterval = setInterval(ganParamInfo.getStatus(), 5000);
                    }, 2000);
                    setTimeout(function () {
            ...
            json_ajax("POST", url, "json", false, {
                ...
                if (data.success) {
                    ...
                    if (obj) {
                        ...
                        for (var key in obj) {
                            var value = obj[key];
                            status.push(ganParamInfo.statusChange(value));
                        }
                        ...
                        timer = setTimeout(function () {
                            ...
                            ganParamInfo.getPeripheralInfo(sensorID);
                        }, 1000);
                    }
    ...
    $(function () {
        ...
        //事件
        $(".info-span").on("click", ganParamInfo.hiddenparameterFn);//点击显示隐藏信息
        $("#dsmInfoSend").on("click", ganParamInfo.paramSendAllClick);//全部修正下发
        $("#dsmInfoSend2").on("click", ganParamInfo.paramSendClick);//本页签修正下发
        $("#dsmInfoRefresh").on("click", ganParamInfo.readConventionalRefreshClick);//刷新
        $(".sensorBtn").on("click", ganParamInfo.sensorBtnClick);//以传感器为准
        $(".platformBtn").on("click", ganParamInfo.platformBtnClick);//以平台设置为准
    })
})($, window)