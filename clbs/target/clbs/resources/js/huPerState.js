...
(function ($, window) {
    ...
    huPerState = {
        init: function () {
            ...
            for (var i=0; i<socketTypeArr.length; i++){
                ...
                //socket订阅
                huPerState.subscribeParam(type);
                // //下发指令
                huPerState.getDatas(sensorId);
            }
        },
        //websocket订阅
        subscribeParam: function (type) {
            webSocket.subscribe(headers, "/topic/per" + type + '_' + vid + "_Info", function (data) {
                huPerState.getSensor0104Param(data, type);
            }, null, null);
        },
        ...
        queryF3BaseParamCall: function (result, type) {
            ...
            for (var i = 0; i < alarmData.length; i++) {
                ...
                var value = huPerState.getAlarmStatus(item.value);
        ...
        getF3BaseParamCall: function (data) {
            if (!data.success) {
                ...
                huPerState.createSocket0104InfoMonitor();
            }
        },
    ...
    $(function () {
        huPerState.init();
        //刷新
        $("#readPerStateRefresh").on("click", huPerState.readInformationRefreshClick);
    })
})($, window);