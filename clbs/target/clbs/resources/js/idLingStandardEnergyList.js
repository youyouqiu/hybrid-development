(function ($, window) {
	...
	idLingStandardEnergyList = {
        echartsInit: function () {
            ...
            var option = {
                tooltip: {
                    ...
                    formatter: function (a) {
                    	...
                    	var unit = ['L','km','km/h','ACC'];
                        var relVal = "";
                        var addRelVal = "";
                        var relValTime = '';
                        if(a[2].data === null || a[2].data === '' || a[2].data === undefined){
                       	 	relVal = "无相关数据";
                        }else{
						   relValTime = a[0].name;
                           for(var i = 0; i < a.length; i++){
                               if(a[i].seriesName != "ACC"){
                               		if(a[i].data == null || a[i].data == undefined){
            ...
            myChart.on('click', idLingStandardEnergyList.chartsClickEvent);
        	...
        	layer.msg('<label style="cursor:pointer;"><input type="radio" data-state="1" name="oilState" checked />基准开始时间</label><br/><label style="cursor:pointer;"><input data-state="2" name="oilState" type="radio" />基准结束时间</label>', {
                ...
                json_ajax("POST", url, "json", false, parameter, function(data) {
                	if (data.success === false) {
                		...
                		if (state === '1') { // 基准开始时间
            				if (x_oil_after !== null) {
            					var time_state = idLingStandardEnergyList.compareTime(time, x_oil_after);
            					if (time_state === true) {
            						x_oil_before = time;
            						y_oil_before = value;
            						$('#criterionStartTime').attr('value', time);
            						idLingStandardEnergyList.echartsInit();
                  	      	    	layer.closeAll();
            					} else {
            				...
            				if (x_oil_before !== null) {
            					var time_state = idLingStandardEnergyList.compareTime(x_oil_before, time);
            					if (time_state === true) {
            						x_oil_after = time;
            						y_oil_after = value;
        ...
        compareTime: function(stime, etime) {
        	...
        	} else if (time_count < (20 * 60 * 1000) || time_count > (24 * 60 * 60 * 1000)) {
        		return timeError;
        	} else {
        		return true;
        	}
        ...
        criterionDataAddFun: function() {
        	...
        	if (tableList.length < 10) {
        		var url="/clbs/m/energy/idLingStandardEnergy/computeIdLingDate";
                var parameter={"vehicleId": vehicleId, "standardStartTime": start_time, "standardEndTime": end_time};
                json_ajax("POST", url, "json", true, parameter, idLingStandardEnergyList.addCriterionState);
        	} else {
        ...
        addCriterionState: function(data) {
        	if (data.success) {
                layer.msg(data.msg);
        		var url="/clbs/m/energy/idLingStandardEnergy/getIdLingStandard";
                var parameter={"vehicleId": vehicleId};
                json_ajax("POST", url, "json", true, parameter, idLingStandardEnergyList.getCriterionData);
        	} else {
        		layer.msg(data.msg);
        	}
        },
        ...
        getCriterionData: function(data) {
        	var tableData = data.obj.idleStandardList; // table数据
			var energyData = data.obj.idleEneryGather; // table下面统计数据
        	idLingStandardEnergyList.assembleTableData(tableData,energyData);
        },
        // 初始化组织树
        treeInit: function(){
        	var setting = {
	            async: {
	                url: idLingStandardEnergyList.getTreeUrl,
	                type: "post",
	                enable: true,
	                autoParam: ["id"],
	                dataType: "json",
	                ...
	                dataFilter: idLingStandardEnergyList.ajaxDataFilter
	            },
	            check: {
	                enable: true,
	                chkStyle: "checkbox",
	            ...
	            callback: {
	                beforeClick: idLingStandardEnergyList.beforeClickVehicle,
	                onCheck: idLingStandardEnergyList.onCheckVehicle,
	                beforeCheck: idLingStandardEnergyList.zTreeBeforeCheck,
					onExpand: idLingStandardEnergyList.zTreeOnExpand,
					onAsyncSuccess: idLingStandardEnergyList.zTreeOnAsyncSuccess,
					onNodeCreated: idLingStandardEnergyList.zTreeOnNodeCreated
	            }
	        };
        ...
        searchVehicleTree: function (param) {
            ...
            if (param == null || param == undefined || param == '') {
                bflag = true;
                idLingStandardEnergyList.treeInit();
            } else {
                bflag = true;
                var querySetting = {
                    async: {
                        ...
                        dataFilter: idLingStandardEnergyList.ajaxQueryDataFilter
                    },
                    check: {
                        enable: true,
                        chkStyle: "checkbox",
        ...
        zTreeOnAsyncSuccess: function(event, treeId, treeNode, msg){
        	...
        	idLingStandardEnergyList.getCharSelect(treeObj);
        },
        ...
        onCheckVehicle: function(e, treeId, treeNode){
        	...
        	if(treeNode.type != "assignment" || (treeNode.type == "assignment" && treeNode.children != undefined)){
        		idLingStandardEnergyList.getCharSelect(zTree);
        	}
        ...
        getCharSelect: function (treeObj) {
			...
			var vehName = idLingStandardEnergyList.unique(veh);
			var vehId = idLingStandardEnergyList.unique(vid);
			$("#charSelect").empty();
			...
			$("#groupSelect,#groupSelectSpan").bind("click",idLingStandardEnergyList.showMenu);
        ...
        yesterdayClick: function () {
            idLingStandardEnergyList.startDay(-1);
            $('#timeInterval').val(startTime + '--' + endTime);
            vehicleId = $("#charSelect").attr("data-id");
            if (vehicleId !== "") {
                // 清空数据
                idLingStandardEnergyList.clearData();
                var url="/clbs/m/energy/idLingStandardEnergy/getIdleDate";
                var parameter={"vehicleId": vehicleId, "startTime": startTime, "endTime": endTime};
                json_ajax("POST", url, "json", true, parameter, idLingStandardEnergyList.searchDataFun);
            } else {
        ...
        startDay: function (day) {
            ...
            if (startValue == "" || endValue == "") {
                ...
                tMonth = idLingStandardEnergyList.doHandleMonth(tMonth + 1);
                tDate = idLingStandardEnergyList.doHandleMonth(tDate);
                var num = -(day + 1);
                ...
                endMonth = idLingStandardEnergyList.doHandleMonth(endMonth + 1);
                endDate = idLingStandardEnergyList.doHandleMonth(endDate);
                ...
                vMonth = idLingStandardEnergyList.doHandleMonth(vMonth + 1);
                vDate = idLingStandardEnergyList.doHandleMonth(vDate);
                ...
                if (day == 1) {
                    ...
                    vendMonth = idLingStandardEnergyList.doHandleMonth(vendMonth + 1);
                    vendDate = idLingStandardEnergyList.doHandleMonth(vendDate);
        ...
        inquireClick: function (number) {
            if (number == 0) {
                idLingStandardEnergyList.getsTheCurrentTime();
            } else if (number == -1) {
                idLingStandardEnergyList.startDay(-1)
            } else if (number == -3) {
                idLingStandardEnergyList.startDay(-3)
            } else if (number == -7) {
                idLingStandardEnergyList.startDay(-7)
            }
            var timeInterval = $('#timeInterval').val().split('--');
            var startTime_now = timeInterval[0];
            var endTime_now = timeInterval[1];
            var state = idLingStandardEnergyList.searchTimeCompare(startTime_now, endTime_now);
            if (state === true) {
            	// var searchVehicleId = '021d496a-5ff7-4a60-9567-4f243e82fa82'
            	vehicleId = $("#charSelect").attr("data-id");
                if (vehicleId != "") {
                	// 清空数据
                	idLingStandardEnergyList.clearData();
        ...
        searchDataFun: function(data) {
        	...
        	if(data.success == true){
                var echartsData = JSON.parse(ungzip(data.obj.idleDataList));
                var tableData = data.obj.idleStandardList; // table数据
                var energyData = data.obj.idleEneryGather; // table下面统计数据
				var isNotBind = data.obj.isNotBind; // 监控对象是否绑定里程传感器
                for (var i = 0, len = echartsData.length; i < len; i++) {
                    ...
                    if(time>300){
                        ...
                        for(var k=0;k<j;k++){
                            vtime=vtime+300;
                            var time = idLingStandardEnergyList.getTime(vtime);
                    ...
                    var time = idLingStandardEnergyList.getTime(value.vtime);
                    echartsDate.push(time);
                    var totalOilwearOne = value.totalOilwearOne;
					if(totalOilwearOne == 0 || totalOilwearOne == null){
						totalOilwearOne = "";
					}else{
                        totalOilwearOne = totalOilwearOne;
					}
                    oilConsume.push(totalOilwearOne);//油耗
					var mileage;
					var speed;
					if(isNotBind){ //绑定里程传感器
                        mileage=Number(value.mileageTotal).toFixed(1);
                        speed=Number(value.mileageSpeed).toFixed(1);
					}else { // 未绑定里程传感器
                        mileage = Number(value.gpsMile).toFixed(1);
                        speed= Number(value.speed).toFixed(1);
					}
					if(mileage == 0 || mileage == null){
						mileage = "";
					}
                    if (mileage.substr(mileage.length - 1, 1) == '0'&&mileage!='0') {
                    ...
                    if(speed==null||mileage==NaN||mileage==0){
                        speed="";
                    }
                    allMileage.push(mileage);//里程
                    echartsSpeed.push(speed);//速度
                    accState.push(value.acc);//acc状态
                }
                $('#graphShow').removeClass('hidden');
                $('#showClick').attr("class", "fa fa-chevron-down");
                if (echartsData.length > 0) {
                    $('#addCriterionTime').removeClass('hidden');
                } else {
                    $('#addCriterionTime').attr('class', 'form-horizontal table-top hidden');
                }
                idLingStandardEnergyList.echartsInit();
                idLingStandardEnergyList.assembleTableData(tableData, energyData);
			}
        ...
        // 组装table及下面统计数据
        assembleTableData: function(tableData, energyData) {
            tableList = [];
            for (var i = 0, len = tableData.length; i < len; i++) {
                ...
                list.push(value.vehicleType);// 车辆类型
                list.push(value.fuelOilType == null ? "-":value.fuelOilType);// 燃油类型
                list.push(value.startTime);// 基准开始时间
                list.push(value.endTime);// 基准结束时间
                list.push(idLingStandardEnergyList.formatDuring(value.idleTime));// 怠速时长
                list.push(value.idleMile == null ? "-" : value.idleMile);// 慢速里程 保留三位小数
                list.push(value.idleFuel == null ? "-" : value.idleFuel);// 怠速油耗
                list.push(value.idleBase == null ? "-" : value.idleBase);//怠速油耗基准
                list.push(value.idleBaseCap == null ? "-" : value.idleBaseCap);//怠速CO2排放基准
                tableList.push(list);
            }
            idLingStandardEnergyList.exampleTable(tableList);
        	if (energyData !== undefined && energyData !== null) {
        		if(energyData.idleTime != null && energyData.idleTime != 0 && energyData.idleTime != undefined){
                    var idleTime = idLingStandardEnergyList.formatDuring(energyData.idleTime);//将毫秒转具体时间
                    $('#allRunMileage').attr('value', idleTime);
				}else{
                    $('#allRunMileage').attr('value', '0小时0分钟0秒');
				}
            	if(energyData.idleTotal != null && energyData.idleTotal != "" && energyData.idleTotal != undefined){
                    $('#allRunOil').attr('value',energyData.idleTotal + ' L');
				}else{
                    $('#allRunOil').attr('value', '0 L');
				}
            	if(energyData.idleBaseList != null && energyData.idleBaseList != "" && energyData.idleBaseList != undefined){
                    $('#runOilCriterion').attr('value', energyData.idleBaseList + ' L/h');
				}else{
                    $('#runOilCriterion').attr('value', '0 L/h');
				}
            	if(energyData.idleMile != null && energyData.idleMile != "" && energyData.idleMile != undefined){
                    $('#averageSpeed').attr('value', energyData.idleMile + ' km');
				}else{
                    $('#averageSpeed').attr('value', '0 km');
				}
            	if(energyData.idleTotalCap != null && energyData.idleTotalCap != "" && energyData.idleTotalCap != undefined){
                    $('#coTotalCap').attr('value',energyData.idleTotalCap + ' kg');
				}else{
                    $('#coTotalCap').attr('value', '0 kg');
				}
            	if(energyData.idleBaseCap != null && energyData.idleBaseCap != "" && energyData.idleBaseCap != undefined){
                    $('#coBaseCap').attr('value', energyData.idleBaseCap + ' kg/h');
				}else{
                    $('#coBaseCap').attr('value', '0 kg/h');
				}
        			...
        			for (var j = 0, jlen = data[i].length; j < jlen; j++) {
                        if (permission == 'false'){
                            ...
                            if (j !== 1) {
                                ...
                                html += '<td><button data-id="' + data[i][j] + '" type="button" onclick="idLingStandardEnergyList.removeCriterionData(\'' + data[i][j] + '\')" class="deleteButton editBtn disableClick"><i class="fa fa-trash-o"></i>删除</button></td>';
                            }
                        }
        			}
        			html += '</tr>'
        ...
        removeCriterionData: function(id) {
        	layer.confirm(publicDelete, {btn : ["确定", "取消"],icon: 3,title: "操作确认"}, function(){
            	// 传入后台删除数据
            	var url="/clbs/m/energy/idLingStandardEnergy/delete";
                var parameter={"id": id,"vehicleId":vehicleId};
                json_ajax("POST", url, "json", true, parameter, idLingStandardEnergyList.criterionDelete);
    		});
        },
        // 基准数据删除回掉函数
...
//        	var travel_mile = $('#allRunMileage').text()
//        	,travel_total = $('#allRunOil').text()
//        	,travel_base = $('#runOilCriterion').text()
//        	,avg_speed = $('#averageSpeed').text()
//        	,travel_total_cap = $('#coTotalCap').text()
//        	,travel_base_cap = $('#coBaseCap').text()
//        	// ,vehicleId = '021d496a-5ff7-4a60-9567-4f243e82fa82'
//        	// ,vehicleId = $('#charSelect').attr('data-id')
//    		,url="/clbs/m/energy/idLingStandardEnergy/updateIdleCollect"
//            ,parameter={'vehicleId': vehicleId};
//            json_ajax("POST", url, "json", true, parameter, idLingStandardEnergyList.saveCriterionFun);
//        },
        // 保存列表数据回掉函数
//        saveCriterionFun: function(data) {
//        	var idleEnergy = data.obj.idleEnergy;
//        	if (data.success) {
//        		layer.msg('保存成功');
//        		var idleTime = idLingStandardEnergyList.formatDuring(idleEnergy.idleTime);//将毫秒转具体时间
//            	$('#allRunMileage').text(idleTime);
//            	$('#allRunOil').text(Number(idleEnergy.idleTotal).toFixed(3));
//            	$('#runOilCriterion').text(Number(idleEnergy.idleBaseList).toFixed(3));
//            	$('#averageSpeed').text(Number(idleEnergy.idleMile).toFixed(3));
//            	$('#coTotalCap').text(Number(idleEnergy.idleTotalCap).toFixed(3));
//            	$('#coBaseCap').text(Number(idleEnergy.idleBaseCap).toFixed(3));
//        	} else {
//        		layer.msg('保存失败');
//        	}
//        },
        ...
        getAddress: function(){
    		...
    		for(var i = 0; i < dataLength; i++){
				...
				if(num == dataLength){
					var addressIndex = 0;
					var addressArray = [];
					backAddressMsg(addressIndex,addressMsg,idLingStandardEnergyList.goBack,addressArray);
					addressMsg = [];
				}
  	  		};
        }, 
        ...
        showMenu: function(e){
            ...
            $("body").bind("mousedown", idLingStandardEnergyList.onBodyDown);
        },
        hideMenu: function(){
            $("#menuContent").fadeOut("fast");
            $("body").unbind("mousedown", idLingStandardEnergyList.onBodyDown);
        },
        onBodyDown: function(event){
            if (!(event.target.id == "menuBtn" || event.target.id == "menuContent" || $(event.target).parents("#menuContent").length > 0)) {
            	idLingStandardEnergyList.hideMenu();
            }
        },
    ...
    $(function () {
        ...
        $("#todayClick").bind("click", idLingStandardEnergyList.todayClick);
        $("#right-arrow").bind("click", idLingStandardEnergyList.yesterdayClick);
        $("#nearlyThreeDays").bind("click", idLingStandardEnergyList.nearlyThreeDays);
        $("#nearlySevenDays").bind("click", idLingStandardEnergyList.nearlySevenDays);
        $("#showClick,#graphAreaHead").bind("click", idLingStandardEnergyList.showClick);
        $("#left-arrow").bind("click",idLingStandardEnergyList.upDay);
	    $("#endTime").bind("click",idLingStandardEnergyList.endTimeClick);
	    $("#groupSelectSpan,#groupSelect").bind("click",idLingStandardEnergyList.showMenu); //组织下拉显示
	    $('#criterionDataAdd').on('click', idLingStandardEnergyList.criterionDataAddFun);
	    // $('#saveCriterionData').on('click', idLingStandardEnergyList.saveCriterionData);
        if (permission == 'false'){
            $("#addCriterionTime button").remove();
        }
        ...
        $('input').inputClear().on('onClearEvent',function(e,data){
            ...
            if(id == 'groupSelect'){
                ...
                idLingStandardEnergyList.searchVehicleTree(param);
            };
        });
