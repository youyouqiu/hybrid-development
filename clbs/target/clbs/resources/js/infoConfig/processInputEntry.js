(function (window, $) {
    ...
    window.processInput = {
        ...
        putConfigValue: function () {
            ...
            if (brandsDropdown) {
                brandsDropdown.destory();
            }
            brandsDropdown = $('#brandsContainer').dropdown({
                data: dataList.value,
                pageCount: 50,
                listItemHeight: 31,
                searchUrl: dataList.value.length >= MAX_MENU_LENGTH ? '/clbs/m/infoconfig/infoFastInput/fuzzyMonitor?monitorType=' + objType + '' : null,
          ...
          flag1 = false;
            ...
            $('#devicesContainer').dropdown({
                ...
                pageCount: 50,
                listItemHeight: 31,
                searchUrl: deviceDataList.value.length >= MAX_MENU_LENGTH ? '/clbs/m/infoconfig/infoFastInput/fuzzyDevice' : null,
                onDataRequestSuccess: function (e, result) {
                },
                onSetSelectValue: function (e, keyword, data) {
                    $(".terminalNew").hide(); // 选择的下拉框的值，隐藏新增模块
            ...
            if (professionalDropdown) {
                professionalDropdown.destory();
            }
            professionalDropdown = $('#professionalsContainer').dropdown({
                data: professionalDataList.value,
                pageCount: 50,
                listItemHeight: 31,
                searchUrl: professionalDataList.value.length >= MAX_MENU_LENGTH ? '/clbs/m/infoconfig/infoinput/getProfessionalSelect' : null,
                onDataRequestSuccess: function (e, result) {
                },
                onSetSelectValue: function (e, keyword, data) {
                    if (processInput.checkDoubleChoosePro(keyword.id)) {// 校验是否重复选择了从业人员
                        $("#professionalsidVal").attr("value", keyword.id);
                        $("#professionalsContainer input[name='professionalsID']").val(keyword.id);
        ...
        peopleAdd: function () {
            ...
            container.attr("id", "professionalsContainer" + people);
            ...
            var searchUrl = professionalDataList.value.length >= MAX_MENU_LENGTH ? '/clbs/m/infoconfig/infoinput/getProfessionalSelect' : null;
            $('#professionalsContainer' + people).dropdown({
                data: professionalDataList.value,
                pageCount: 50,
                listItemHeight: 31,
                selectedValue: '',
                ...
                onSetSelectValue: function (e, keyword, data) {
                    ...
                    if (processInput.checkDoubleChoosePro(keyword.id)) {// 校验是否重复选择了从业人员
                        ...
                        $(this).siblings("input").val(keyword.id);
                        data.$this.find("input[name='professionalsID']").val(keyword.id);
                        // 根据选择的从业人员id，查询从业人员详情，并在页面显示
        ...
        nextBtnBrand: function () {
            ...
            if ($("#addNew").is(":hidden")) {
                ...
                /*var processGroupIdList = $('.processGroupId');
        ...
        showMenu1: function (e) {
            ...
            if ($("#menuContent").is(":hidden")) {
                ...
                /* var treeObj = $.fn.zTree.getZTreeObj("ztreeDemo1");
                 treeObj.checkAllNodes(false);
                 search_ztree('ztreeDemo1', cid, 'assignment');*/
        ...
        clearProcessInfo: function () {
            ...
            $('#brandVal').val('');
            /* $('#citySel').val('').attr('value', '');
             $("#groupingArea input[name='citySel']").val('').attr('value', '');
             $("#addGrouping input[name='citySel']").val('').attr('value', '');
             $('#ztreeDemo1 .curSelectedNode').removeClass('curSelectedNode');*/
        ...
        submits: function () {
            ...
            layer.confirm(confirmMsg,
                {
                    btn: [confirmSureBtn, confirmCancleBtn],
                    cancel: function () {
                        enterFlag = false;
                    }
                },
                processInput.doSubmits,
                function () {
                    enterFlag = false;
                }
            );
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data != null && data.obj != null && data.obj.vehicleInfo != null) {
                        ...
                        $("#vehicle_category").text(processInput.converterNullToBlank(data.obj.vehicleInfo.vehicleCategoryName));
                        ...
                        $("#vehicle_county").text(processInput.converterNullToBlank(data.obj.vehicleInfo.county));
                        $("#vehicle_operating_categories").text(processInput.converterNullToBlank(data.obj.vehicleInfo.vehiclePurposeName));
                    }
                },
                error: function () {
                    layer.msg(systemError, {
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data != null && data.obj != null && data.obj.deviceInfo != null) {
                        ...
                        $("#device_installDate").text(processInput.converterNullToBlank(data.obj.deviceInfo.installTime));
                    }
                },
                error: function () {
                    layer.msg(systemError, {
                ...
                success: function (data) {
                    if (data != null && data.obj != null && data.obj.professionalInfo != null) {
                        if (type == "0") { // 模板
                            ...
                            // $("#people_phone").text(processInput.converterNullToBlank(data.obj.professionalInfo.phone));
                            // $("#people_landline").text(processInput.converterNullToBlank(data.obj.professionalInfo.landline));

                            $("#people_name").text(processInput.converterNullToBlank(data.obj.professionalInfo.name));
                            $("#people_enterprise").text(processInput.converterNullToBlank(data.obj.professionalInfo.orgName));
                            $("#people_position_type").text(processInput.converterNullToBlank(data.obj.professionalInfo.type));
                            $("#people_status").text(processInput.getPeopleStatus(data.obj.professionalInfo.state));
                            $("#people_phone").text(processInput.converterNullToBlank(data.obj.professionalInfo.phone));
                            $("#people_landline").text(processInput.converterNullToBlank(data.obj.professionalInfo.landline));
                            ...
                            spans.find("span:eq(0)").text(processInput.converterNullToBlank(data.obj.professionalInfo.name));
                            spans.find("span:eq(1)").text(processInput.converterNullToBlank(data.obj.professionalInfo.orgName));
                            spans.find("span:eq(2)").text(processInput.converterNullToBlank(data.obj.professionalInfo.type));
                            spans.find("span:eq(3)").text(processInput.getPeopleStatus(data.obj.professionalInfo.state));
                            spans.find("span:eq(4)").text(processInput.converterNullToBlank(data.obj.professionalInfo.phone));
                            spans.find("span:eq(5)").text(processInput.converterNullToBlank(data.obj.professionalInfo.landline));
                        }
                    }
                },
                error: function () {
        ...
        check_sim: function () {
            ...
            if (processInput.checkIsEmpty(elementId, errorMsg1)
                && processInput.checkLength(elementId, maxLength, errorMsg2)
                && processInput.checkIsLegal(elementId, reg, null, '请输入数字字母，范围：7-20位')
        ...
        checkRightSim: function () {
            ...
            var reg1 = /^((\d{3,4}-\d{7,9})|([1-9]{1}\d{6,12}))$/g;*/
            var reg = /^[0-9a-zA-Z]{7,20}$/g;
            if (processInput.checkIsLegal("sims", reg, null, '请输入数字字母，范围：7~20位')) {
