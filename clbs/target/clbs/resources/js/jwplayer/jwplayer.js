"undefined" == typeof jwplayer && (jwplayer = function(h) {
    ...
    e.isIOS = function(f) {
        return f ? a(RegExp("iP(hone|ad|od).+\\sOS\\s" + f, "i")) : a(/iP(hone|ad|od)/i)
    };
    ...
    e.translateEventResponse = function(f, a) {
        ...
        if (f == h.events.JWPLAYER_FULLSCREEN && !k.fullscreen) k.fullscreen = "true" === k.message, delete k.message;
            ...
            delete k.data;
            k = e.extend(k, m)
        } else "object" == typeof k.metadata && e.deepReplaceKeyName(k.metadata, ["__dot__", "__spc__", "__dsh__", "__default__"], [".", " ", "-", "default"]);
    ...
    e.flashVersion = function() {
        ...
        if ("undefined" != typeof g.ActiveXObject) try {
            if (a = new g.ActiveXObject("ShockwaveFlash.ShockwaveFlash")) return parseInt(a.GetVariable("$version").split(" ")[1].split(",")[0], 10)
        } catch (m) {}
        return 0
    };
        ...
        switch (h.utils.typeOf(f)) {
            ...
            case "object":
                e.foreach(f, function(m, b) {
                    ...
                    for (var d = m, c = 0; c < e.length; c++) d = d.replace(RegExp(a[c], "g"), k[c]);
    ...
    e.ajax = function(f, a, k, m) {
        ...
        if (f && 0 <= f.indexOf("://") && f.split("/")[2] != g.location.href.split("/")[2] && e.exists(g.XDomainRequest)) b = new g.XDomainRequest, b.onload = l(b, f, a, k, m), b.ontimeout = b.onprogress = function() {}, b.timeout = 5E3;
    ...
    e.filterPlaylist = function(a, b, k) {
        var m = [],
            d, c, g, j;
        for (d = 0; d < a.length; d++) if (c = e.extend({}, a[d]), c.sources = e.filterSources(c.sources, !1, k), 0 < c.sources.length) {
    ...
    var q = b.css = function(b, m, f) {
        ...
        if (h) {
            if (!e[b]) {
                m = n && n.sheet && n.sheet.cssRules && n.sheet.cssRules.length || 0;
                if (!n || 5E4 < m) n = c();
                e[b] = n
            }
    ...
    q.unblock = function(a) {
        if (d && (!a || d.id === a)) {
            ...
            for (a = 0; a < d.elements.length; a++) b = d.elements[a], j(b, b.__cssRules);
            d = null
        }
    };
    ...
    h.touch = function(j) {
        function g(d) {
            d.type == l ? (a = !0, f = e(k.DRAG_START, d)) : d.type == c ? a && (q || (b(k.DRAG_START, d, f), q = !0), b(k.DRAG, d)) : (a && (q ? b(k.DRAG_END, d) : (d.cancelBubble = !0, b(k.TAP, d))), a = q = !1, f = null)
        }
        function b(a, b, c) {
    ...
    c.decrypt = function(g, b) {
        if (0 == g.length) return "";
        for (var e = c.strToLongs(l.decode(g)), h = c.strToLongs(j.encode(b).slice(0, 16)), a = e.length, d = e[a - 1], f = e[0], q, k = 2654435769 * Math.floor(6 + 52 / a); 0 != k;) {
            q = k >>> 2 & 3;
    ...
    c.longsToStr = function(c) {
        for (var b = Array(c.length), e = 0; e < c.length; e++) b[e] = String.fromCharCode(c[e] & 255, c[e] >>> 8 & 255, c[e] >>> 16 & 255, c[e] >>> 24 & 255);
        return b.join("")
    };
    var l = {
            ...
            encode: function(c, b) {
                ...
                for (k = 0; k < m.length; k += 3) e = m.charCodeAt(k), h = m.charCodeAt(k + 1), a = m.charCodeAt(k + 2), d = e << 16 | h << 8 | a, e = d >> 18 & 63, h = d >> 12 & 63, a = d >> 6 & 63, d &= 63, f[k / 3] = r.charAt(e) + r.charAt(h) + r.charAt(a) + r.charAt(d);
                f = f.join("");
            ...
            if (b) for (var f = 0; f < b.length; f++) {
                ...
                if (e) {
                    ...
                    try {
                        ...
                        c.log('Error handling "' + d + '" event listener [' + f + "]: " + g.toString(), e.listener, a)
                    }
                }
            }
        }
    ...
    h.plugin = function(j) {
        ...
        c.extend(this, k);
        this.load = function() {
            if (n == c.loaderstatus.NEW) if (0 < j.lastIndexOf(".swf")) a = j, n = c.loaderstatus.COMPLETE, k.sendEvent(l.COMPLETE);
    ...
    h.parsers = {
        ...
        textContent: function(c) {
            return c ? c.textContent ? h.utils.trim(c.textContent) : c.text ? h.utils.trim(c.text) : "" : ""
        },
    ...
    (c.jwparser = function() {}).parseEntry = function(l, j) {
        ...
        if (b.length) {
            j.tracks = [];
            for (n = 0; n < b.length; n++) 0 < b[n].file.length && (b[n]["default"] = "true" == b[n]["default"] ? !0 : !1, b[n].kind = !b[n].kind.length ? "captions" : b[n].kind, b[n].label.length || delete b[n].label, j.tracks.push(b[n]))
        }
        return j
    }
}(jwplayer), function(h) {
    ...
    e.parseGroup = function(h, a) {
        ...
        for (f = 0; f < b(h); f++) if (d = h.childNodes[f], "media" == d.prefix && j(d)) switch (j(d).toLowerCase()) {
            case "content":
                ...
                l(d, "url") && (a.sources || (a.sources = []), a.sources.push({
                    file: l(d, "url"),
                    type: l(d, "type"),
                    width: l(d, "width"),
                    label: l(d, "label")
    ...
    h.playlist = function(c) {
        var l = [];
        if ("array" == h.utils.typeOf(c)) for (var j = 0; j < c.length; j++) l.push(new h.playlist.item(c[j]));
    ...
    c.adprovider = function(j, g) {
        ...
        d.updateModel = function(d, e) {
            ...
            d.tag && (f.tag && d.tag !== f.tag) && (c.error("ad messages not received in order. new model:", d, "old model:", f), b());
            h.utils.extend(f, d);
            a.updateModel(d, e)
        };
        ...
        d.getAdMessage = function() {
            ...
            f.podMessage && 1 < f.podcount && (a = f.podMessage.replace(/__AD_POD_CURRENT__/g, "" + f.sequence).replace(/__AD_POD_LENGTH__/g, "" + f.podcount) + a);
            return a
        };
        d.skipAd = function(a) {
            e("skipAd", {
    ...
    j.controller = function(a, d) {
        ...
        function H(c) {
            if ("state" === c.type) {
                ...
                null === y && (y = new j.adprovider(j.NS, w), y.init(), d.setVideo(y));
    ...
    j.provider = function(h) {
        ...
        d.updateModel = function(a, c) {
            a.newstate && (q.newstate = a.newstate, q.oldstate = a.oldstate || q.oldstate, d.sendEvent(b.JWPLAYER_PLAYER_STATE, {
                oldstate: q.oldstate,
                newstate: q.newstate
            }));
            if ("ad" !== c) {
    ...
    var j = h.utils,
        ...
        a = h.embed = function(d) {
            function f() {
                if (!B) if ("array" === j.typeOf(u.playlist) && 2 > u.playlist.length && (0 === u.playlist.length || !u.playlist[0].sources || 0 === u.playlist[0].sources.length)) m();
                else if (!F) if ("string" === j.typeOf(u.playlist)) C = new h.playlist.loader, C.addEventListener(g.JWPLAYER_PLAYLIST_LOADED, function(a) {
    ...
    (h.embed.config = function(b) {
        ...
        } - 1 == b.width.toString().indexOf("%") ? delete b.aspectratio : e ? b.aspectratio = e : delete b.aspectratio;
        return b
        ...
        switch (u) {
            ...
            case "enterprise":
                if (d.sitecatalyst) try {
                    window.s && window.s.hasOwnProperty("Media") ? new h.embed.sitecatalyst(g) : b()
                } catch (p) {
                    b()
                }
            case "premium":
    ...
    h.api = function(l) {
        ...
        p.play = function(a) {
            ...
            a = p.getState();
            var d = B && B.getState();
            d ? d === b.IDLE || d === b.PLAYING || d === b.BUFFERING ? t("jwInstreamPause") : t("jwInstreamPlay") : a == b.PLAYING || a == b.BUFFERING ? t("jwPause") : t("jwPlay");
            return p
        };
        p.pause = function(a) {
    ...
    h.api.destroyPlayer = function(a) {
        ...
        j.clearCss("#" + d.id);
        if (f = e.getElementById(d.id + ("flash" == d.renderingMode ? "_wrapper" : ""))) {
    ...
    h.api.instream = function(g, b) {
        ...
        function h(a, b) {
            q[a] || (q[a] = [], e(c.JWPLAYER_PLAYER_STATE, function(b) {
                var c = b.newstate,
                    d = b.oldstate;
                if (c == a) {
                    var e = q[c];
