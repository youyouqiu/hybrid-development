(function (window, $) {
    ...
    locationQualifiedRateReport = {
        ...
        startDay: function (day) {
            ...
            if (startValue == "" || endValue == "") {
                ...
                tMonth = locationQualifiedRateReport.doHandleMonth(tMonth + 1);
                tDate = locationQualifiedRateReport.doHandleMonth(tDate);
                var num = -(day + 1);
                ...
                endMonth = locationQualifiedRateReport.doHandleMonth(endMonth + 1);
                endDate = locationQualifiedRateReport.doHandleMonth(endDate);
                ...
                vMonth = locationQualifiedRateReport.doHandleMonth(vMonth + 1);
                vDate = locationQualifiedRateReport.doHandleMonth(vDate);
                ...
                if (day == 1) {
                    ...
                    vendMonth = locationQualifiedRateReport.doHandleMonth(vendMonth + 1);
                    vendDate = locationQualifiedRateReport.doHandleMonth(vendDate);
        ...
        inquireClick: function (number) {
            if (number == 0) {
                locationQualifiedRateReport.getsTheCurrentTime();
            } else if (number == -1) {
                locationQualifiedRateReport.startDay(-1)
            } else if (number == -3) {
                locationQualifiedRateReport.startDay(-3)
            } else if (number == -7) {
                locationQualifiedRateReport.startDay(-7)
            }
            if (number != 1) {
            ...
            locationQualifiedRateReport.getCheckedNodes();
            if (!locationQualifiedRateReport.validates()) {
                return;
            }
            var url = "/clbs/sx/sxReportManagement/locationQualifiedRateReport/list";
            var parameter = {"band": vehicleId, "startTime": startTime, "endTime": endTime};
            json_ajax("POST", url, "json", true, parameter, locationQualifiedRateReport.getCallback);
        },
        exportAlarm: function () {
            if(getRecordsNum() > 60000){
                return layer.msg("导出数据量过大，请缩小查询范围再试（单次导出最多支持6万条）！");
            }
            var url = "/clbs/sx/sxReportManagement/locationQualifiedRateReport/export";
        ...
        zTreeOnAsyncSuccess: function (event, treeId, treeNode, msg) {
            ...
            if (size <= 100 && ifAllCheck) {
                ...
                locationQualifiedRateReport.getCharSelect(treeObj);
            }
            bflag = false;
        },
        ...
        refreshTableClick: function () {
            ...
            //字段搜索重置
            //刷新表格
            locationQualifiedRateReport.inquireClick(1, false);
        },
        onCheckVehicle: function (e, treeId, treeNode) {
            ...
            if (treeNode.checked) {
                ...
                setTimeout(() => {
                    locationQualifiedRateReport.getCheckedNodes();
                    locationQualifiedRateReport.validates();
                }, 600);
            }
            crrentSubV = [];
            crrentSubV.push(treeNode.id);
            locationQualifiedRateReport.getCharSelect(zTree);
            locationQualifiedRateReport.getCheckedNodes();
        },
            ...
            if (nodes != null && nodes != undefined && nodes.length > 0) {
                for (var i = 0; i < nodes.length; i++) {
                    ...
                    if (node.type == "assignment") {
                        ...
                        locationQualifiedRateReport.getGroupChild(node.children, assign);
                    }
                }
            }
        },
                    ...
                    for (var i = 0; i < flowReportList.length; i++) {
                        ...
                        var dateList =
                            [
                                ...
                                flowReportList[i].unqualifiedCount,
                                flowReportList[i].totalCount,
                                flowReportList[i].qualifiedRate

                            ];
                        dataListArray.push(dateList);
                    }

                    locationQualifiedRateReport.reloadData(dataListArray);
        ...
        showMenu: function (e) {
            ...
            $("body").bind("mousedown", locationQualifiedRateReport.onBodyDown);
        },
        hideMenu: function () {
            $("#menuContent").fadeOut("fast");
            $("body").unbind("mousedown", locationQualifiedRateReport.onBodyDown);
        },
        onBodyDown: function (event) {
            if (!(event.target.id == "menuBtn" || event.target.id == "menuContent" || $(event.target).parents("#menuContent").length > 0)) {
                locationQualifiedRateReport.hideMenu();
            }
        },
        //模糊查询树
        searchVehicleTree: function (param) {
            ...
            if (param == null || param == undefined || param == '') {
                bflag = true;
                locationQualifiedRateReport.treeInit()
            } else {
                bflag = true;
                var querySetting = {
                    async: {
                        ...
                        dataFilter: locationQualifiedRateReport.ajaxQueryDataFilter
                    },
                    check: {
                        enable: true,
                        chkStyle: "checkbox",
                    ...
                    callback: {
                        beforeClick: locationQualifiedRateReport.beforeClickVehicle,
                        onCheck: locationQualifiedRateReport.onCheckVehicle,
                        onExpand: locationQualifiedRateReport.zTreeOnExpand,
                        onNodeCreated: locationQualifiedRateReport.zTreeOnNodeCreated
                    }
                };
        ...
        treeInit: function () {
            ...
            var setting = {
                async: {
                    url: locationQualifiedRateReport.getTreeUrl,
                    type: "post",
                    enable: true,
                    autoParam: ["id"],
                    dataType: "json",
                    otherParam: {"type": "multiple", "icoType": "0"},
                    dataFilter: locationQualifiedRateReport.ajaxDataFilter
                },
                check: {
                    enable: true,
                    chkStyle: "checkbox",
                ...
                callback: {
                    beforeClick: locationQualifiedRateReport.beforeClickVehicle,
                    onCheck: locationQualifiedRateReport.onCheckVehicle,
                    beforeCheck: locationQualifiedRateReport.zTreeBeforeCheck,
                    onExpand: locationQualifiedRateReport.zTreeOnExpand,
                    onAsyncSuccess: locationQualifiedRateReport.zTreeOnAsyncSuccess,
                    onNodeCreated: locationQualifiedRateReport.zTreeOnNodeCreated
                }
            };
    ...
    $(function () {
        ...
        //当前时间
        $('input').inputClear();
        $("#refreshTable").bind("click", locationQualifiedRateReport.refreshTableClick);
        locationQualifiedRateReport.getsTheCurrentTime();
        locationQualifiedRateReport.getTable('#dataTable');
        $('#timeInterval').dateRangePicker({
            dateLimit: 31
        });
        $("#groupSelectSpan,#groupSelect").bind("click", locationQualifiedRateReport.showMenu); //组织下拉显示
        $('input').inputClear().on('onClearEvent', function (e, data) {
            var id = data.id;
            if (id == 'groupSelect') {
                var param = $("#groupSelect").val();
                locationQualifiedRateReport.searchVehicleTree(param);
            }
            ;
        });
        /**
        ...
        $("#groupSelect").on('input propertychange', function (value) {
            ...
            inputChange = setTimeout(function () {
                if (isSearch) {
                    ...
                    locationQualifiedRateReport.searchVehicleTree(param);
                    setTimeout(function () {
                        locationQualifiedRateReport.showMenu();
                    }, 300);
                }
                isSearch = true;
            }, 500);
        ...
        //导出
        $("#exportAlarm").bind("click", locationQualifiedRateReport.exportAlarm);
