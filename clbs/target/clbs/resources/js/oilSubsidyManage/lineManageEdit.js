(function ($, window) {
    ...
    var assignmentAdd = {
        ...
        init: function () {
            ...
            resultBackData = formDataList
            console.log(formDataList, 'formDataListformDataList')
            if (formDataList) {
                ...
                $("input[name='id']").val(formDataList.id)
        ...
        doSubmit: function () {
            if (submissionFlag) { // 防止重复提交
                ...
                if (assignmentAdd.validates()) {
                    ...
                    console.log(stationUp, 'stationUpstationUp')
                    console.log(stationDown, 'stationDownstationDown')
                    ...
                    directionData[1]['id'] = resultBackData.direction[1].id
                    ...
                    directionData[0]['id'] = resultBackData.direction[0].id
                    ...
                    var url = "/clbs/m/line/manage/update";
                    var data = {
        ...
        renderTable: function () {
            console.log(stationTable, 'stationTable下行初始化')
            let str = "<option style='display: none'></option>";
            for (var i = 0; i < resultStation.length; i++) {
                str += '<option  value="' + resultStation[i].id + '">' + resultStation[i].name + '(' + resultStation[i].number + ')' + '</option>'
            }
            ...
            for (let i = 0; i < stationTable.length; i++) {
                ...
                <div class="station-line">
                <span class="add-station-line"  id=${'add-station-manage-line' + stationTable[i].order}>添加行</span>
                ${i>0?`<span class="delete-station-line" id="delete-station-manage-line${stationTable[i].order}">删除</span>`:''}
                </div>
            ...
            for (let i = 0; i < stationTable.length; i++) {
                ...
                $(`#delete-station-manage-line${i+2}`).on('click', assignmentAdd.deleteLine)
        ...
        renderUpTable: function () {
            // console.log(stationTableUp, 'stationTableUpstationTableUpstationTableUp上行初始化')
            let data = ''
            let str = "<option style='display: none'></option>";
            for (var i = 0; i < resultStation.length; i++) {
                str += '<option  value="' + resultStation[i].id + '">' + resultStation[i].name + '(' + resultStation[i].number + ')' + '</option>'
            ...
            <div class="station-line">
            <span class="add-station-line"  id=${'add-up-station-manage-line' + stationTable[i].order}>添加行</span>
            ${i>0?`<span class="delete-station-line" id="delete-station-manage-line-up${stationTableUp[i].order}"+>删除</span>`:''}
            </div>
        ...
        `
            ...
            for (let i = 0; i < stationTableUp.length; i++) {
                ...
                $(`#delete-station-manage-line-up${i+2}`).on('click', assignmentAdd.deleteUpLine)
        ...
        deleteUpLine: function () {
            ...
            // console.log(stationTableUp, 'stationTableUpstationTableUp')
            for (let i = 0; i < stationTableUp.length; i++) {
                $(`#station-line-manage-table-up .station-body:nth-child(${i+2})>.orderNumber`).text(i + 1)
            }
        },
