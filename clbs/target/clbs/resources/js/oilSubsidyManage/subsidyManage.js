(function (window, $) {
    ...
    locationInformation = {
        init: function () {
            ...
            var setting = {
                ...
                callback: {
                    ...
                    beforeCheck: locationInformation.beforeCheckVehicle,
                    onDblClick: locationInformation.zTreeOnDblClick,
                }
            };
        ...
        beforeCheckVehicle: function (treeId, treeNode) {
            var flag = true;
        ...
        bcClick: function () {
            var timeInterval = $('#timeInterval').val().split('--');
            if (!forwardOrgIds) {
                layer.msg('请选择组织', {move: false});
                return;
            }
            var timeDifference = locationInformation.datedifference(timeInterval[0], timeInterval[1]);
            if (timeDifference > 31) {
                layer.msg('最大支持31天的数据补传', {move: false});
                return;
            }
        ...
        bcCloseClick: function () {
            ...
            $('#bcStartClick').prop('disabled', false);
            $('#bcStartClick').css('background', 'rgb(22,155,213)');
            listId = []; // 关闭弹窗清空车辆id
        },
        bcStartClick: function () {
            $('#bcStartClick').prop('disabled', true);
            $('#bcStartClick').css('background', '#9D9D9D');
            for (var i = 0; i < listId.length; i++) {
                $(`#sc_${listId[i]}`).attr("disabled", true).css("pointer-events", "none");
                $(`#sc_${listId[i]}`).text('');
            }
            var timeInterval = $('#timeInterval').val().split('--');
            var params = {
                ...
                if (data.obj != null && data.obj.length != 0) {
                    ...
                    for (var i = 0; i < bcTableList.length; i++) {
                        ...
                        dataList = [
                            ...
                            '<a id="sc_' + bcTableList[i].vehicleId + '" href="javascript:void(0);" onClick="locationInformation.delTableList(\'' + bcTableList[i].vehicleId + '\')">不传</a>',
                        ];
        ...
        delTableList: function (id) {
            $(`#bcjd_${id}`).text('不传');
            $(`#sc_${id}`).attr("disabled", true).css("pointer-events", "none");
            $(`#sc_${id}`).css('color', '#9D9D9D');
            for (var i = 0; i < listId.length; i++) {
                if (listId[i] == id) {
                    listId.splice(i, 1);
                }
    ...
    $(function () {
        ...
        $('#closeBox').on('click', function () {
            locationInformation.bcCloseClick();
            $('#bcModelBox').css('visibility', 'hidden');
        });
    })
})(window, $);