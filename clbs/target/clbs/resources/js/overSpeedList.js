(function (window, $) {
    ...
    overSpeedGroup = {
        ...
        init: function () {
            ...
            var setting = {
                async: {
                    url: overSpeedGroup.getTreeUrl,
                    type: "post",
                    enable: true,
                    autoParam: ["id"],
                    dataType: "json",
                    otherParam: {"type": "multiple", "icoType": "0"},
                    dataFilter: overSpeedGroup.ajaxDataFilter
                },
                check: {
                    enable: true,
                    chkStyle: "radio",
                ...
                callback: {
                    beforeClick: overSpeedGroup.beforeClickVehicle,
                    onAsyncSuccess: overSpeedGroup.zTreeOnAsyncSuccess,
                    beforeCheck: overSpeedGroup.zTreeBeforeCheck,
                    onCheck: overSpeedGroup.onCheckVehicle,
                    onNodeCreated: overSpeedGroup.zTreeOnNodeCreated,
                    onExpand: overSpeedGroup.zTreeOnExpand
                }
            };
        ...
        zTreeOnAsyncSuccess: function (event, treeId, treeNode, msg) {
            ...
            overSpeedGroup.getCharSelect(treeObj);
        },
        ...
        onCheckVehicle: function (e, treeId, treeNode) {
            ...
            if (treeNode.checked) {
                ...
                setTimeout(() => {
                    overSpeedGroup.getCheckedNodes("treeDemo");
                    overSpeedGroup.validates();
                }, 600);
            }
            overSpeedGroup.getCheckedNodes("treeDemo");
            overSpeedGroup.getCharSelect(zTree);
        ...
        initTable: function () {
            ...
            //表格setting
            var setting = {
                listUrl: "/clbs/cb/cbReportManagement/speedingStatistics/group/speedingStatisticsList",
                ...
                ajaxCallBack: overSpeedGroup.drawTableCallbackFun,
                pageable: true, //是否分页
        ...
        inquireClick: function (number) {
            if (number != 1) {
                overSpeedGroup.setNewDay(number);
            }
            overSpeedGroup.getCheckedNodes('treeDemo');
            if (!overSpeedGroup.validates()) {
                return;
            }

            overSpeedGroup.initTable();
            var time = $('#month').val();
            var ajaxDataParam = {
            ...
            var url = '/clbs/cb/cbReportManagement/speedingStatistics/group/graphical';
            json_ajax("post", url, "json", true, ajaxDataParam, function (result) {
                if (result.success) {
                    $('#stretch3-body').slideDown();
                    overSpeedGroup.chartRender(result, 'accountedEchart', 'trendEchart');
                } else {
                    var result = {
                        obj: {
                            data: {
        ...
        chartRender: function (result, accountedElement, trendElement) {
            ...
            var accountedData = [
                {value: data.totalOver50PerCentNum, name: '50%(含)以上'},
                {value: data.totalCent20To50PerNum, name: '20%(含)-50%'},
                {value: data.totalLess20Per, name: '20%以下'},
            ];
            overSpeedGroup.accountedEchart(accountedData, accountedElement);
            ...
            var echartData = {
                ...
                'totalNum': [],
                'monitorNum': [],
                'totalOver50PerCentNum': [],
                'totalCent20To50PerNum': [],
                'totalLess20Per': [],
                'sequentialData': [],
                'comparedData': [],
            };
            for (var i = 0; i < statisticInfo.length; i++) {
                ...
                echartData.totalOver50PerCentNum.push(item.over50PerCentNum ? item.over50PerCentNum : 0);
                echartData.totalCent20To50PerNum.push(item.cent20To50PerNum ? item.cent20To50PerNum : 0);
                echartData.totalLess20Per.push(item.less20PerNum ? item.less20PerNum : 0);
                echartData.sequentialData.push(item.ringRatio);
                echartData.comparedData.push(item.yearRate);
            }

            overSpeedGroup.trendEchart(echartData, trendElement);
        },
        /**
         * 超速车辆占比情况饼图
         * @param data 饼图数据
        ...
        trendEchart: function (echartData, element) {
            ...
            var option = {
                ...
                legend: {
                    ...
                    right: 10,
                    type: 'scroll',
                    data: ['50%(含)以上', '20%(含)-50%', '20%以下', '环比率', '同比率']
                },
                dataZoom: [{// 初始化滚动条
                    type: 'slider',
                    show: true,
                ...
                series: [
                    {
                        ...
                        barWidth: 10,
                        barGap: '30%',
                        data: echartData.totalOver50PerCentNum,
                    }, {
                        name: '20%(含)-50%',
                        type: 'bar',
                        barWidth: 10,
                        barGap: '30%',
                        data: echartData.totalCent20To50PerNum,
                    }, {
                        name: '20%以下',
                        type: 'bar',
                        barWidth: 10,
                        barGap: '30%',
                        data: echartData.totalLess20Per,
                    },
                    {
                        name: '环比率',
                        type: 'line',
        ...
        trClickFun: function (e) {
            ...
            if (_this.hasClass('active')) {
                ...
                var time = $('#month').val().replace('-', '');
                curSelectTime = time;
                overSpeedGroup.getOrgDetailInfo();
                overSpeedGroup.initDetailTable();
            }
        ...
        getOrgDetailInfo: function () {
            var ajaxDataParam = {
                "groupId": curSelectOrgId,
                "time": curSelectTime
            };
            var infoUrl = '/clbs/cb/cbReportManagement/speedingStatistics/group/rankInfo';
            json_ajax("post", infoUrl, "json", true, ajaxDataParam, function (result) {
                if (result.success) {
                    var data = result.records[0] ? result.records[0] : {};
                    // 详情抽屉字段赋值
                    ...
                    $('#over50PerCentNum').html(data.over50PerCentNum === undefined ? noStr : data.over50PerCentNum);
                    $('#cent20To50PerNum').html(data.cent20To50PerNum === undefined ? noStr : data.cent20To50PerNum);
                    $('#less20PerNum').html(data.less20PerNum === undefined ? noStr : data.less20PerNum);
            ...
            json_ajax("post", url, "json", true, ajaxDataParam, function (result) {
                if (result.success) {
                    overSpeedGroup.chartRender(result, 'accountedDetailEchart', 'trendDetailEchart');
                } else {
                    var result = {
                        obj: {
                            data: {
        ...
        left_arrow: function () {
            ...
            var newDate = dateYear + '-' + overSpeedGroup.appendZero(dateMonth);
        ...
        export: function () {
            ...
            var url = "/clbs/cb/cbReportManagement/speedingStatistics/group/exportOrgListData";
        ...
        detailExport: function () {
            ...
            var url = "/clbs/cb/cbReportManagement/speedingStatistics/group/exportOrgSpeedDetailsData";
            json_ajax("post", url, "json", true, paramer, function (result) {
                if (result.success) {
                    layer.confirm(exportTitle, {
                        title: '操作确认',
        ...
        monthDataInit: function () {
            ...
            laydate.render({
                ...
                change: function (value, dates, edate) {
                    ...
                    // 获取当前实际的年月
                    var getNow = overSpeedGroup.getNow;
                    var myDate = new Date();
                    var nowYear = myDate.getFullYear();
                    var nowMonth = myDate.getMonth() + 1;

                    if (year > nowYear) {
                        final = overSpeedGroup.getYesterMonth(1);
    ...
    $(function () {
         ...
         */
        overSpeedGroup.init();
        overSpeedGroup.getTable('#dataTable');
        $("#groupSelect").bind("click", showMenuContent);
        $("#refreshTable").bind("click", overSpeedGroup.refreshTable);
        $('#exportAlarm').on('click', overSpeedGroup.export);
        $('#detailExport').on('click', overSpeedGroup.detailExport);
        ...
        // 模糊查询
        $('#detailSimpleQueryParam').keyup(function (event) {
            if (event.keyCode == 13) {
        ...
        // 点击页面其他地方,关闭详情抽屉
        $(document).on('click', overSpeedGroup.closeDetail);
