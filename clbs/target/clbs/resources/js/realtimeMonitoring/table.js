...
var Table = function (options, dependency) {
    ...
    this.defaultStatusColumns = window.defaultStatusColumns;
    this.defaultAlarmColumns = window.defaultAlarmColumns;
    this.defaultRunColumns = window.defaultRunColumns;
    this.defaultStopColumns = window.defaultStopColumns;
    this.defaultOBDColumns = window.defaultOBDColumns;
    this.statusColumns = [];
    this.alarmColumns = [];
    this.obdColumns = [];
    this.stopColumns = [];
...
Table.prototype.initSetting = function (isUpdate, callback) {
    $.ajax({
        ...
        dataType: "json",
        async: true,
        data: {marks: this.columnKeys.join(',')},
        success: function (data) {
            ...
            var runColumns = [$.extend({}, window.defaultRunColumns[0])];
            var stopColumns = [$.extend({}, window.defaultStopColumns[0])];
            if (data.success && data.obj) {
                ...
                runColumns.push(window.defaultRunColumns[window.defaultRunColumns.length - 1]);
                stopColumns.push(window.defaultStopColumns[window.defaultStopColumns.length - 1]);
            }
...
Table.prototype.replaceAlarmTable = function () {
    ...
    this.tableAlarm.replaceOptionData(tableData);
}
...
Table.prototype.replaceRunTable = function () {
    ...
    if (runFragmentData !== null) {
        this.tableRun.replaceOptionData(runFragmentData);
    }else{
        this.tableRun.replaceOptionData([]);
    }
}
...
Table.prototype.replaceStopTable = function () {
    ...
    if (stopFragmentData !== null) {
        this.tableStop.replaceOptionData(stopFragmentData);
    }else{
        this.tableStop.replaceOptionData([]);
    }
}
...
Table.prototype.replaceObdTable = function () {
    ...
    if (obdData !== null) {
        ...
        this.tableObd.replaceOptionData(tableData);
        this.setActiveRow();
    }else{
        this.tableObd.replaceOptionData([]);
    }
}
...
Table.prototype.replaceTable = function () {
    ...
    if (positions === null){
        ...
        this.tableStatus.replaceOptionData([]);
    }else{
        ...
        this.tableStatus.replaceOptionData(tableData);
        this.setActiveRow();
...
Table.prototype.getRowId = function (item) {
    return item.plateNumber + ' ' + item.vtime;
}
...
Table.prototype.getRestTableData = function () {
    ...
    var sensorFlag = sensorArray[dataDependency.getValidClickedIndex()];
