var scheduledAttendanceReport;
var myTable, myTable2, myTable3;

(function (window, $) {
  var peopleMap = null,// 选择人员组织树数据集合
    monitorIds = [],// 选择人员id
    peopleTreeName = [],// 选择人员名称
    monitorId = '';// 具体明细monitorId
  ...
  scheduledAttendanceReport = {
    init: function () {
      $('input').inputClear();
      scheduledAttendanceReport.getSchedule();
    },
    //排班下拉
    getSchedule: function () {
      json_ajax('post', '/clbs/talkback/reportManagement/scheduledAttendanceReport/getScheduledList',
        'json', true, {}, scheduledAttendanceReport.scheduleCB);
    },
    scheduleCB: function (data) {
      if (data.success) {
        ...
        if (obj.length > 0) {
          scheduledAttendanceReport.getPeople(initId || obj[0].id);
        }
        scheduledAttendanceReport.timerInit();
      } else {
        layer.msg(data.msg);
      }
    },
    ...
    selectFun: function () {
      ...
      peopleTreeName = [];
      $('#groupSelect').val('');
      scheduledAttendanceReport.getPeople(value);
    },
    //获取人员
    getPeople: function (monitorId) {
      ...
      json_ajax('post', '/clbs/talkback/reportManagement/scheduledAttendanceReport/findMonitoringObject',
        'json', true, paramer, scheduledAttendanceReport.getPeopleCB);
    },
    getPeopleCB: function (data) {
      if (data.success && data.obj) {
        peopleMap = data.obj;
        scheduledAttendanceReport.setPeopleTree();
      } else {
        layer.msg(data.msg);
      }
    },
    setPeopleTree: function () {
      ...
      var setting = {
        ...
        callback: {
          onCheck: scheduledAttendanceReport.onCheckPeople,
          beforeClick: scheduledAttendanceReport.beforeClickPeople
        }
      };
    ...
    inquireClick: function () {
      if (!scheduledAttendanceReport.validates()) {
        return;
      }
      ...
      //tab重置
      scheduledAttendanceReport.setTab(0);
      ...
      json_ajax('post', '/clbs/talkback/reportManagement/scheduledAttendanceReport/list',
          'json', true, paramer, scheduledAttendanceReport.inquireClickCB);
    },
    inquireClickCB: function (data) {
      if (data.success) {
        scheduledAttendanceReport.initTable();
        scheduledAttendanceReport.initTable2();
        scheduledAttendanceReport.getChartsData();
      }
    },
    getChartsData: function () {
      json_ajax('post', '/clbs/talkback/reportManagement/scheduledAttendanceReport/getAllSummary',
        'json', true, {}, function (data) {
          ...
          // scheduledAttendanceReport.sortRecords(records);
          //图形图标数据组装
          for (var i = 0; i < records.length; i++) {
            var item = records[i];
            var attendance = Number(item.attendance);
            ...
            yArr.push(attendance);
            sum += attendance;
          }
          scheduledAttendanceReport.setCharts();
          ...
          if (yArr.length > 0) {
            ...
            $('#max').text(parseFloat(max) + '%');
            $('#min').text(parseFloat(min) + '%');
            $('#average').text(parseFloat(avg) + '%');
          }
        });
    },
    //报表
    initTable: function () {
      ...
      var columns = [
        ...
        },
        {
          ...
          render: function (data, type, row, meta) {
            ...
            result += '<button onclick="scheduledAttendanceReport.detailBtn(\'' + row.monitorId + '\', this)" type="button" class=" editBtn editBtn-info"><i class="fa fa-sun-o"></i>出勤明细</button>&nbsp;';
            return result;
          }
        },
        {
        ...
        },
        {
          ...
          'data': 'shouldWorkDurationStr',
          'class': 'text-center'
        }, {
          'data': 'actualWorkDays',
          'class': 'text-center'
        }, {
          'data': 'actualWorkingDurationStr',
          'class': 'text-center'
        }, {
          'data': 'attendance',
          'class': 'text-center',
      ...
      //表格setting
      var setting = {
        listUrl: '/clbs/talkback/reportManagement/scheduledAttendanceReport/getSummary',
        ...
        ajaxCallBack: function (data) {
          ...
          if (records && records.length > 0) {
            ...
            scheduledAttendanceReport.initTable3();
          }
        }
      };
    ...
    sortRecords: function (data) {
      data.sort(function (a, b) {
        if (Number(a.attendance) < Number(b.attendance)) {
          return 1;
        } else if (Number(a.attendance) > Number(b.attendance)) {
          return -1;
        } else {
          return 0;
        }
    ...
    initTable2: function () {
      ...
      //表格setting
      var setting = {
        listUrl: '/clbs/talkback/reportManagement/scheduledAttendanceReport/getAll',
        columnDefs: columnDefs, //表格列定义
        columns: columns, //表格列
    ...
    initTable3: function () {
      ...
      //表格setting
      var setting = {
        listUrl: '/clbs/talkback/reportManagement/scheduledAttendanceReport/getDetail',
        columnDefs: columnDefs, //表格列定义
        columns: columns, //表格列
    ...
    detailBtn: function (id, self) {
      ...
      scheduledAttendanceReport.attendanceDetail(id, curInx);
      //图表缩放设置
      var start = curInx,
        end = curInx + 19;
    ...
    attendanceDetail: function (id, curInx) {
      monitorId = id;
      scheduledAttendanceReport.initTable3();
      ...
      //tab切换
      scheduledAttendanceReport.setTab(2);
    },
    //图形展示
    setCharts: function () {
      option = {
      ...
      myChart.on('click', function (param) {
        ...
        scheduledAttendanceReport.attendanceDetail(id, inx);
      });
    },
    ...
    export2: function () {
      window.location.href = '/clbs/talkback/reportManagement/scheduledAttendanceReport/exportDetail_' + monitorId;
    },
    // 离职人员勾选 (是否在职 0:离职； 2:在职  1:显示空白)
    changeIncumbency: function () {
      ...
      if (isIncumbency) {//包含离职
        ...
        for (var i = 0; i < incumbency.length; i++) {
          ...
          if (item.checked && monitorIds.indexOf(item.id) === -1) {
            monitorIds.push(item.id);
            peopleTreeName.push(item.name);
          }
        }
        ...
        for (var i = 0; i < incumbency.length; i++) {
          var item = incumbency[i];
          if (item.checked && monitorIds.indexOf(item.id) !== -1) {
            monitorIds.splice(monitorIds.indexOf(item.id), 1);
  ...
  $(function () {
    scheduledAttendanceReport.init();

    //事件
    $('#schedule').on('change', scheduledAttendanceReport.selectFun);
    $('#groupSelect').on('click', showMenuContent);
    $('#isIncumbency').on('change', scheduledAttendanceReport.changeIncumbency);
    $('#inquireClick').on('click', scheduledAttendanceReport.inquireClick);
    $('#stretch2').on('click', scheduledAttendanceReport.setCharts);
    $('.nav-tabs').on('click', 'li', scheduledAttendanceReport.tabChange);
    $('#alarmExport2').on('click', scheduledAttendanceReport.export2);
    //echarts图表resize
    $('#toggle-left-button').on('click', function () {
      setTimeout(function () {
        scheduledAttendanceReport.chartResize();
      }, 500);
    });
    $(window).resize(scheduledAttendanceReport.chartResize);
  });
})(window, $);