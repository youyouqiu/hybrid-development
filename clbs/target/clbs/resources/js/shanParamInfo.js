...
(function ($, window) {
    ...
    //获取参数设置信息
    var sensorIDs = [100, 101, 103];//[前向监测, 驾驶员监测, 盲区]
    var socketTypes = [62308, 62309, 62311];//[前向监测, 驾驶员监测, 盲区]
    //参数下发是的风险事件id
    var arr0 = ['', '', 186401, 186402, 186403, 186404, 186405, 186407, 186409],//前向事件id,''表示公共参数
        arr1 = ['', '', 186513, 186503, 186502, 186508, 186504, 186515],//驾驶员事件id
        arr2 = [186704, ''];  //盲区事件id
    ...
    shanParamInfo = {
        init: function () {
            ...
            for(var i=0;i<socketTypes.length;i++){
                ...
                shanParamInfo.subscribeSocket(socketType);//socket订阅
                shanParamInfo.getPeripheralInfo(sensorID);//传感器参数下发
            }
        },
        /**
         * socket订阅
        ...
        subscribeSocket: function (type) {
            webSocket.subscribe(headers, "/user/topic/per" + type + "Info", function (data) {
                shanParamInfo.getSensor0104Param(data, type);
            }, null, null);
        },
        /**
         * 传感器参数指令下发
        ...
        getPeripheralInfo: function (sensorID) {
            ...
            json_ajax("POST", url, "json", false, {
                ...
                "sensorID": sensorID,
                "commandType": 243
            }, shanParamInfo.setreadConventionalCall);
        },
        setreadConventionalCall: function (data) {
            if (data.success) {
                ...
                shanParamInfo.createSocket0104InfoMonitor(data.msg);
            } else {
                if (data.msg) {
                    layer.msg(data.msg);
                }
        ...
        getSensor0104Param: function (msg, type) {
            ...
            shanParamInfo.setSenorInputData(param, type);
        ...
        setSenorInputData: function (result, type) {
            ...
            for (var key in data) {
                ...
                switch (key) {
                    case 'driveDeedType':
                    case 'cameraStrategy':
                        value = shanParamInfo.getCaptureType(value);
                        dom.val(value);
                        break;
                    case 'eventEnable':
                    case 'alarmEnable':
                        shanParamInfo.getBinaryValue(value, tabId, key);
                        break;
                    case 'cameraResolution':
                        var dataValue = '0x0' + parseInt(value, 10).toString(16);
                        value = shanParamInfo.changeCameraResolution(value);
                        dom.val(value).attr('data-value', dataValue);
                        break;
                    case 'videoResolution':
                        var dataValue = '0x0' + parseInt(value, 10).toString(16);
                        value = shanParamInfo.changeVideoResolution(value);
                        dom.val(value).attr('data-value', dataValue);
                        break;
                    case 'fixedCamera':
                        value = value / 1000;
                    ...
                    case 'unit'://胎压单位
                        ...
                        break;
                    case 'tyreNumber':
                        dom.val(value).attr('data-value',shanParamInfo.tireModel(value));
                        break;
                    default:
                        dom.val(value);
                        break;
        ...
        getPlatformData: function () {
            ...
            for (var i = 0; i < platformData.length; i++) {
                ...
                if (i < 2) {//有公共参数的页签
                    shanParamInfo.getCommonParamSetting(item.commonParamSetting, tab);
                }
                shanParamInfo.getAdasAlarmParamSettings(item.adasAlarmParamSettings, tab);
            }
        },
        //全局参数
            ...
            for (var key in data) {
                ...
                switch (key) {
                    ...
                    case 'cameraResolution'://拍照分辨率
                        ...
                        value = shanParamInfo.changeCameraResolution(num);
                        break;
                    case 'videoResolution'://视频分辨率
                        var num = Number(parseInt(value, 16).toString(10));
                        value = shanParamInfo.changeVideoResolution(num);
                        break;
                    case 'photographTime':
                        value = value / 10;
                        break;
        ...
        getAdasAlarmParamSettings: function (data, tab) {
            for (var i = 0; i < data.length; i++) {
                ...
                switch (riskId) {
                    case 186704: //盲区
                        shanParamInfo.setInputValue(item,riskEvent.eq(0));
                        break;
                    case 186401://前向碰撞
                    case 186513://疲劳驾驶
                        shanParamInfo.setInputValue(item, riskEvent.eq(1));
                        break;
                    case 186402://车道偏离
                    case 186503://抽烟
                        shanParamInfo.setInputValue(item, riskEvent.eq(2));
                        break;
                    case 186403://车距过近
                    case 186502://接打手持电话
                        shanParamInfo.setInputValue(item, riskEvent.eq(3));
                        break;
                    case 186404://行人碰撞
                    case 186508://分神驾驶
                        shanParamInfo.setInputValue(item, riskEvent.eq(4));
                        break;
                    case 186405://频繁变道
                    case 186504://驾驶人行为异常
                        shanParamInfo.setInputValue(item, riskEvent.eq(5));
                        break;
                    case 186407://障碍物
                    case 186515://驾驶员身份识别
                        shanParamInfo.setInputValue(item, riskEvent.eq(6));
                        break;
                    case 186409://道路标识超限
                        shanParamInfo.setInputValue(item, riskEvent.eq(7));
                        break;
                    default:
                        break;
                }
        ...
        paramSendAllClick: function () {
            var self = $(this);
            shanParamInfo.preventclick(self);
            isAll = true;
            var jsonDataArr = [];
            ...
            for (var i = 0; i < riskIds.length; i++) {
                ...
                var jsonData = shanParamInfo.setJsonData(riskIds[i], $('.tab-pane').eq(i));
                jsonDataArr.push(jsonData[0]);
            }
            // console.log('全部下发参数', jsonDataArr);
            shanParamInfo.dsmInfoSend(jsonDataArr);
        },
        paramSendClick: function () {
            ...
            var jsonData = shanParamInfo.setJsonData(arr, tab);
            // console.log('本页签下发参数', jsonData);
            shanParamInfo.dsmInfoSend(jsonData);
        },
        //组装下发参数
        setJsonData: function (arr, tab) {
            ...
            for (var i = 0; i < arr.length; i++) {
                ...
                if (item == '') {//全局参数/主动抓拍策略
                    ...
                    commonParamSetting = shanParamInfo.getCommontData(inputs);
                } else {//风险事件
                    inputs = tab.find('.info-content').eq(i).find('input');
                    adasAlarmParamSettings.push(shanParamInfo.getInputData(item, inputs));
                }
            }
        ...
        dsmInfoSend: function (jsonData) {
            ...
            json_ajax("POST", "/clbs/adas/standard/param/setting.gsp", "json", false, data, shanParamInfo.dsmInfoSendCallback);
        },
        dsmInfoSendCallback: function (data) {
            if (data != null) {
                if (data.success) {
                    ...
                    //获取下发状态
                    setTimeout(function () {
                        timerInterval = setInterval(shanParamInfo.getStatus(), 5000);
                    }, 2000);
                    setTimeout(function () {
            ...
            json_ajax("POST", url, "json", false, {
                ...
                if (data.success) {
                    ...
                    if (obj) {
                        ...
                        for (var key in obj) {
                            var value = obj[key];
                            status.push(shanParamInfo.statusChange(value));
                        }
                        ...
                        timer = setTimeout(function () {
                            ...
                            shanParamInfo.getPeripheralInfo(sensorID);
                        }, 1000);
                    }
    ...
    $(function () {
        ...
        //事件
        $(".info-span").on("click", shanParamInfo.hiddenparameterFn);//点击显示隐藏信息
        $("#dsmInfoSend").on("click", shanParamInfo.paramSendAllClick);//全部修正下发
        $("#dsmInfoSend2").on("click", shanParamInfo.paramSendClick);//本页签修正下发
        $("#dsmInfoRefresh").on("click", shanParamInfo.readConventionalRefreshClick);//刷新
        $(".sensorBtn").on("click", shanParamInfo.sensorBtnClick);//以传感器为准
        $(".platformBtn").on("click", shanParamInfo.platformBtnClick);//以平台设置为准
    })
})($, window)