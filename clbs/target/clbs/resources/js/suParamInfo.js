...
(function ($, window) {
    ...
    var arr0 = ['', '', 156401, 156402, 156403, 156404, 156405, 156407, 156409],//前向事件id,''表示全局参数和主动拍照参数
        arr1 = ['', '', 156513, 156503, 156502, 156505, 156504, 156515];//驾驶员事件id
    ...
    suParamInfo = {
        init: function () {
            ...
            for (var i=0; i<socketTypes.length; i++) {
                ...
                suParamInfo.subscribeSocket(type); //socket订阅
                suParamInfo.getPeripheralInfo(sensorId); //传感器参数下发
            }
        ...
        subscribeSocket: function (type) {
            webSocket.subscribe(headers, "/user/topic/per" + type + "Info", function (data) {
                suParamInfo.getSensor0104Param(data, type);
            }, null, null);
        },
        /**
         * 传感器参数指令下发
        ...
        getPeripheralInfo: function (sensorID) {
            ...
            json_ajax("POST", url, "json", false, {
                ...
                "sensorID": sensorID,
                "commandType": 243
            }, suParamInfo.setreadConventionalCall);
        },
        setreadConventionalCall: function (data) {
            if (data.success) {
                ...
                suParamInfo.createSocket0104InfoMonitor(data.msg);
            } else {
                if (data.msg) {
                    layer.msg(data.msg);
                }
        ...
        getSensor0104Param: function (msg, type) {
            ...
            suParamInfo.setSenorInputData(param, type);
        ...
        setSenorInputData: function (result, type) {
            ...
            for (var key in data) {
                ...
                switch (key) {
                    case 'driveDeedType':
                    case 'cameraStrategy':
                        value = suParamInfo.getCaptureType(value);
                        dom.val(value);
                        break;
                    case 'eventEnable':
                    case 'alarmEnable':
                        suParamInfo.getBinaryValue(value, tabId, key);
                        break;
                    case 'cameraResolution':
                        var dataValue = '0x0' + parseInt(value, 10).toString(16);
                        value = suParamInfo.changeCameraResolution(value);
                        dom.val(value).attr('data-value', dataValue);
                        break;
                    case 'videoResolution':
                        var dataValue = '0x0' + parseInt(value, 10).toString(16);
                        value = suParamInfo.changeVideoResolution(value);
                        dom.val(value).attr('data-value', dataValue);
                        break;
                    case 'fixedCamera':
                        value = value / 1000;
        ...
        getPlatformData: function () {
            ...
            for (var i = 0; i < platformData.length; i++) {
                ...
                suParamInfo.getCommonParamSetting(item.commonParamSetting, tab);
                suParamInfo.getAdasAlarmParamSettings(item.adasAlarmParamSettings, tab);
            }
        },
        //全局参数
            ...
            for (var key in data) {
                ...
                switch (key) {
                    ...
                    case 'cameraResolution'://拍照分辨率
                        ...
                        value = suParamInfo.changeCameraResolution(num);
                        break;
                    case 'videoResolution'://视频分辨率
                        var num = Number(parseInt(value, 16).toString(10));
                        value = suParamInfo.changeVideoResolution(num);
                        break;
                    case 'photographTime':
                        value = value / 10;
                        break;
        ...
        getAdasAlarmParamSettings: function (data, tab) {
            for (var i = 0; i < data.length; i++) {
                ...
                switch (riskId) {
                    case 156401://前向碰撞
                    case 156513://疲劳驾驶
                        suParamInfo.setInputValue(item, riskEvent.eq(1));
                        break;
                    case 156402://车道偏离
                    case 156503://抽烟
                        suParamInfo.setInputValue(item, riskEvent.eq(2));
                        break;
                    case 156403://车距过近
                    case 156502://接打手持电话
                        suParamInfo.setInputValue(item, riskEvent.eq(3));
                        break;
                    case 156404://行人碰撞
                    case 156505://分神驾驶
                        suParamInfo.setInputValue(item, riskEvent.eq(4));
                        break;
                    case 156405://频繁变道
                    case 156504://驾驶员异常行为
                        suParamInfo.setInputValue(item, riskEvent.eq(5));
                        break;
                    case 156407://障碍物
                    case 156515://驾驶员身份识别
                        suParamInfo.setInputValue(item, riskEvent.eq(6));
                        break;
                    case 156409://道路标识超限
                        suParamInfo.setInputValue(item, riskEvent.eq(7));
                        break;
                    default:
                        break;
                }
        ...
        paramSendAllClick: function () {//全部下发
            ...
            for (var i = 0; i < riskIds.length; i++) {
                ...
                var jsonData = suParamInfo.setJsonData(riskIds[i], $('.tab-pane').eq(i));
                jsonDataArr.push(jsonData[0]);
            }

            suParamInfo.dsmInfoSend(jsonDataArr);
        },
        paramSendClick: function () {//本页签下发
            ...
            var jsonData = suParamInfo.setJsonData(arr, tab);
            suParamInfo.dsmInfoSend(jsonData);
        },
        //组装下发参数
        setJsonData: function (arr, tab) {
            ...
            for (var i = 0; i < arr.length; i++) {
                ...
                if (item == '') {//全局参数/主动抓拍策略
                    ...
                    commonParamSetting = suParamInfo.getCommontData(inputs);
                } else {//风险事件
                    inputs = tab.find('.info-content').eq(i).find('input');
                    adasAlarmParamSettings.push(suParamInfo.getInputData(item, inputs));
                }
            }
        ...
        dsmInfoSend: function (jsonData) {
            ...
            json_ajax("POST", "/clbs/adas/standard/param/setting.gsp", "json", false, data, suParamInfo.dsmInfoSendCallback);
        },
        dsmInfoSendCallback: function (data) {
            if (data.success) {
                ...
                //获取下发状态
                setTimeout(function () {
                    timerInterval = setInterval(suParamInfo.getStatus(), 5000);
                }, 2000);
                setTimeout(function () {
            ...
            json_ajax("POST", url, "json", false, {
                ...
                if (data.success) {
                    ...
                    if (obj) {
                        ...
                        for (var key in obj) {
                            var value = obj[key];
                            status.push(suParamInfo.statusChange(value));
                        }
                        ...
                        timer = setTimeout(function () {
                            ...
                            suParamInfo.getPeripheralInfo(sensorID);
                        }, 1000);
                    }
    ...
    $(function () {
        ...
        //事件
        $(".info-span").on("click", suParamInfo.hiddenparameterFn);//点击显示隐藏信息
        $("#dsmInfoSend").on("click", suParamInfo.paramSendAllClick);//全部修正下发
        $("#dsmInfoSend2").on("click", suParamInfo.paramSendClick);//本页签修正下发
        $("#dsmInfoRefresh").on("click", suParamInfo.readConventionalRefreshClick);//刷新
        $(".sensorBtn").on("click", suParamInfo.sensorBtnClick);//以传感器为准
        $(".platformBtn").on("click", suParamInfo.platformBtnClick);//以平台设置为准
    })
})($, window)