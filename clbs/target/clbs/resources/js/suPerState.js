...
(function ($, window) {
    ...
    suPerState = {
        init: function () {
            ...
            for (var i=0; i<socketTypeArr.length; i++) {
                ...
                suPerState.subscribeParam(type);
                suPerState.getDatas(sensorId);
            }
        },
        //websocket订阅
        subscribeParam: function (type) {
            webSocket.subscribe(headers, "/topic/per" + type + '_' + vid + "_Info", function (data) {
                suPerState.getSensor0104Param(data, type);
            }, null, null);
        },
        ...
        queryF3BaseParamCall: function (result, type) {
            ...
            for (var i = 0; i < alarmData.length; i++) {
                ...
                var value = suPerState.getAlarmStatus(item.value);
        ...
        getF3BaseParamCall: function (data) {
            if (!data.success) {
                ...
                suPerState.createSocket0104InfoMonitor();
            }
        },
    ...
    $(function () {
        suPerState.init();
        //刷新
        $("#readPerStateRefresh").on("click", suPerState.readInformationRefreshClick);
    })
})($, window)