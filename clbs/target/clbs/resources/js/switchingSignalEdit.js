(function(window,$){
	...
	editioParSetting= {
		init: function(){
            ...
            json_ajax("POST","/clbs/m/switching/type/addAllowlist","json",true,null,editioParSetting.initTypeCallback);
            ...
            $("#referBrands").bsSuggest({
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if (data.success) {
                            editioParSetting.setVehicleData(data.msg);
                        }else {
                            layer.msg(data.msg);
                        }
                    },
            ...
            editioParSetting.IOTypeChangeFn(0);
            editioParSetting.IOTypeChangeFn(1);
            editioParSetting.IOTypeChangeFn(2);
            editioParSetting.IOTypeChangeFn(3);
        },
        //参照车辆设置
        setVehicleData:function (data) {
            data =  JSON.parse(data);
            $("#sensorType3").val(data.signalThreeName);
            $("#sensorType3").attr("data-id",data.signalThree);
            $("#threeType").find("option[value='"+data.threeType+"']").attr("selected","selected");
            $("#sensorType2").attr("data-id",data.signalTwo);
            $("#sensorType2").val(data.signalTwoName);
            $("#twoType").find("option[value='"+data.twoType+"']").attr("selected","selected");
            $("#sensorType1").attr("data-id",data.signalOne);
            $("#sensorType1").val(data.signalOneName);
            $("#oneType").find("option[value='"+data.oneType+"']").attr("selected","selected");
            $("#sensorType0").val(data.signalZeroName);
            $("#sensorType0").attr("data-id",data.signalZero);
            $("#zeroType").find("option[value='"+data.zeroType+"']").attr("selected","selected");
            editioParSetting.IOTypeChangeFn(0);
            editioParSetting.IOTypeChangeFn(1);
            editioParSetting.IOTypeChangeFn(2);
            editioParSetting.IOTypeChangeFn(3);
		...
		doSubmits: function(){
            ...
            if($("#sensorType3").val()==""&&$("#sensorType2").val()==""&&$("#sensorType1").val()==""&&$("#sensorType0").val()==""){
                editioParSetting.showErrorMsg(signalChannelNull, "sensorType0");
                return;
            }
            temp_signal_ids="";
            if(!editioParSetting.checkTypeChange("sensorType0","zeroType"))
                return;
            temp_signal_ids=zero+"#";
            if(!editioParSetting.checkTypeChange("sensorType1","oneType"))
                return;
            temp_signal_ids+=one+"#";
            if(!editioParSetting.checkTypeChange("sensorType2","twoType"))
                return;
            temp_signal_ids+=two+"#";
            if(!editioParSetting.checkTypeChange("sensorType3","threeType"))
                return;
		...
		checkTypeChange:function(typeid,type){
            ...
            if(one!=""&&one!=undefined){
                if(temp_signal_ids.indexOf(one)>-1){
                    editioParSetting.showErrorMsg("存在相同的监测功能类型", typeid);//"sensorType1"
                    return false;
                }
                oneFlage=true;
            }
            ...
            if(oneFlage&&!oneTypeFlage){
                editioParSetting.showErrorMsg("请设置为常开/常闭或取消该设置信号口",type);// "oneType"
                return false;
            }
            if(oneTypeFlage&&!oneFlage){
                editioParSetting.showErrorMsg(signalChannelSetNull, typeid);
                return false;
            }
            return true;
        },
        ...
        initTypeCallback:function (data) {
            ...
            $(".sensorType").bsSuggest({
                ...
                if(sesecltTypeId.indexOf(keyword.id)>-1){
                    editioParSetting.showErrorMsg(signalChannelExist, $(this).attr("id"));
                    return;
                }
            }).on('onUnsetSelectValue', function() {

            });
           $("#sensorType0").attr("data-id",$("#signalZero").val());
           $("#sensorType1").attr("data-id",$("#signalOne").val());
           $("#sensorType2").attr("data-id",$("#signalTwo").val());
           $("#sensorType3").attr("data-id",$("#signalThree").val());
        },
		...
		//下发8102
        sendParams: function(){
        	webSocket.subscribe(headers,"/user/topic/realLocationP", editioParSetting.setDeviceData, null, null);
            ...
            json_ajax("POST", "/clbs/m/switching/signal/sendPosition", "json", false, {
                "vehicleId": vehicleId
            },editioParSetting.getSendParamCall);
        },
...
//              webSocket.subscribe(headers,"/user/" + msg.userName + "/realLocationP", editioParSetting.setDeviceData, "/app/vehicle/realLocationP", requestStrS);
            }
        },
        setDeviceData:function (data) {
            ...
            if (data.msgHead.msgId=513&&cmsgSN == data.msgBody.msgSNAck) {
                ...
                if(ioSignalData!=null&&ioSignalData.length>0) {
                    ...
                    editioParSetting.changeColor(singal0, "zeroType");
                    var singal1 = ioSignalData[0].signal1;
                    editioParSetting.changeColor(singal1, "oneType");
                    var singal2 = ioSignalData[0].signal2;
                    editioParSetting.changeColor(singal2, "twoType");
                    var singal3 = ioSignalData[0].signal3;
                    editioParSetting.changeColor(singal3, "threeType");
                }else{
                    layer.msg("暂未获取到信号信息!");
                    return;
				}
	...
	$(function(){
		...
		$("#doSubmits").bind("click",editioParSetting.doSubmits);
        $("#ioParSettingRefresh").bind("click",editioParSetting.sendParams);
	})
})(window,$)