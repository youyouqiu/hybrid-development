(function (window, $) {
    ...
    speedSearch = {
        ...
        inquireClick: function (number) {
            ...
            var url = "/clbs/sx/sxReportManagement/sxSpeedAlarmReport/list";
            var parameter = {
        ...
        exportAlarm: function () {
            ...
            var url = "/clbs/sx/sxReportManagement/sxSpeedAlarmReport/export";
        ...
        zTreeBeforeCheck: function (treeId, treeNode) {
            ...
            if (!treeNode.checked) {
                if (treeNode.type == "group" || treeNode.type == "assignment") { //若勾选的为组织或分组
                    ...
                    // /clbs/m/personalized/ico/getCheckedVehicle
        ...
        getCallback: function (date) {
            ...
            if (date.success == true) {
                ...
                if (date.obj.list != null && date.obj.list.length != 0) {
                    ...
                    for (var i = 0; i < alarm.length; i++) {
                        ...
                        var speedType=speedSearch.getSpeedType(alarm[i].speedType);
                        var dateList =
                            [
                                i + 1,
                                alarm[i].brnad,
                                ...
                                speedType,
                                alarm[i].alarmStartTimeStr,
                                alarm[i].alarmEndTimeStr,
                                alarm[i].speed,
                                alarm[i].durationTime,
                                "加载中...",
                                "加载中...",
                                alarm[i].professionalName,
                                alarm[i].phone
                            ];
        ...
        goBack: function (GeocoderResult) {
            ...
            for (var i = 0; i < dataLength; i++) {
                if (msgArray[i] != undefined) {
                    $dataTableTbody.children("tr:nth-child(" + (startIndex + 1) + ")").children("td:nth-child(12)").text(msgArray[i][0]);
                    $dataTableTbody.children("tr:nth-child(" + (startIndex + 1) + ")").children("td:nth-child(13)").text(msgArray[i][1]);
                    startIndex++;
                }
            }
        },
        ...
        getAddress: function (startStr,endStr) {
            ...
            for (var i = 0; i < dataLength; i++) {
                ...
                if (num == dataLength) {
                    var addressIndex = 0;
                    var addressArray = [];
                    backAddressMsg(addressIndex, addressMsg, speedSearch.goBack, addressArray);
                    addressMsg = [];
                }
            }
            ;
        ...
        getTable: function (table) {
            myTable = $(table).DataTable({
                ...
                "drawCallback": function (settings) {
                    ...
                    var $dataTableTbody = $("#dataTable tbody");
                    var alarmStartLocation = $dataTableTbody.children("tr:last-child").children("td:nth-child(12)").text();
                    var alarmEndLocation = $dataTableTbody.children("tr:last-child").children("td:nth-child(13)").text();
                    //报警位置进行逆地址解析
                    if ((alarmStartLocation == "加载中..." || alarmEndLocation == "加载中...")&&!getAddressStatus) {
                        speedSearch.getAddress(newStartLoc,newEndLoc);
                    }
                },
        ...
        showMenu: function (e) {
            ...
            $("body").bind("mousedown", speedSearch.onBodyDown);
        },
        hideMenu: function () {
            $("#menuContent").fadeOut("fast");
            $("body").unbind("mousedown", speedSearch.onBodyDown);
        },
        onBodyDown: function (event) {
            if (!(event.target.id == "menuBtn" || event.target.id == "menuContent" || $(event.target).parents("#menuContent").length > 0)) {
                speedSearch.hideMenu();
            }
        },
        //模糊查询树
    ...
    $(function () {
        ...
        $("#groupSelectSpan,#groupSelect").bind("click", speedSearch.showMenu); //组织下拉显示
