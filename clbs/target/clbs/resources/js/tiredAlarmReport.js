(function (window, $) {
    ...
    tiredAlarmReport = {
        ...
        startDay: function (day) {
            ...
            if (startValue == "" || endValue == "") {
                ...
                tMonth = tiredAlarmReport.doHandleMonth(tMonth + 1);
                tDate = tiredAlarmReport.doHandleMonth(tDate);
                var num = -(day + 1);
                ...
                endMonth = tiredAlarmReport.doHandleMonth(endMonth + 1);
                endDate = tiredAlarmReport.doHandleMonth(endDate);
                ...
                vMonth = tiredAlarmReport.doHandleMonth(vMonth + 1);
                vDate = tiredAlarmReport.doHandleMonth(vDate);
                ...
                if (day == 1) {
                    ...
                    vendMonth = tiredAlarmReport.doHandleMonth(vendMonth + 1);
                    vendDate = tiredAlarmReport.doHandleMonth(vendDate);
        ...
        zTreeOnAsyncSuccess: function (event, treeId, treeNode, msg) {
            ...
            if (size <= 100 && ifAllCheck) {
                ...
                tiredAlarmReport.getCharSelect(treeObj);
            }
            bflag = false;
        },
        ...
        onCheckVehicle: function (e, treeId, treeNode) {
            ...
            if (treeNode.checked) {
                ...
                setTimeout(() => {
                    tiredAlarmReport.getCheckedNodes();
                    tiredAlarmReport.validates();
                }, 600);
            }
            crrentSubV = [];
            crrentSubV.push(treeNode.id);
            tiredAlarmReport.getCharSelect(zTree);
            tiredAlarmReport.getCheckedNodes();
        },
            ...
            if (nodes != null && nodes != undefined && nodes.length > 0) {
                for (var i = 0; i < nodes.length; i++) {
                    ...
                    if (node.type == "assignment") {
                        ...
                        tiredAlarmReport.getGroupChild(node.children, assign);
                    }
                }
            }
        },
        ...
        inquireClick: function (number) {
            ...
            if (number == 0) {
                tiredAlarmReport.getsTheCurrentTime();
            } else if (number == -1) {
                tiredAlarmReport.startDay(-1)
            } else if (number == -3) {
                tiredAlarmReport.startDay(-3)
            } else if (number == -7) {
                tiredAlarmReport.startDay(-7)
            }
            if (number != 1) {
            ...
            tiredAlarmReport.getCheckedNodes();
            if (!tiredAlarmReport.validates()) {
                return;
            }
            var url = "/clbs/sx/sxReportManagement/tiredAlarmReport/list";
            var parameter = {"vehicleList": vehicleId, "startTime": startTime, "endTime": endTime};
            json_ajax("POST", url, "json", true, parameter, tiredAlarmReport.getCallback);
        },
        exportAlarm: function () {
            if (getRecordsNum() > 60000) {
                return layer.msg("导出数据量过大，请缩小查询范围再试（单次导出最多支持6万条）！");
            }
            var url = "/clbs/sx/sxReportManagement/tiredAlarmReport/export";
                    ...
                    for (var i = 0; i < list.length; i++) {
                        var dateList =
                            [
                                ...
                                list[i].color,
                                list[i].vehicleType,
                                list[i].alarmStartTimeStr,
                                "加载中...",
                                list[i].professionalName,
                                list[i].phone
                            ];
                        dataListArray.push(dateList);
                        stopLoc.push(list[i].alarmStartLocation);
                    }
                    $("#simpleQueryParam").val("");
                    tiredAlarmReport.reloadData(dataListArray);
                } else {
                    tiredAlarmReport.reloadData(dataListArray);
                    $("#simpleQueryParam").val("");
            ...
            myTable = $(table).DataTable({
                ...
                "drawCallback": function (settings) {
                    ...
                    if (alarmLocation == "加载中..." && !getAddressStatus) {
                        tiredAlarmReport.getAddress(newStopLoc);
                    }
                },
                "order": [
                    [0, null]
        ...
        showMenu: function (e) {
            ...
            $("body").bind("mousedown", tiredAlarmReport.onBodyDown);
        },
        hideMenu: function () {
            $("#menuContent").fadeOut("fast");
            $("body").unbind("mousedown", tiredAlarmReport.onBodyDown);
        },
        onBodyDown: function (event) {
            if (!(event.target.id == "menuBtn" || event.target.id == "menuContent" || $(event.target).parents("#menuContent").length > 0)) {
                tiredAlarmReport.hideMenu();
            }
        },
        //模糊查询树
            ...
            if (param == null || param == undefined || param == '') {
                ...
                var querySetting = {
                    async: {
                        ...
                        dataFilter: tiredAlarmReport.ajaxQueryDataFilter
                    },
                    check: {
                        enable: true,
                        chkStyle: "checkbox",
                    ...
                    callback: {
                        beforeClick: tiredAlarmReport.beforeClickVehicle,
                        onCheck: tiredAlarmReport.onCheckVehicle,
                        onExpand: tiredAlarmReport.zTreeOnExpand,
                        onNodeCreated: tiredAlarmReport.zTreeOnNodeCreated
                    }
                };
        ...
        treeInit: function () {
            ...
            var setting = {
                async: {
                    url: tiredAlarmReport.getTreeUrl,
                    type: "post",
                    enable: true,
                    autoParam: ["id"],
                    dataType: "json",
                    otherParam: {"type": "multiple", "icoType": "0"},
                    dataFilter: tiredAlarmReport.ajaxDataFilter
                },
                check: {
                    enable: true,
                    chkStyle: "checkbox",
                ...
                callback: {
                    beforeClick: tiredAlarmReport.beforeClickVehicle,
                    onCheck: tiredAlarmReport.onCheckVehicle,
                    beforeCheck: tiredAlarmReport.zTreeBeforeCheck,
                    onExpand: tiredAlarmReport.zTreeOnExpand,
                    onAsyncSuccess: tiredAlarmReport.zTreeOnAsyncSuccess,
                    onNodeCreated: tiredAlarmReport.zTreeOnNodeCreated
                }
            };
    ...
    $(function () {
        ...
        //初始化页面
        tiredAlarmReport.treeInit();
        tiredAlarmReport.getTable('#dataTable');
        //当前时间
        tiredAlarmReport.getsTheCurrentTime();
        $('#timeInterval').dateRangePicker({
            dateLimit: 31
        });
        $("#groupSelectSpan,#groupSelect").bind("click", tiredAlarmReport.showMenu); //组织下拉显示
        $('input').inputClear().on('onClearEvent', function (e, data) {
            var id = data.id;
            if (id == 'groupSelect') {
                var param = $("#groupSelect").val();
                tiredAlarmReport.searchVehicleTree(param);
            }
            ;
        });
        /**
