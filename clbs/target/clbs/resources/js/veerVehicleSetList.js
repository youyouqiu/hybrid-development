(function(window,$){
	...
	veerMonitorset = {
		...
		init: function(){
		    ...
		    // 接收到消息
            // setTimeout(function () {
             //    webSocket.subscribe(headers,'/topic/fencestatus', veerMonitorset.updataFenceData,null, null);
            // },500);
		    ...
		    },
	        {
	            ...
	            render: function (data, type, row, meta) {
					...
					var bindUrlPre = '/clbs/v/veerManagement/veerVehicleSet/bind_{id}';
					var detailUrlPre = '/clbs/v/veerManagement/veerVehicleSet/detail_{id}';
					var result = '';
					if (row.sensorNumber == null || row.sensorNumber == "") {
						// 设置
                        ...
                        // 下发参数
                        result += '<button onclick="veerMonitorset.sendveerOneVechice(\''+row.id+'\')" type="button" class="editBtn editBtn-info"><i class="fa fa-issue"></i>下发参数</button>&ensp;';
                        // 详细信息
                    ...
                    "class" : "text-center",
                    render : function(data, type, row, meta) {
                    	return veerMonitorset.renderStatus(row,false);
                    }
            }, {
	            "data": "brand",
	            "class": "text-center"
		    ...
		    var setting = {
		        ...
		        editUrl: '/clbs/v/veerManagement/veerVehicleSet/edit_',
	        ...
	        var treeSetting = {
        		async : {
        			...
        			dataFilter: veerMonitorset.ajaxDataFilter
        		},
                view : {
        			selectedMulti : false,
        			nameIsHTML: true,
        		...
        		callback : {
        			onClick : veerMonitorset.zTreeOnClick
        		}
        	};
		...
		updataFenceData: function(msg){
	        if (msg != null) {
	            ...
	            if (result != null) {
					veerMonitorset.updateColumn(result.desc.monitorId);
	            }
	        }
	    },
	    //批量删除
        ...
        sendModelClick: function(){
	        ...
	        // 下发方法调用
	       veerMonitorset.sendVeer(JSON.stringify(checkedList));
        },
        sendVeer: function(sendParam){//批量下发执行
            var url="/clbs/v/sensorSettings/sendSetDeviceParam";
            var parameter={"sendParam": sendParam,"sensorType":3,"flag":1};
            json_ajax("POST",url,"json",true,parameter,veerMonitorset.sendVeerCallback);
	    },
	    // 下发流量回调方法
	    sendVeerCallback: function(data){
        	if(veesendflag){
        		//修復订阅后通用应答上来一直刷list接口问题
                webSocket.subscribe(headers,"/user/topic/forward_and_backward", veerMonitorset.updataFenceData,null, null);
                veesendflag=false;
			}
	    ...
	    sendveerOneVechice: function(id){// 下发参数 （单个）
			var arr = [];
			var obj = {};
			obj.id = id;
			arr.push(obj);
			var jsonStr = JSON.stringify(arr);
			veerMonitorset.sendVeer(jsonStr);
		},
		...
		updateColumn: function (id){
			for(var i= 0; i< tableList.length; i++) {
				if(id == tableList[i].vehicleId){
					var url = '/clbs/v/sensorSettings/refreshSendStatus?vehicleId=' + id + '&sensorType=3';
					json_ajax('GET', url, 'json', false, null, function (messages) {
						if(messages.success){
							var data = messages.obj;
							$("#status_" + id).text(veerMonitorset.renderStatus(data, true));
						}
					});
				}
			}
	...
	$(function(){
		...
		//全选
	    $("#checkAll").bind("click",veerMonitorset.checkAllClick);
	    subChk.bind("click",veerMonitorset.subChkClick);
	    //批量删除
	    $("#del_model").bind("click",veerMonitorset.delModelClick);
	    //刷新
	    $("#refreshTable").on("click",veerMonitorset.refreshTable);
		//批量下发
	    $("#send_model").bind("click",veerMonitorset.sendModelClick);
