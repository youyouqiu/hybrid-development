(function(window,$){
	...
	// 振动传感器list  
	var vibrationSensorList = JSON.parse($("#vibrationSensorList").attr("value"));
	// 初始化车辆数据 
    var dataList = {value: []};
 	// 初始化振动传感器 
	var dataVibrationList = {value: []};
	settingHourlyVehicle = {
		 init: function(){
		  		// 初始化振动传感器 
			  	...
			  	$("#outputCorrectionK").blur(function() {
			  		settingHourlyVehicle.checkOutputCorrectionCoefficientKModify(this); // 判断当前修正系数是否被修改
				});
				$("#outputCorrectionB").blur(function() {
					settingHourlyVehicle.checkOutputCorrectionCoefficientBModify(this); // 判断当前修正系数是否被修改
				});
        ...
        hideErrorMsg: function(){
		    ...
		    // 提交
		    doSubmit: function(){
                settingHourlyVehicle.hideErrorMsg();
                var vibrationSensor = $("#vibrationSensor").attr("data-id");
                if (!vibrationSensor) {
                    settingHourlyVehicle.showErrorMsg(sensorNull, "vibrationSensor");
                    return;
                }
		    	if(settingHourlyVehicle.validates()){
	...
	$(function(){
		...
		settingHourlyVehicle.init();
		$('input').inputClear();
		//提交
		$("#doSubmit").bind("click",settingHourlyVehicle.doSubmit);
	})
})(window,$)