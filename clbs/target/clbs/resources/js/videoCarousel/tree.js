var Tree = function (selector, options, dependency) {
    ...
    this.aliasesClass = 'aliasesStyle';// 监控对象别名样式

    this.MAXCHECKNUM = 5000;// 最多勾选数量
};
...
Tree.prototype.getTreeUrlAndParam = function () {
    ...
    if (treeType === 'all') {
        ...
        if (offlineNum <= 300) {
            ...
            param = {"type": 7, "webType": 1, "isCarousel": 1};
        } else {
            url = "/clbs/m/functionconfig/fence/bindfence/bigDataMonitorTree";
        }
        ...
        var queryParam = $("#search_condition").val();
        param = {"queryParam": queryParam, "queryType": queryType, "webType": 1, "isCarousel": 1};
    } else {
        url = "/clbs/m/basicinfo/monitoring/vehicle/treeStateInfo";
        if (treeType === 'online') {
            param = {"type": 1, "webType": 1, "isCarousel": 1};
        } else if (treeType === 'heartBeat') {
            param = {"type": 9, "webType": 1, "isCarousel": 1};
        } else if (treeType === 'notPosition') {
            param = {"type": 6, "webType": 1, "isCarousel": 1};
        } else if (treeType === 'run') {
            param = {"type": 3, "webType": 1, "isCarousel": 1};
        } else if (treeType === 'stop') {
            param = {"type": 2, "webType": 1, "isCarousel": 1};
        } else if (treeType === 'overSpeed') {
            param = {"type": 5, "webType": 1, "isCarousel": 1};
        } else if (treeType === 'alarm') {
            param = {"type": 4, "webType": 1, "isCarousel": 1};
        }
    }
...
Tree.prototype.loadTree = function () {
    ...
    var setting = {
        ...
        view: {
            ...
            nameIsHTML: true,
            aliasesClass: this.aliasesClass,
...
Tree.prototype.zTreeOnAsyncError = function (event, treeId, treeNode, XMLHttpRequest, textStatus, errorThrown) {
    if(textStatus == 'error') {
...
Tree.prototype.ajaxDataFilter = function (treeId, parentNode, responseData) {
    var dataDependency = this.dependency.get('data');
    var checkMonitorArray = dataDependency.getCheckMonitorArray();// 已勾选的监控对象
    var treeType = dataDependency.getTreeType();
    switch (treeType) {
        case 'all':
        case 'offline':
    ...
    for (var i = responseData.length - 1; i >= 0; i--) {
        ...
        if (checkMonitorArray.indexOf(responseData[i].id) !== -1) {
            responseData[i].checked = true;
        }
        if (treeType === 'offline' && responseData[i].type === 'assignment') {
            if (responseData[i].offLine === 0) {// 离线树,过滤离线监控对象数为0的分组
                responseData.splice(i, 1);
            }
        }
    }
    if (treeType === 'online' || treeType === 'search') {
...
// 为监控对象添加通道号节点
Tree.prototype.getGroupList = function (treeNode, flag) {
    var treeObj = $.fn.zTree.getZTreeObj("treeDemo");
    var logicChannelArr = treeNode.logicChannel ? treeNode.logicChannel.split(',') : [];
    var physicsChannelArr = treeNode.physicsChannel ? treeNode.physicsChannel.split(',') : [];
    var connectionFlagArr = treeNode.connectionFlag ? treeNode.connectionFlag.split(',') : [];
    var channelTypeArr = treeNode.channelType ? treeNode.channelType.split(',') : [];
    var streamTypeArr = treeNode.streamType ? treeNode.streamType.split(',') : [];
    var mobileArr = treeNode.mobile ? treeNode.mobile.split(',') : [];
    var channelLen = logicChannelArr.length;
    var newNodes = [];
    for (var i = 0; i < channelLen; i++) {
        var obj = {
            ...
            "type": "channel",
            "sort": i,
            "name": logicChannelArr[i],
            "iconSkin": "channelSkin",
            "logicChannel": logicChannelArr[i],
            "connectionFlag": connectionFlagArr[i],
            "physicsChannel": physicsChannelArr[i],
            "channelType": channelTypeArr[i],
    ...
    var addV = treeObj.addNodes(treeNode, newNodes, flag);
    /*if (addV != null && (treeNode.type == 'vehicle' || treeNode.type == 'thing' || treeNode.type === 'people')) { // 通道图标更换
...
Tree.prototype.zTreeOnExpand = function (event, treeId, treeNode) {
    ...
    var treeType = this.dependency.get('data').getTreeType();
    treeNodeNew = treeNode; //获取当前展开节点
    var _this = this;
    if (treeNode.pId !== null) {
        if (treeType !== 'online' && treeNode.children === undefined && treeNode.type == "assignment") {
            this.getChildNode4Assignment(treeNode);
        } else if (treeType !== 'online' && treeNode.type == "group") {
            ...
            if (!treeNode.children) {
                _this.getGroupList(treeNode, false)
            }
            // });
        } else {
...
Tree.prototype.getChildNode4Assignment = function (treeNode) {
    ...
    var url = "/clbs/lkyw/videoCarousel/getMonitor";
    json_ajax("post", url, "json", false, {
        "type": "assignment", "id": treeNode.id, "isChecked": treeNode.checked
    }, function (data) {
        var nodes = data.obj[treeNode.id];
        if (!nodes) return;
        for (var i = 0; i < nodes.length; i++) {
            var vId = nodes[i].id;
            if (checkMonitorArray.indexOf(vId) !== -1) {
        ...
        if (addV !== null) {
            var param = [];
            for (var i = 0; i < treeNode.children.length; i++) {
                var vId = treeNode.children[i].id;
                param.push(vId);
            }
            if (treeNode.checked && treeNode.children.length > 0) {
                this.batchUpdateVideoPlayArray(treeNode.children, true);
            }
            this.subscribeStatus(param);
            if (subscribVidArray.length !== 0) {
                for (var i = 0; i < subscribVidArray.length; i++) {
...
Tree.prototype.getChildNode4Group = function (treeNode) {
    ...
    var url = "/clbs/lkyw/videoCarousel/getMonitor";
    json_ajax("post", url, "json", false, {
        "type": "group", "id": treeNode.id, "isChecked": treeNode.checked
    }, function (data) {
        var result = data.obj;
        if (result != null && result != undefined) {
            $.each(result, function (i) {
                ...
                if (!zTreeIdJson[pid]) return;
                var parentTid = zTreeIdJson[pid][0];
                var parentNode = treeObj.getNodeByTId(parentTid);
                for (var i = 0; i < chNodes.length; i++) {
                    chNodes[i].isParent = true;
                    chNodes[i].open = false;
                    if (checkMonitorArray.indexOf(chNodes[i].id) !== -1) {
                        chNodes[i].checked = true;
                    }
                }
                if (parentNode.children === undefined) {
                    treeObj.addNodes(parentNode, chNodes);
                    if (treeNode.checked && chNodes.length > 0) {
                        _this.batchUpdateVideoPlayArray(chNodes, true);
                    }
                    if (chNodes.length > 0) {
                        var param = [];
                        for (var i = 0; i < chNodes.length; i++) {
                            if (chNodes[i].id) {
                                param.push(chNodes[i].id)
                            }
                        }
                        if (param.length > 0) {
                            _this.subscribeStatus(param);
                        }
                    }
                }
            });
...
Tree.prototype.cancelSubscribeLocation = function () {
    ...
    webSocket.unsubscribealarm(headers, "/app/vehicle/location/unsubscribe", requestStrS);
};
...
Tree.prototype.updateTreeNodeColor = function (status, vid) {
    ...
    switch (status) {
        ...
        case 'overSpeed': // 超速
            ...
            break;*/
        case 'offline': // 离线
            iconClass = 'btnImage iconArea vehicleSkin';
            textColor = '#b6b6b6';
            break;
        default:
            iconClass = 'btnImage iconArea icon-online';
            textColor = '#78af3a';
            break;
    }
...
Tree.prototype.treeBeforeCheck = function (treeId, treeNode) {
    var dataDependency = this.dependency.get('data');
    var checkNodesArray = dataDependency.getVideoPlayArray();
    var checkNum = checkNodesArray.length;
    var treeType = dataDependency.getTreeType();

    if (treeNode.type !== "group" && treeNode.type !== "assignment" && !treeNode.logicChannel) {
        layer.msg('该监控对象未设置通道号');
        return false;
    }
    if (!treeNode.checked && checkNum === this.MAXCHECKNUM) {
        layer.msg('最多勾选' + this.MAXCHECKNUM + '个监控对象');
        return false;
    }
    if (treeType !== 'online' && !treeNode.checked && (treeNode.type === 'group' || treeNode.type === 'assignment')) {
        var queryParam = $("#search_condition").val();
        if (queryParam !== '') {
            var treeObj = $.fn.zTree.getZTreeObj(treeId);
            var changeNodes = treeObj.getNodesByFilter(function (node) {
                return node.type !== "group" && node.type !== "assignment" && !node.checked;
            }, false, treeNode);
            checkNum += changeNodes.length;
        } else {
            json_ajax("post", "/clbs/m/personalized/ico/getCheckedVehicle",
                "json", false, {"parentId": treeNode.id, "type": treeNode.type},
                function (data) {
                    if (data.success) {
                        checkNum += data.obj;
                    }
...
Tree.prototype.treeOnCheck = function (e, treeId, treeNode) {
    if (!treeNode) return;
    console.log(treeNode, 'treeNode33333333');
    var treeObj = $.fn.zTree.getZTreeObj("treeDemo");
    var checked = treeNode.checked;
    var nodes = treeObj.getCheckedNodes(true);
    var list = [];
    for(var i=0; i< nodes.length; i++) {
        if(nodes[i].type == "vehicle" && nodes[i].checked == true) {
    ...
    if (checked) {
        ...
        if (treeNode.type === "group" || treeNode.type === "assignment") {
            ...
            if (changeNodes.length > 0) {
                this.batchUpdateVideoPlayArray(changeNodes, true);
            }
            this.zTreeOnExpand(e, treeId, treeNode);
        } else {
            if (!treeNode.logicChannel) {
                layer.msg('该监控对象未设置通道号');
                return;
            }
            this.updateVideoPlayArray(treeNode, true);
        }
        ...
        if (changeNodes.length > 0) {
            this.batchUpdateVideoPlayArray(changeNodes, false);

        }
    } else {
        this.updateVideoPlayArray(treeNode, false);
    }
};
 ...
 * 更新轮播队列数据
 * @param
 * treeNode:监控对象节点,checkStatus:勾选或者取消勾选
 * */
// 批量更新
Tree.prototype.batchUpdateVideoPlayArray = function (treeNodeArr, checkStatus) {
    var dataDependency = this.dependency.get('data');
    var videoPlayArray = dataDependency.getVideoPlayArray();// 轮播队列
    var checkMonitorArray = dataDependency.getCheckMonitorArray();// 已勾选监控对象
    var monitorChannelObj = dataDependency.getMonitorChannelObj();// 监控对象在轮播队列中的通道数量
    ...
    for (var j = 0; j < treeNodeArr.length; j++) {
        var treeNode = treeNodeArr[j];
        var monitorId = treeNode.id;
        if (checkStatus && treeType === 'offline' && onlineList.indexOf(monitorId) !== -1) continue;
        if (checkStatus && !monitorChannelObj[monitorId]) {
            var logicChannelArr = treeNode.logicChannel ? treeNode.logicChannel.split(',') : [];
            var panoramicArr = treeNode.panoramic ? treeNode.panoramic.split(',') : [];
            var physicsChannelArr = treeNode.physicsChannel ? treeNode.physicsChannel.split(',') : [];
            var channelTypeArr = treeNode.channelType ? treeNode.channelType.split(',') : [];
            var streamTypeArr = treeNode.streamType ? treeNode.streamType.split(',') : [];
            var mobileArr = treeNode.mobile ? treeNode.mobile.split(',') : [];
            var channelLen = logicChannelArr.length;
            monitorChannelObj[monitorId] = channelLen;
            for (var i = 0; i < channelLen; i++) {
                var obj = {
                    "vehicleId": monitorId,
                    "monitorName": treeNode.name,
                    "channelNum": logicChannelArr[i],
                    "physicsChannelNum": physicsChannelArr[i],
                    "channelType": channelTypeArr[i],
                    "streamType": streamTypeArr[i],
                    "requestType": 0,
                    "orderType": 11,
                    "mobile": mobileArr[i],
                    "panoramic": panoramicArr[i],
                };
                videoPlayArray.push(obj);
            }

        } else if (!checkStatus) {
            var channelNum = monitorChannelObj[monitorId];
            if (channelNum > 0) {
                var startIndex = 0;
                for (var i = 0; i < videoPlayArray.length; i++) {
                    var item = videoPlayArray[i];
                    if (item.vehicleId === monitorId) {
                        startIndex = i;
                        break
                    }
                }
                monitorChannelObj[monitorId] = 0;
                videoPlayArray.splice(startIndex, channelNum);
            }
        }
        var checkIndex = checkMonitorArray.indexOf(monitorId);
        if (checkStatus && checkIndex === -1) {
            checkMonitorArray.push(monitorId);
        } else if (!checkStatus && checkIndex !== -1) {
            checkMonitorArray.splice(checkIndex, 1);
        }
    }
    dataDependency.setVideoPlayArray(videoPlayArray);// 更新轮播队列
    dataDependency.setMonitorChannelObj(monitorChannelObj);// 更新监控对象在轮播队列中的索引位置
    dataDependency.setCheckMonitorArray(checkMonitorArray);// 更新已勾选监控对象
};
// 单条更新
Tree.prototype.updateVideoPlayArray = function (treeNode, checkStatus) {
    if (checkStatus && treeNode.isHidden) return;
    var monitorId = treeNode.id;
    var dataDependency = this.dependency.get('data');
    var videoPlayArray = dataDependency.getVideoPlayArray();// 轮播队列
    var checkMonitorArray = dataDependency.getCheckMonitorArray();// 已勾选监控对象
