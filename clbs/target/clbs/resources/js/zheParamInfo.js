...
(function ($, window) {
    ...
    var paramType = 64,
        protocolType = 16;//浙标
    //获取参数设置信息
    var sensorIDs = [100, 101, 102];//[前向监测, 驾驶员监测, 盲区]
    var socketTypes = [62308, 62309, 62310];//[前向监测, 驾驶员监测, 盲区]
    //参数下发风险事件id
    var arr0 = ['', '', 166401, 166402, 166403, 166404, 166405, 166407 , 166410, 166409],//前向事件id,''表示公共参数
        arr1 = ['', '', 166513, 166503, 166502, 166508, 166504, 166515, 166516, 166514, 166517],//驾驶员事件id
        arr2 = [166704, '']; //盲区事件id
    var paramTypes = [64, 65, 67];//[前向监测, 驾驶员监测, 盲区]
    ...
    zheParamInfo = {
        init: function () {
            ...
            for(var i=0;i<socketTypes.length;i++){
                ...
                zheParamInfo.subscribeSocket(socketType);//socket订阅
                zheParamInfo.getPeripheralInfo(sensorID);//传感器参数下发
            }
        },
        /**
         * socket订阅
        ...
        subscribeSocket: function (type) {
            webSocket.subscribe(headers, "/user/topic/per" + type + "Info", function (data) {
                zheParamInfo.getSensor0104Param(data, type);
            }, null, null);
        },
        /**
         * 传感器参数指令下发
        ...
        getPeripheralInfo: function (sensorID) {
            ...
            json_ajax("POST", url, "json", false, {
                ...
                "sensorID": sensorID,
                "commandType": 243
            }, zheParamInfo.setreadConventionalCall);
        },
        setreadConventionalCall: function (data) {
            if (data.success) {
                ...
                zheParamInfo.createSocket0104InfoMonitor(data.msg);
            } else {
                if (data.msg) {
                    layer.msg(data.msg);
                }
        ...
        getSensor0104Param: function (msg, type) {
            ...
            zheParamInfo.setSenorInputData(param, type);
        ...
        setSenorInputData: function (result, type) {
            var dataTypeMap = {
                ...
                62310: result.deadZoneDetection,//盲区
            };
            ...
            for (var key in data) {
                ...
                switch (key) {
                    case 'driveDeedType':
                    case 'cameraStrategy':
                        value = zheParamInfo.getCaptureType(value);
                        dom.val(value);
                        break;
                    case 'eventEnable':
                    case 'alarmEnable':
                        zheParamInfo.getBinaryValue(value, tabId, key);
                        break;
                    case 'cameraResolution':
                        var dataValue = '0x0' + parseInt(value, 10).toString(16);
                        value = zheParamInfo.changeCameraResolution(value);
                        dom.val(value).attr('data-value', dataValue);
                        break;
                    case 'videoResolution':
                        var dataValue = '0x0' + parseInt(value, 10).toString(16);
                        value = zheParamInfo.changeVideoResolution(value);
                        dom.val(value).attr('data-value', dataValue);
                        break;
                    case 'fixedCamera':
                        value = value / 1000;
                    ...
                    case 'unit'://胎压单位
                        ...
                        break;
                    case 'tyreNumber':
                        dom.val(value).attr('data-value',zheParamInfo.tireModel(value));
                        break;
                    default:
                        dom.val(value);
                        break;
        ...
        getPlatformData: function () {
            ...
            for (var i = 0; i < platformData.length; i++) {
                ...
                if (i < 2) {//有公共参数的页签
                    zheParamInfo.getCommonParamSetting(item.commonParamSetting, tab);
                }
                zheParamInfo.getAdasAlarmParamSettings(item.adasAlarmParamSettings, tab);
            }
        },
        //全局参数
            ...
            for (var key in data) {
                ...
                switch (key) {
                    ...
                    case 'cameraResolution'://拍照分辨率
                        ...
                        value = zheParamInfo.changeCameraResolution(num);
                        break;
                    case 'videoResolution'://视频分辨率
                        var num = Number(parseInt(value, 16).toString(10));
                        value = zheParamInfo.changeVideoResolution(num);
                        break;
                    case 'photographTime':
                        value = value / 10;
                        break;
        ...
        getAdasAlarmParamSettings: function (data, tab) {
            for (var i = 0; i < data.length; i++) {
                ...
                switch (riskId) {
                    case 166704: //盲区
                        zheParamInfo.setInputValue(item,riskEvent.eq(0));
                        break;
                    case 166401://前向碰撞
                    case 166513://疲劳驾驶
                        zheParamInfo.setInputValue(item, riskEvent.eq(1));
                        break;
                    case 166402://车道偏离
                    case 166503://抽烟
                        zheParamInfo.setInputValue(item, riskEvent.eq(2));
                        break;
                    case 166403://车距过近
                    case 166502://接打手持电话
                        zheParamInfo.setInputValue(item, riskEvent.eq(3));
                        break;
                    case 166404://行人碰撞
                    case 166508://分神驾驶
                        zheParamInfo.setInputValue(item, riskEvent.eq(4));
                        break;
                    case 166405://频繁变道
                    case 166504://驾驶行为异常
                        zheParamInfo.setInputValue(item, riskEvent.eq(5));
                        break;
                    case 166407://障碍物
                    case 166515://换人驾驶报警
                        zheParamInfo.setInputValue(item, riskEvent.eq(6));
                        break;
                    case 166410://快速通过路口
                    case 166516://探头遮挡报警
                        zheParamInfo.setInputValue(item, riskEvent.eq(7));
                        break;
                    case 166409://道路标识超限
                    case 166514://超时驾驶报警
                        zheParamInfo.setInputValue(item, riskEvent.eq(8));
                        break;
                    case 166517://驾驶员身份异常
                        zheParamInfo.setInputValue(item, riskEvent.eq(9));
                        break;
                    default:
                        break;
                }
        ...
        paramSendAllClick: function () {
            var self = $(this);
            zheParamInfo.preventclick(self);
            isAll = true;
            var jsonDataArr = [];
            var riskIds = [arr0, arr1, arr2];//前向,驾驶员,盲区风险事件id

            for (var i = 0; i < riskIds.length; i++) {
                paramType = paramTypes[i];
                var jsonData = zheParamInfo.setJsonData(riskIds[i], $('.tab-pane').eq(i));
                jsonDataArr.push(jsonData[0]);
            }
            zheParamInfo.dsmInfoSend(jsonDataArr);
        },
        paramSendClick: function () {
            ...
            var riskIds = [arr0, arr1, arr2];
            var arr = riskIds[inx];
            paramType = tab.data('type');

            var jsonData = zheParamInfo.setJsonData(arr, tab);
            zheParamInfo.dsmInfoSend(jsonData);
        },
        //组装下发参数
        setJsonData: function (arr, tab) {
            ...
            for (var i = 0; i < arr.length; i++) {
                ...
                if (item == '') {//全局参数/主动抓拍策略
                    ...
                    commonParamSetting = zheParamInfo.getCommontData(inputs);
                } else {//风险事件
                    inputs = tab.find('.info-content').eq(i).find('input');
                    adasAlarmParamSettings.push(zheParamInfo.getInputData(item, inputs));
                }
            }
        ...
        dsmInfoSend: function (jsonData) {
            ...
            json_ajax("POST", "/clbs/adas/standard/param/setting.gsp", "json", false, data, zheParamInfo.dsmInfoSendCallback);
        },
        dsmInfoSendCallback: function (data) {
            if (data != null) {
                if (data.success) {
                    ...
                    //获取下发状态
                    setTimeout(function () {
                        timerInterval = setInterval(zheParamInfo.getStatus(), 5000);
                    }, 2000);
                    setTimeout(function () {
            ...
            json_ajax("POST", url, "json", false, {
                ...
                if (data.success) {
                    ...
                    if (obj) {
                        ...
                        for (var key in obj) {
                            var value = obj[key];
                            status.push(zheParamInfo.statusChange(value));
                        }
                        ...
                        timer = setTimeout(function () {
                            ...
                            zheParamInfo.getPeripheralInfo(sensorID);
                        }, 1000);
                    }
    ...
    $(function () {
        ...
        //事件
        $(".info-span").on("click", zheParamInfo.hiddenparameterFn);//点击显示隐藏信息
        $("#dsmInfoSend").on("click", zheParamInfo.paramSendAllClick);//全部修正下发
        $("#dsmInfoSend2").on("click", zheParamInfo.paramSendClick);//本页签修正下发
        $("#dsmInfoRefresh").on("click", zheParamInfo.readConventionalRefreshClick);//刷新
        $(".sensorBtn").on("click", zheParamInfo.sensorBtnClick);//以传感器为准
        $(".platformBtn").on("click", zheParamInfo.platformBtnClick);//以平台设置为准
    })
})($, window);