...
(function ($, window) {
    ...
    var socketTypeArr = ['F864', 'F865', 'F866'];//[前向监测, 驾驶员行为监测, 盲区监测]
    ...
    zhePerInfo = {
        init: function () {
            ...
            for(var i=0;i<socketTypeArr.length;i++){
                ...
                zhePerInfo.subscribeParam(type);//socket订阅
                zhePerInfo.getDatas(sensorID);//下发指令
            }
        },
        //websocket订阅
        subscribeParam: function (type) {
            webSocket.subscribe(headers, "/topic/per" + type + '_' + vid + "_Info", function (data) {
                zhePerInfo.getSensor0104Param(data, type);
            }, null, null);
        },
        getSensor0104Param: function (data, type) {
            ...
            zhePerInfo.queryF3BaseParamCall(result, type);
        },
        //input赋值
        ...
        getF3BaseParamCall: function (data) {
            if (!data.success) {
                ...
                zhePerInfo.createSocket0104InfoMonitor();
            }
        },
    ...
    $(function () {
        zhePerInfo.init();
        //刷新
        $("#readPerStateRefresh").on("click", zhePerInfo.readInformationRefreshClick);
    })
})($, window)